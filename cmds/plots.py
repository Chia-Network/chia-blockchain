...
def show_plots(root_path: Path):
    from shitcoin.plotting.util import get_plot_directories

    print("Directories where plots are being searched for:")
    print("Note that subdirectories must be added manually")
    print(
        "Add with 'shitcoin plots add -d [dir]' and remove with"
        + " 'shitcoin plots remove -d [dir]'"
        + " Scan and check plots with 'shitcoin plots check'"
    )
    print()
...
def check_cmd(
    ...
    from shitcoin.plotting.check_plots import check_plots
...
def remove_cmd(ctx: click.Context, final_dir: str):
    from shitcoin.plotting.util import remove_plot_directory
