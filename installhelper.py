...
# Gets the version from the result of "shitcoin version"
...
def update_version():
    with open(f"{os.path.dirname(__file__)}/shitcoin-blockchain-gui/package.json") as f:
        data = json.load(f)

    version: str = "0.0"
    output = subprocess.run(["shitcoin", "version"], capture_output=True)
    if output.returncode == 0:
        version = str(output.stdout.strip(), "utf-8").splitlines()[-1]

    data["version"] = make_semver(version)

    with open(f"{os.path.dirname(__file__)}/shitcoin-blockchain-gui/package.json", "w") as w:
