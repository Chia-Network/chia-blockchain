...
class FullNodeSimulator(FullNodeAPI):
    ...
    async def farm_new_block(self, request: FarmNewBlockProtocol):
        ...
        if bundle is not None:
            ...
            dict_h[peak.height + 1] = (program, bundle.aggregated_signature)
            fees = bundle.fees()

        current_blocks = await self.get_all_full_blocks()
        more = bt.get_consecutive_blocks(1, block_list_input=current_blocks, force_overflow=True, guarantee_block=True)
        await self.full_node.blockchain.receive_block(more[-1])
