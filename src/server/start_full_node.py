...
async def main():
    ...
    for peer in full_node.initial_peers:
        if not (host == peer.host and port == peer.port):
            ...
            peer_task = start_chia_client(peer, full_node, NodeType.FULL_NODE)
    ...
    if connect_to_farmer:
        try:
            farmer_con_task, farmer_client = await start_chia_client(full_node.farmer_peer, full_node, NodeType.FARMER)
    ...
    if connect_to_timelord:
        try:
            timelord_con_task, timelord_client = await start_chia_client(full_node.timelord_peer,
                                                                         full_node, NodeType.TIMELORD)
