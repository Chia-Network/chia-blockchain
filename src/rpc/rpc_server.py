...
class Handler:
    ...
    async def get_block(self, request) -> web.Response:
        ...
        header_hash = str_to_bytes(request_data["header_hash"])
    ...
    async def get_header(self, request) -> web.Response:
        ...
        if "header_hash" not in request_data:
            return web.HTTPBadRequest()
        header_hash = str_to_bytes(request_data["header_hash"])
        header_block: Optional[HeaderBlock] = self.full_node.blockchain.header_blocks.get(header_hash, None)
    ...
    async def open_connection(self, request) -> web.Response:
        ...
        if self.full_node.server is None or not (await self.full_node.server.start_client(target_node, None)):
            return web.HTTPInternalServerError()
        return web.Response(text="")

    async def close_connection(self, request) -> web.Response:
        request_data = await request.json()
        node_id = str_to_bytes(request_data["node_id"])
        if self.full_node.server is None:
            return web.HTTPInternalServerError()

        connections_to_close = [c for c in self.full_node.server.global_connections.get_connections()
