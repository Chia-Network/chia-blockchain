...
class Blockchain:
    def __init__(self):
        ...
        self.blocks: Dict[bytes32, TrunkBlock] = {
            genesis_trunk.header.get_hash(): genesis_trunk
        }
    ...
    def get_difficulty(self, header_hash: bytes32) -> uint64:
        return uint64(constants.STARTING_DIFFICULTY)
    ...
    def add_block(self, block: FullBlock) -> bool:
        ...
        header_hash = trunk.header.get_hash()
        ...
        if prev_hash in self.floating_demand.get(prev_trunk.prev_header_hash, ()):
            ...
            # TODO(alex): verify block is actually consistent with blockchain
            while stack:
        ...
        elif prev_trunk in self.heads:
            # Block is the child of one of our heads

            # TODO(alex): verify block is actually consistent with blockchain
            self.heads.remove(prev_trunk)
            self.heads.append(trunk)

            log.info(f"Updated heads, new heights: {[t.challenge.height for t in self.heads]}")
            added_to_head = True
        elif prev_hash in self.blocks:
            # Block is not a child of a head, and not in floating demand. It might be the child
            # of the parent of one of the heads (sibling of a head). Or a reorg. These blocks
            # need to be compared to heads.
            # TODO(alex): implement
            pass
        else:
            # Block is floating
            # TODO(alex): we want to distinguish between block invalid, and block added but did not change
            # the heads (this case)
