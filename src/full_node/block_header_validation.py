        ...
        for finished_sub_slot_n, sub_slot in enumerate(header_block.finished_sub_slots):
            ...
            if finished_sub_slot_n == 0:
                ...
                if not header_block.finished_sub_slots[
                           finished_sub_slot_n - 1].challenge_chain.get_hash() == challenge_hash:
                    return None, Err.INVALID_PREV_CHALLENGE_SLOT_HASH
            ...
            if genesis_block:
                ...
                if curr.deficit == constants.MIN_SUB_BLOCKS_PER_CHALLENGE_BLOCK:
                    ...
                    if finished_sub_slot_n == 0:
                        while curr.deficit < constants.MIN_SUB_BLOCKS_PER_CHALLENGE_BLOCK - 1 and not curr.first_in_slot:
                            curr = sub_blocks[curr.prev_hash]
                        ...
                        icc_challenge_hash = header_block.finished_sub_slots[
                            finished_sub_slot_n - 1].infused_challenge_chain.get_hash()
                        icc_iters_committed = calculate_slot_iters(constants, prev_sb.ips)
                        icc_iters_proof = icc_iters_committed
            ...
            if not sub_slot.proofs.reward_chain_slot_proof.is_valid(constants, sub_slot.reward_chain.end_of_slot_vdf,
                                                                    target_vdf_info):
    ...
    if new_slot:
        if overflow:
            ...
            challenge: bytes32 = header_block.finished_sub_slots[
                -1].challenge_chain.challenge_chain_end_of_slot_vdf.challenge_hash
        else:
...
async def validate_finished_header_block(
    ...
    required_iters, validate_unfinished_result = await validate_unfinished_header_block(constants, sub_blocks,
                                                                                        height_to_hash,
    ...
    if not genesis_block:
        overflow = is_overflow_sub_block(constants, ips, required_iters)
        deficit = calculate_deficit(constants, header_block.height, prev_sb, overflow,
