...
class WeightProofHandler:
    ...
    async def _extend_proof_of_weight(
        ...
        if recent_chain is None:
            return None
        end_height = weight_proof.recent_chain_data[-1].reward_chain_sub_block.height
    ...
    async def __create_sub_epoch_segments(
        ...
        sub_blocks = await self.blockchain.get_sub_block_records_in_range(
            se_start.height, ses_block.height + self.constants.MAX_SUB_SLOT_SUB_BLOCKS
        )
        ...
        while curr.height < ses_block.height:
            if sub_blocks[curr.header_hash].is_challenge_sub_block(self.constants):
                ...
                seg, height = await self._handle_challenge_segment(
                    curr, sub_epoch_n, header_blocks, sub_blocks, sub_epoch_start
                )
                if seg is None:
                    self.log.error(f"failed creating segment {curr.header_hash} ")
                    return None
                segments.append(seg)
                sub_epoch_start = False
            else:
    ...
    async def _handle_challenge_segment(
        ...
        first_sub_slots = await self.__first_sub_slots_data(header_block, header_blocks, se_start)
    ...
    async def __first_sub_slots_data(
        self, header_block: HeaderBlock, header_blocks: Dict[bytes32, HeaderBlock], sub_epoch_start: bool
        ...
        if sub_epoch_start:
            curr_header, sub_slots = handle_sub_epoch_segment_start(curr_header, header_blocks)
        else:
        ...
        self.log.info(f"add challenge block height {header_block.height}")
        ssd = SubSlotData(
