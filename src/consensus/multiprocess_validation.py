...
def batch_pre_validate_sub_blocks(
    ...
    for i in range(len(full_blocks_pickled)):
        try:
            full_block: FullBlock = FullBlock.from_bytes(full_blocks_pickled[i])
            required_iters, error = validate_finished_header_block(
                constants,
                sub_blocks,
                full_block.get_block_header(),
                check_filter,
            ...
            if not error and full_block.transactions_generator is not None:
                cost_result = calculate_cost_of_program(
                    full_block.transactions_generator, constants.CLVM_COST_RATIO_CONSTANT
                )
    ...
    for i in range(0, len(blocks), batch_size):
        ...
        futures.append(
            asyncio.get_running_loop().run_in_executor(
                ...
                constants_json,
                final_pickled,
                [bytes(block) for block in blocks_to_validate],
                True,
