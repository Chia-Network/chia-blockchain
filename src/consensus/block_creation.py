...
def create_foliage(
    ...
    if prev_block is None:
        ...
        if prev_block.sub_block_height == 0:
            height: uint32 = uint32(1)
        else:
            height: uint32 = uint32(prev_block.prev_block_height + 2)
    ...
    if is_block:
        ...
        if sub_height > 0:
            ...
            if curr.sub_block_height == 0:
                ...
                prev_block_height = curr.prev_block_height + 1
            pool_coin = create_pool_coin(
                curr.sub_block_height,
                curr.pool_puzzle_hash,
                calculate_pool_reward(prev_block_height, curr.sub_block_height == 0),
            )
            farmer_coin = create_farmer_coin(
                curr.sub_block_height,
                curr.farmer_puzzle_hash,
                calculate_base_farmer_reward(prev_block_height) + curr.fees,
            )
            ...
            if curr.sub_block_height > 0:
                ...
                while not curr.is_block:
                    pool_coin = create_pool_coin(
                        curr.sub_block_height,
                        curr.pool_puzzle_hash,
                        calculate_pool_reward(curr.prev_block_height + 1, False),
                    )
                    farmer_coin = create_farmer_coin(
                        curr.sub_block_height,
                        curr.farmer_puzzle_hash,
                        calculate_base_farmer_reward(curr.prev_block_height + 1),
                    )
                    reward_claims_incorporated += [pool_coin, farmer_coin]
                    curr = sub_blocks[curr.prev_hash]
