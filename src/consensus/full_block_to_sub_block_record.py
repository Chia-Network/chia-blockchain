...
def block_to_sub_block_record(
    ...
    return header_block_to_sub_block_record(
        constants, required_iters, block, sub_slot_iters, overflow, deficit, uint32(height), ses
    )
...
def header_block_to_sub_block_record(
    constants: ConsensusConstants,
    required_iters: uint64,
    header_block: Union[FullBlock, HeaderBlock],
    ...
    if len(header_block.finished_sub_slots) > 0:
        finished_challenge_slot_hashes: Optional[List[bytes32]] = [
            sub_slot.challenge_chain.get_hash() for sub_slot in header_block.finished_sub_slots
        ]
        finished_reward_slot_hashes: Optional[List[bytes32]] = [
            sub_slot.reward_chain.get_hash() for sub_slot in header_block.finished_sub_slots
        ]
    ...
    elif header_block.sub_block_height == 0:
        ...
        finished_infused_challenge_slot_hashes = None
    prev_block_hash = header_block.foliage_block.prev_block_hash if header_block.foliage_block is not None else None
    timestamp = header_block.foliage_block.timestamp if header_block.foliage_block is not None else None
    fees = header_block.transactions_info.fees if header_block.transactions_info is not None else None
