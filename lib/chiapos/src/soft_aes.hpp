...
void soft_phex(char *pre, uint8_t* str, int len)
{
    printf("%s %d ",pre,len);
...
static void soft_KeyExpansion(uint8_t* RoundKey, const uint8_t* Key, int keyNr, int keyNk)
{
  unsigned i, j, k;
...
void soft_aes_load_key(uint8_t *enc_key, int keylen) {
    ...
    switch(keylen){
        case 32:
            soft_KeyExpansion(RoundKey256, enc_key, KEYNR256, KEYNK256);
            soft_phex((char *)"soft 256",RoundKey256,sizeof(RoundKey256));
            break;
        case 16:
            soft_KeyExpansion(RoundKey128, enc_key, KEYNR128, KEYNK128);
            soft_phex((char *)"soft 128",RoundKey128,sizeof(RoundKey128));
            break;
    }
}
...
* XOR 128 bits
*/
static inline void soft_xor128(const uint8_t *in1, const uint8_t *in2, uint8_t *out) {
    for(int i=0;i<16;i++) {
     out[i]=in1[i]^in2[i];
    }
}
