...
from chinilla.consensus.network_type import NetworkType
...
from chinilla.protocols.harvester_protocol import PoolDifficulty
...
from chinilla.util.api_decorators import api_request, peer_required
...
class FarmerAPI:
    ...
    async def new_proof_of_space(
        self, new_proof_of_space: harvester_protocol.NewProofOfSpace, peer: ws.WSChinillaConnection
    ):
        """
        ...
        if self.farmer.constants.NETWORK_TYPE != NetworkType.VANILLANET:
                ...
                try:
                    async with aiohttp.ClientSession() as session:
                        async with session.post(
                            ...
                            headers={"User-Agent": f"Chinilla Blockchain v.{__version__}"},
                        ) as resp:
                            if resp.ok:
    ...
    @api_request
    @peer_required
    async def respond_plots(self, _: harvester_protocol.RespondPlots, peer: ws.WSChinillaConnection):
