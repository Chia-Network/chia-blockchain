...
from chinilla.full_node.weight_proof import chunks
from chinilla.pools.pool_puzzles import SINGLETON_LAUNCHER_HASH, solution_to_pool_state
...
class WalletNode:
    ...
    server: Optional[ChinillaServer]
    log: logging.Logger
    ...
    async def get_key_for_fingerprint(self, fingerprint: Optional[int]) -> Optional[PrivateKey]:
        try:
            ...
            self.log.warning("No keys present. Create keys with the UI, or with the 'chinilla keys' program.")
            return None
    ...
    async def new_puzzle_hash_created(self, puzzle_hashes: List[bytes32]):
        if len(puzzle_hashes) == 0:
            return
        assert self.server is not None
        full_nodes: Dict[bytes32, WSChinillaConnection] = self.server.connection_by_type.get(NodeType.FULL_NODE, {})
    ...
    def on_disconnect(self, peer: WSChinillaConnection):
    ...
    async def trusted_sync(self, full_node: WSChinillaConnection):
        """
    ...
    async def subscribe_to_phs(self, puzzle_hashes: List[bytes32], peer: WSChinillaConnection, height=uint32(0)):
        """
    ...
    async def state_update_received(self, request: wallet_protocol.CoinStateUpdate, peer: WSChinillaConnection):
        assert self.wallet_state_manager is not None
        assert self.server is not None
    ...
    async def fetch_last_tx_from_peer(self, height: uint32, peer: WSChinillaConnection) -> Optional[HeaderBlock]:
    ...
    async def new_peak_wallet(self, peak: wallet_protocol.NewPeakWallet, peer: WSChinillaConnection):
        assert self.wallet_state_manager is not None
        assert self.server is not None
        async with self.new_peak_lock:
            if self.wallet_state_manager is None:
    ...
    async def complete_blocks(self, header_blocks: List[HeaderBlock], peer: WSChinillaConnection):
        if self.wallet_state_manager is None:
            return None
    ...
    async def get_additions(
        self, peer: WSChinillaConnection, block_i, additions: Optional[List[bytes32]], get_all_additions: bool = False
    ) -> Optional[List[Coin]]:
    ...
    async def get_removals(
        self, peer: WSChinillaConnection, block_i, additions, removals, request_all_removals=False
    ) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
    ...
    async def fetch_and_validate_the_weight_proof(
        self, peer: WSChinillaConnection, peak: HeaderBlock
    ...
    async def untrusted_sync_to_peer(
        self, peer: WSChinillaConnection, weight_proof: WeightProof, syncing: bool, fork_height: int
    ):
        assert self.wallet_state_manager is not None
