...
from chinilla.plotting.create_plots import resolve_plot_keys
from chinilla.plotters.plotters_util import run_plotter, run_command
...
def get_madmax_executable_path_for_ksize(plotters_root_path: Path, ksize: int = 32) -> Path:
    ...
    madmax_exec: str = "chinilla_plot"
    if ksize > 32:
        madmax_exec += "_k34"  # Use the chinilla_plot_k34 executable for k-sizes > 32
    ...
    if is_madmax_supported():
        ...
        run_command(
            [
                "git",
                "clone",
                "https://github.com/Chinilla/chinilla-plotter-madmax.git",
                MADMAX_PLOTTER_DIR,
            ],
...
def plot_madmax(args, chinilla_root_path: Path, plotters_root_path: Path):
