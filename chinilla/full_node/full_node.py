...
class FullNode:
    ...
    async def short_sync_batch(self, peer: ws.WSChinillaConnection, start_height: uint32, target_height: uint32) -> bool:
        """
    ...
    def remove_subscriptions(self, peer: ws.WSChinillaConnection):
    ...
    async def sync_from_fork_point(
        ...
        async def fetch_block_batches(batch_queue, peers_with_peak: List[ws.WSChinillaConnection]):
            try:
        ...
        batch_queue: asyncio.Queue[Tuple[ws.WSChinillaConnection, List[FullBlock]]] = asyncio.Queue(
    ...
    async def respond_unfinished_block(
        ...
        if farmed_block is True:
            ...
            self.log.info(
                ...
                f"Pool pk {encode_puzzle_hash(block.foliage.foliage_block_data.pool_target.puzzle_hash, 'xchi')}, "
                f"validation time: {validation_time:0.4f} seconds, {pre_validation_log}"
                f"cost: {block.transactions_info.cost if block.transactions_info else 'None'}"
                f"{percent_full_str}"
            )
