...
from chinilla.pools.pool_wallet_info import create_pool_state, FARMING_TO_POOL, PoolWalletInfo, PoolState
...
from chinilla.util.keychain import KeyringIsLocked, bytes_to_mnemonic, generate_mnemonic
...
from chinilla.wallet.derive_keys import master_sk_to_singleton_owner_sk, master_sk_to_wallet_sk_unhardened
...
class WalletRpcApi:
    ...
    async def _state_changed(self, *args) -> List[WsRpcMessage]:
        ...
        return [create_payload_dict("state_changed", data, "chinilla_wallet", "wallet_ui")]
    ...
    async def _check_key_used_for_rewards(
        ...
        farmer_target = config["farmer"].get("xchi_target_address")
        pool_target = config["pool"].get("xchi_target_address")
        found_farmer = False
        found_pool = False
