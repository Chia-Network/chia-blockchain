...
from chinilla.util.config import config_path_for_filename, load_config
from chinilla.util.keychain import Keychain
from chinilla.util.path import mkdir
from chinilla.wallet.derive_keys import master_sk_to_farmer_sk, master_sk_to_local_sk, master_sk_to_pool_sk
...
async def create_plots(
    ...
    if args.size < config["min_vanillanet_k_size"] and test_private_keys is None:
        log.warning(f"Creating plots with size k={args.size}, which is less than the minimum required for vanillanet")
    if args.size < 22:
