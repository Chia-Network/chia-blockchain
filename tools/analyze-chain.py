...
# run
def run_gen(env_data: bytes, block_program_args: bytes, flags: uint32) -> Tuple[Optional[int], List[Any], Optional[int]]:
    ...
    try:
        start_time = time()
        err, result = run_generator2(  # type: ignore[no-any-return]
            GENERATOR_ROM,
            env_data,
            max_cost,
            flags,
