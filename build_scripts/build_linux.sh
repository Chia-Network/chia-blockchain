...
if [ ! "$1" ]; then
  echo "This script requires either amd64 of arm64 as an argument"
	...
	DIR_NAME="deafwave-blockchain-linux-x64"
else
	PLATFORM="$1"
	DIR_NAME="deafwave-blockchain-linux-arm64"
fi
...
electron-packager . deafwave-blockchain --asar.unpack="**/daemon/**" --platform=linux \
...
echo "Create deafwave-$DEAFWAVE_INSTALLER_VERSION.deb"
rm -rf final_installer
mkdir final_installer
electron-installer-debian --src dist/$DIR_NAME/ --dest final_installer/ \
--arch "$PLATFORM" --options.version $DEAFWAVE_INSTALLER_VERSION
LAST_EXIT_CODE=$?
if [ "$LAST_EXIT_CODE" -ne 0 ]; then
...
if [ "$REDHAT_PLATFORM" = "x86_64" ]; then
	echo "Create deafwave-blockchain-$DEAFWAVE_INSTALLER_VERSION.rpm"
  electron-installer-redhat --src dist/$DIR_NAME/ --dest final_installer/ \
  --arch "$REDHAT_PLATFORM" --options.version $DEAFWAVE_INSTALLER_VERSION \
  --license ../LICENSE
  LAST_EXIT_CODE=$?
  if [ "$LAST_EXIT_CODE" -ne 0 ]; then
