...
from sector.protocols.shared_protocol import protocol_version
from sector.server.introducer_peers import IntroducerPeers
from sector.server.outbound_message import Message, NodeType
from sector.server.ssl_context import private_ssl_paths, public_ssl_paths
...
from sector.util.errors import Err, ProtocolError
from sector.util.ints import uint16
from sector.util.network import is_localhost, is_in_network
...
class SectorServer:
    def __init__(
        ...
        self.all_connections: Dict[bytes32, WSSectorConnection] = {}
        self.tasks: Set[asyncio.Task] = set()

        self.connection_by_type: Dict[NodeType, Dict[bytes32, WSSectorConnection]] = {
            NodeType.FULL_NODE: {},
            NodeType.WALLET: {},
            NodeType.HARVESTER: {},
            NodeType.FARMER: {},
    ...
    async def garbage_collect_connections_task(self) -> None:
        ...
        while True:
            ...
            to_remove: List[WSSectorConnection] = []
    ...
    async def incoming_connection(self, request):
        ...
        if peer_id == self.node_id:
            return ws
        connection: Optional[WSSectorConnection] = None
        try:
            connection = WSSectorConnection(
                self._local_type,
                ws,
                self._port,
                self.log,
    ...
    async def connection_added(self, connection: WSSectorConnection, on_connect=None):
    ...
    def connection_closed(self, connection: WSSectorConnection, ban_time: int):
    ...
    async def incoming_api_task(self) -> None:
        ...
        while True:
            ...
            async def api_call(full_message: Message, connection: WSSectorConnection, task_id):
    ...
    async def send_to_others(
        ...
        node_type: NodeType,
        origin_peer: WSSectorConnection,
    ):
    ...
    def get_outgoing_connections(self) -> List[WSSectorConnection]:
        result = []
        for _, connection in self.all_connections.items():
    ...
    def get_full_node_outgoing_connections(self) -> List[WSSectorConnection]:
        result = []
    ...
    def get_full_node_connections(self) -> List[WSSectorConnection]:
        return list(self.connection_by_type[NodeType.FULL_NODE].values())

    def get_connections(self) -> List[WSSectorConnection]:
        result = []
        for _, connection in self.all_connections.items():
            result.append(connection)
        return result
    ...
    def is_trusted_peer(self, peer: WSSectorConnection, trusted_peers: Dict) -> bool:
