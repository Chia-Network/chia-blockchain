...
from sector.cmds.init_funcs import sector_init
from sector.daemon.windows_signal import kill
from sector.server.server import ssl_context_for_root, ssl_context_for_server
from sector.ssl.create_ssl import get_mozilla_ca_crt
...
from sector.util.service_groups import validate_service
from sector.util.setproctitle import setproctitle
from sector.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Sector.")
    quit()
...
service_plotter = "sector plots create"
...
if getattr(sys, "frozen", False):
    name_map = {
        "sector": "sector",
        "sector_wallet": "start_wallet",
        "sector_full_node": "start_full_node",
        "sector_harvester": "start_harvester",
        "sector_farmer": "start_farmer",
        "sector_introducer": "start_introducer",
        "sector_timelord": "start_timelord",
        "sector_timelord_launcher": "timelord_launcher",
        "sector_full_node_simulator": "start_simulator",
    }
...
class WebSocketServer:
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("sector daemon exiting in 5 seconds")
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered SECTOR_ROOT
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with SECTOR_ROOT: {os.environ['SECTOR_ROOT']}")
...
async def async_run_daemon(root_path: Path) -> int:
    sector_init(root_path)
    config = load_config(root_path, "config.yaml")
    setproctitle("sector_daemon")
