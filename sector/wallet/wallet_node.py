...
from sector.pools.pool_puzzles import SINGLETON_LAUNCHER_HASH
from sector.protocols import wallet_protocol
from sector.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
from sector.protocols.protocol_message_types import ProtocolMessageTypes
from sector.protocols.wallet_protocol import (
...
from sector.server.node_discovery import WalletPeers
...
from sector.types.blockchain_format.coin import Coin, hash_coin_list
...
from sector.util.ints import uint32, uint128
from sector.util.keychain import Keychain
from sector.util.lru_cache import LRUCache
from sector.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
from sector.util.path import mkdir, path_from_root
from sector.wallet.block_record import HeaderBlockRecord
from sector.wallet.derivation_record import DerivationRecord
from sector.wallet.settings.settings_objects import BackupInitialized
from sector.wallet.transaction_record import TransactionRecord
from sector.wallet.util.backup_utils import open_backup_file
from sector.wallet.util.wallet_types import WalletType
from sector.wallet.wallet_action import WalletAction
from sector.wallet.wallet_blockchain import ReceiveBlockResult
from sector.wallet.wallet_state_manager import WalletStateManager
...
class WalletNode:
    ...
    def get_key_for_fingerprint(self, fingerprint: Optional[int]) -> Optional[PrivateKey]:
        ...
        if len(private_keys) == 0:
            self.log.warning("No keys present. Create keys with the UI, or with the 'sector keys' program.")
            return None
    ...
    async def on_connect(self, peer: WSSectorConnection):
        if self.wallet_state_manager is None or self.backup_initialized is False:
            return None
    ...
    async def complete_blocks(self, header_blocks: List[HeaderBlock], peer: WSSectorConnection):
    ...
    async def new_peak_wallet(self, peak: wallet_protocol.NewPeakWallet, peer: WSSectorConnection):
        if self.wallet_state_manager is None:
            return
    ...
    async def _sync(self) -> None:
        ...
        peers: List[WSSectorConnection] = self.server.get_full_node_connections()
        if len(peers) == 0:
    ...
    async def get_additions(
        self, peer: WSSectorConnection, block_i, additions: Optional[List[bytes32]], get_all_additions: bool = False
    ) -> Optional[List[Coin]]:
    ...
    async def get_removals(
        self, peer: WSSectorConnection, block_i, additions, removals, request_all_removals=False
    ) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
