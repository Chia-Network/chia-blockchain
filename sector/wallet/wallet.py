...
from sector.consensus.cost_calculator import calculate_cost_of_program, NPCResult
...
from sector.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import (
    DEFAULT_HIDDEN_PUZZLE_HASH,
    calculate_synthetic_secret_key,
    puzzle_for_pk,
    solution_for_conditions,
)
from sector.wallet.puzzles.puzzle_utils import (
...
from sector.wallet.secret_key_store import SecretKeyStore
from sector.wallet.sign_coin_solutions import sign_coin_solutions
...
class Wallet:
    ...
    async def create_spend_bundle_relative_sector(self, sector_amount: int, exclude: List[Coin]) -> SpendBundle:
        list_of_solutions = []
        utxos = None
        ...
        if sector_amount < 0:
            utxos = await self.select_coins(abs(sector_amount), exclude)
        else:
        ...
        sector_amount = spend_value + sector_amount
        ...
        for coin in utxos:
            ...
            if output_created is None:
                ...
                primaries = [{"puzzlehash": newpuzhash, "amount": sector_amount}]
