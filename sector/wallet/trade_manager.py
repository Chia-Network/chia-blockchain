...
from sector.wallet.cc_wallet.cc_utils import CC_MOD, SpendableCC, spend_bundle_for_spendable_ccs, uncurry_cc
from sector.wallet.cc_wallet.cc_wallet import CCWallet
from sector.wallet.puzzles.genesis_by_coin_id_with_0 import genesis_coin_id_for_genesis_coin_checker
from sector.wallet.trade_record import TradeRecord
from sector.wallet.trading.trade_status import TradeStatus
from sector.wallet.trading.trade_store import TradeStore
from sector.wallet.transaction_record import TransactionRecord
from sector.wallet.util.trade_utils import (
            ...
            for id in offer.keys():
                ...
                if isinstance(wallet, CCWallet):
                    ...
                    if spend_bundle is None:
                        ...
                        to_exclude = spend_bundle.removals()
                    new_spend_bundle = await wallet.create_spend_bundle_relative_sector(amount, to_exclude)
                else:
    ...
    async def respond_to_offer(self, file_path: Path) -> Tuple[bool, Optional[TradeRecord], Optional[str]]:
        ...
        for coinsol in offer_spend_bundle.coin_solutions:
            ...
            if r:
                ...
                # standard sector coin
                unspent = await self.wallet_state_manager.get_spendable_coins_for_wallet(1)
                if coinsol.coin in [record.coin for record in unspent]:
                    return False, None, "can't respond to own offer"
                if sector_discrepancy is None:
                    sector_discrepancy = get_output_discrepancy_for_puzzle_and_solution(coinsol.coin, puzzle, solution)
                else:
                    sector_discrepancy += get_output_discrepancy_for_puzzle_and_solution(coinsol.coin, puzzle, solution)
                coinsols.append(coinsol)

        sector_spend_bundle: Optional[SpendBundle] = None
        if sector_discrepancy is not None:
            sector_spend_bundle = await self.wallet_state_manager.main_wallet.create_spend_bundle_relative_sector(
                sector_discrepancy, []
            )
            if sector_spend_bundle is not None:
                for coinsol in coinsols:
                    sector_spend_bundle.coin_solutions.append(coinsol)
        ...
        for colour in cc_discrepancies.keys():
            if cc_discrepancies[colour] < 0:
                ...
                if sector_spend_bundle is None:
                    to_exclude: List = []
                else:
                    to_exclude = sector_spend_bundle.removals()
                ...
                if my_cc_spends is None or my_cc_spends == set():
                    ...
                    if zero_spend_bundle is None:
                        return (
                            False,
                            None,
                            "Unable to generate zero value coin. Confirm that you have sector available",
                        )
        ...
        if sector_spend_bundle is not None:
            ...
            if sector_discrepancy < 0:
                tx_record = TransactionRecord(
                    ...
                    amount=uint64(abs(sector_discrepancy)),
                    fee_amount=uint64(0),
                    confirmed=False,
                    sent=uint32(10),
                    spend_bundle=sector_spend_bundle,
                    additions=sector_spend_bundle.additions(),
                    removals=sector_spend_bundle.removals(),
                    wallet_id=uint32(1),
                    sent_to=[],
                    trade_id=std_hash(spend_bundle.name() + bytes(now)),
                    type=uint32(TransactionType.OUTGOING_TRADE.value),
                    name=sector_spend_bundle.name(),
                )
            else:
                tx_record = TransactionRecord(
                    confirmed_at_height=uint32(0),
