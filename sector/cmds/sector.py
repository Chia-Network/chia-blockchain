...
from sector.cmds.configure import configure_cmd
from sector.cmds.farm import farm_cmd
from sector.cmds.init import init_cmd
from sector.cmds.keys import keys_cmd
from sector.cmds.netspace import netspace_cmd
from sector.cmds.plots import plots_cmd
from sector.cmds.show import show_cmd
from sector.cmds.start import start_cmd
from sector.cmds.stop import stop_cmd
from sector.cmds.wallet import wallet_cmd
from sector.cmds.plotnft import plotnft_cmd
...
@click.group(
    help=f"\n  Manage sector blockchain infrastructure ({__version__})\n",
    epilog="Try 'sector start node', 'sector netspace -d 192', or 'sector show -s'",
...
@cli.command("version", short_help="Show sector version")
def version_cmd() -> None:
    print(__version__)


@cli.command("run_daemon", short_help="Runs sector daemon")
@click.pass_context
def run_daemon_cmd(ctx: click.Context) -> None:
    from sector.daemon.server import async_run_daemon
    import asyncio
