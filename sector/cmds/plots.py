...
def show_plots(root_path: Path):
    from sector.plotting.plot_tools import get_plot_directories

    print("Directories where plots are being searched for:")
    print("Note that subdirectories must be added manually")
    print(
        "Add with 'sector plots add -d [dir]' and remove with"
        + " 'sector plots remove -d [dir]'"
        + " Scan and check plots with 'sector plots check'"
    )
    print()
...
def plots_cmd(ctx: click.Context):
    ...
    from sector.util.sector_logging import initialize_logging

    root_path: Path = ctx.obj["root_path"]
    if not root_path.is_dir():
        raise RuntimeError("Please initialize (or migrate) your config directory with 'sector init'")
...
def create_cmd(
    ...
    from sector.plotting.create_plots import create_plots
...
def check_cmd(
    ...
    from sector.plotting.check_plots import check_plots
...
def add_cmd(ctx: click.Context, final_dir: str):
    from sector.plotting.plot_tools import add_plot_directory
...
def remove_cmd(ctx: click.Context, final_dir: str):
    from sector.plotting.plot_tools import remove_plot_directory
