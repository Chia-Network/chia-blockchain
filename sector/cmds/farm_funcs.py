...
from sector.cmds.units import units
...
from sector.util.ints import uint16
from sector.util.misc import format_bytes
from sector.util.misc import format_minutes
...
async def summary(rpc_port: int, wallet_rpc_port: int, harvester_rpc_port: int, farmer_rpc_port: int) -> None:
    ...
    if amounts is not None:
        print(f"Total sector farmed: {amounts['farmed_amount'] / units['sector']}")
        print(f"User transaction fees: {amounts['fee_amount'] / units['sector']}")
        print(f"Block rewards: {(amounts['farmer_reward_amount'] + amounts['pool_reward_amount']) / units['sector']}")
    ...
    if amounts is None:
        if wallet_not_running:
            print("For details on farmed rewards and fees you should run 'sector start wallet' and 'sector wallet show'")
        elif wallet_not_ready:
            print("For details on farmed rewards and fees you should run 'sector wallet show'")
    else:
        print("Note: log into your key using 'sector wallet show' to see rewards for each key")