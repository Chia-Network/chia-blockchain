...
@wallet_cmd.command("get_address", short_help="Get the current public wallet address")
@click.option(
    "-wp",
    "--wallet-rpc-port",
    help="Set the port where the Wallet is hosting the RPC interface. See the rpc_port under wallet in config.yaml",
...
@click.option( "-n", "--new_address", help="Generate new public wallet address.", type=bool, default=False, is_flag=True, show_default=True)
@click.option("-i", "--id", help="Id of the wallet to use", type=int, default=1, show_default=True, required=True)
@click.option("-f", "--fingerprint", help="Set the fingerprint to specify which wallet to use", type=int)

def get_address_cmd(wallet_rpc_port: Optional[int], id, new_address, fingerprint: int) -> None:
    extra_params = {"id": id, "get_new_address": new_address}
    import asyncio
