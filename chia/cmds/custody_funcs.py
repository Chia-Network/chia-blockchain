...
async def create_custody_store_cmd(rpc_port: Optional[int], fee: Optional[str]) -> None:
    ...
    async with get_any_service_client("custody_layer", rpc_port) as (client, config, _):
        if client is not None:
            res = await client.create_custody_store(fee=final_fee)
            print(res)
...
async def update_custody_store_cmd(
    ...
    async with get_any_service_client("custody_layer", rpc_port) as (client, config, _):
        if client is not None:
            res = await client.update_custody_store(store_id=store_id_bytes, changelist=changelist, fee=final_fee)
            print(res)
