...
def print_nft_info(nft: NFTInfo, *, config: Dict[str, Any], sequence_info: Optional[Tuple[int, int]] = None) -> None:
    indent: str = "   "
...
async def list_nfts(args: Dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    wallet_id = args["wallet_id"]
    start: Optional[int] = args.get("start")
    num: Optional[int] = args.get("num")
    try:
        config = load_config(DEFAULT_ROOT_PATH, "config.yaml", SERVICE_NAME)
        count_response: Dict[str, Any] = await wallet_client.count_nfts(wallet_id)
        count: int = count_response["count"]
        response = await wallet_client.list_nfts(wallet_id, pagination=(start, num))
        nft_list = response["nft_list"]
        if len(nft_list) > 0:
            for i, n in enumerate(nft_list):
...
async def count_nfts(args: Dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    wallet_id = args["wallet_id"]
    try:
        response = await wallet_client.count_nfts(wallet_id)
        count = response["count"]
        print(f"Number of NFTs for wallet id {wallet_id}: {count}")
    except Exception as e:
        print(f"Failed to count NFTs for wallet with id {wallet_id} on key {fingerprint}: {e}")
