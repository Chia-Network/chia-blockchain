...
async def print_fee_info(node_client: FullNodeRpcClient) -> None:
    ...
    if not res["full_node_synced"]:
        print("Full node syncing. Use 'chia show -s' to monitor sync status.")
        return
    print(res)
    print("\n")
    ...
    mempool_fee_rate = 0.0 if res["mempool_size"] == 0 else res["mempool_fees"] / res["mempool_size"]
    print(f"  Mempool fee rate: {mempool_fee_rate:>12.6} mojos per CLVM cost")
    print()
    ...
    print(f"    Max Block cost: {res['max_block_clvm_cost']:>12} CLVM cost")
    print(f"        Block cost: {res['last_block_cost']:>12} CLVM cost")
    print(f"    Block fee rate: {res['fee_rate_last_block']:>12.6} mojos per CLVM cost")
