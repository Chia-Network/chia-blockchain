...
def db_upgrade_func(
   ...
   copy_v1_to_v2(in_db_path, out_db_path, out_path_string)
...
def connect_to_db(open_in_path: Path, attach_out_path: str):
   with sqlite3.connect(open_in_path) as db:
       try:
           with db.execute("SELECT * from database_version") as cursor:
               row = cursor.fetchone()
...
def copy_v1_to_v2(in_path: Path, out_path: Path, out_path_string: str) -> None:
...
#               commit_start = time()
               in_db.executemany(
                   "INSERT INTO v2db.full_blocks VALUES(?, ?, ?, ?, ?, ?, ?, ?)", block_values
               )
               in_db.commit()
#               commit_end = time()
...
#               print(f"\rTime to shrink: {shrink_end - shrink_start:.2f} seconds")
   ...
   in_db.execute(
       "CREATE INDEX IF NOT EXISTS v2db.height on full_blocks(height)"
   )
...
#   shrink_in = COIN_SHRINK_RATE
   ...
   while True:
       ...
       for row in rows:
           ...
           # Not sure if this was necessary, as the fetching query already had a condition where confirmed_index <= peak
...
#   shrink_in = SES_SHRINK_RATE
   ...
   in_db = connect_to_db(temp2_out_path, out_path_string)
   ...
   print(f"\tCreate Index on puzzle_hash: {coin_puzzle_end - coin_store_start:.2f} seconds                             ")

   # 2. INDEX
   in_db.execute("CREATE INDEX IF NOT EXISTS v2db.coin_parent_index on coin_record(coin_parent)")
   coin_parent_end = time()
   print(f"\tCreate Index on coin_parent: {coin_parent_end - coin_puzzle_end:.2f} seconds                             ")

   # 3. INDEX
   in_db.execute("CREATE INDEX IF NOT EXISTS v2db.coin_confirmed_index on coin_record(confirmed_index)")
   coin_conf_end = time()
   print(f"\tCreate Index on confirmed index: {coin_conf_end - coin_parent_end:.2f} seconds                             ")
...
#   print(f"\tCreate Unique Index on coin_name: {coin_name_end - coin_spent_end:.2f} seconds                             ")
   ...
   in_db = connect_to_db(temp3_out_path, out_path_string)
