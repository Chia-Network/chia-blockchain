...
def load_plot(
    ...
    if filename.is_dir():
        return None, f"Path: {filename} is a directory not a plot file."
    ...
    try:
        ...
        if prover.get_size() >= 30 and stat_info.st_size < 0.98 * expected_size:
            return None, f"Not a valid plot {filename}. Size is {stat_info.st_size / (1024**3)} GiB, but expected,\nat least: {expected_size / (1024 ** 3)} GiB. We assume the file is being copied."

        pool_public_key_or_puzzle_hash, farmer_public_key, _ = parse_plot_info(prover.get_memo())
        stat_info = filename.stat()
        return   PlotInfo(
                prover,
                pool_public_key_or_puzzle_hash,
        ...
        tb = traceback.format_exc()
        return None, f"Failed to open file {filename}. {e} {tb}."
