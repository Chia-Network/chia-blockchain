...
from tad.plotting.plot_tools import add_plot_directory, stream_plot_info_ph, stream_plot_info_pk
...
from tad.util.config import config_path_for_filename, load_config
...
def get_farmer_public_key(alt_fingerprint: Optional[int] = None) -> G1Element:
    ...
    if sk_ent is None:
        raise RuntimeError("No keys, please run 'tad keys add', 'tad keys generate' or provide a public key with -f")
...
def get_pool_public_key(alt_fingerprint: Optional[int] = None) -> G1Element:
    ...
    if alt_fingerprint is not None:
        ...
        sk_ent = keychain.get_first_private_key()
    if sk_ent is None:
        raise RuntimeError("No keys, please run 'tad keys add', 'tad keys generate' or provide a public key with -p")
