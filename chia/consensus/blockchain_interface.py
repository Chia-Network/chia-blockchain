...
class MMRManagerProtocol(Protocol):
    """Protocol for MMR managers used in blockchain interfaces."""
    ...
    # Canonical-update transaction support (MMR persistence). A canonical
    # append/reorg is begun inside the block-store write transaction, the new
    # flat nodes and singleton state are persisted, and the update is committed
    # (or rolled back) once the transaction commits (or fails).
