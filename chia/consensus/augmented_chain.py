...
from chia.consensus.mmr_manager import _ReadOnlyMMRManager
...
class AugmentedBlockchain:
    ...
    def _min_overlay_entry(self) -> tuple[uint32, bytes32] | None:
        """
        Return the minimum overlay height/hash pair.
        """
        if not self._height_to_hash:
            return None

        min_overlay_height = min(self._height_to_hash)
        return min_overlay_height, self._height_to_hash[min_overlay_height]
