...
class CustodyServer:
    ...
    async def start(self) -> None:
        if self.webserver is not None:
            raise RuntimeError("CustodyServer already started")
        ...
        # Setup UPnP for the custody_service port
        self.upnp.setup()
...
async def async_start(root_path: Path) -> int:
    ...
    setproctitle("custody_http")
    initialize_logging(
        service_name="custody_http",
        logging_config=dl_config["logging"],
        root_path=root_path,
    )

    custody_server = CustodyServer(root_path, dl_config, log, shutdown_event)
    await custody_server.start()
    await shutdown_event.wait()
    await custody_server.await_closed()
