...
class PoolWallet:
    ...
    def get_coin_ids_we_are_interested_in(self) -> List[bytes32]:
        # Return all coin_ids that we might care about (singleton coin ids that we know of in all forks)
        pass

    @staticmethod
    def get_next_interesting_coin_ids(spend: CoinSolution) -> List[bytes32]:
        # CoinSolution of one of the coins that we cared about, or a spend of a coin into a Launcher coin
        # This coin was spent in a block, but might be in a reorg
        # If we return a value, it is a coin ID that we are also interested in (to support two transitions per block)
        pass

    def apply_state_transitions(self, block_spends: List[CoinSolution], block_height: uint32):
        # CoinSolution of one of the coins that we cared about as included into the blockchain
        pass
