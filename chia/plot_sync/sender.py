...
from chia.protocols.plot_sync_protocol import Done, Identifier, PathList, PlotList, Response, Start
...
class MessageGenerator:
    ...
    def generate(self) -> Tuple[Identifier, Any]:
        identifier = Identifier(uint64(int(time.time())), self.sync_id, self.message_id)
...
class ExpectedResponse:
    ...
    message: Optional[Response] = None
...
class Sender:
    ...
    def set_response(self, response: Response) -> bool:
    ...
    def sync_start(self, count: float, initial: bool) -> None:
        ...
        self._add_message(ProtocolMessageTypes.plot_sync_start, Start, initial, self._last_sync_id, uint32(int(count)))
    ...
    def process_batch(self, loaded: List[PlotInfo], remaining: int) -> None:
        ...
        if len(loaded) > 0 or remaining == 0:
            ...
            self._add_message(ProtocolMessageTypes.plot_sync_loaded, PlotList, converted, remaining == 0)
    ...
    def sync_done(self, removed: List[Path], duration: float) -> None:
        ...
        self._add_list_batched(ProtocolMessageTypes.plot_sync_invalid, PathList, failed_to_open_list)
        no_key_list = [str(x) for x in self._plot_manager.no_key_filenames]
        self._add_list_batched(ProtocolMessageTypes.plot_sync_keys_missing, PathList, no_key_list)
        self._add_message(ProtocolMessageTypes.plot_sync_done, Done, uint64(int(duration)))
