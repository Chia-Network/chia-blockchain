    ...
    async with DBConnection(2) as db_wrapper:
        ...
        async with contextlib.AsyncExitStack() as exit_stack:
            ...
            for index in range(concurrent_task_count):
                task = pit.create_task(increment_counter(db_wrapper))
                tasks.append(task)
...
async def test_concurrent_readers(acquire_outside: bool, get_reader_method: GetReaderMethod) -> None:
    async with DBConnection(2) as db_wrapper:
        ...
        async with contextlib.AsyncExitStack() as exit_stack:
            ...
            values: list[int] = []
            for index in range(concurrent_task_count):
                task = pit.create_task(sum_counter(db_wrapper, values))
                tasks.append(task)
    ...
    async with PathDBConnection(2) as db_wrapper:
        ...
        async with contextlib.AsyncExitStack() as exit_stack:
            ...
            for index in range(concurrent_task_count):
                task = pit.create_task(increment_counter(db_wrapper))
                tasks.append(task)
                task = pit.create_task(decrement_counter(db_wrapper))
                tasks.append(task)
                task = pit.create_task(sum_counter(db_wrapper, values))
                tasks.append(task)
