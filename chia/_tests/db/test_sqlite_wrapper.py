...
async def PathDBConnection(db_version: int) -> AsyncIterator[DBWrapper2]:
    with tempfile.TemporaryDirectory() as directory:
        ...
        async with sqlite_wrapper.managed(database=db_path, reader_count=4, db_version=db_version) as _db_wrapper:
            yield _db_wrapper


if TYPE_CHECKING:
    ConnectionContextManager = contextlib.AbstractAsyncContextManager[SqliteConnection]
...
async def query_value(connection: SqliteConnection) -> int:
    async with connection.execute("SELECT value FROM counter") as cursor:
