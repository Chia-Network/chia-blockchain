...
@pytest.mark.anyio
async def test_fork_info_potential_ff_singletons(
    ...
    eligible for fast forward, the puzzle hash and amount match. We're covering
    here `include_spends`, `rollback` and `reset` in the context of potential
    ...
    assert all(
        a in fork_info.additions_since_fork and fork_info.additions_since_fork[a].maybe_ff_singleton
    ...
    assert all(not fork_info.additions_since_fork[a].maybe_ff_singleton for a in non_ff_additions)
    if rollback:
