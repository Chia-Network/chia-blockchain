...
@pytest.mark.anyio
async def test_add_coin_records() -> None:
    """
    ...
    async with DBConnection(2) as db_wrapper:
        ...
        maybe_ff_singleton = Coin(bytes32([0] * 32), bytes32([0] * 32), uint64(1337))
        maybe_ff_singleton_id = maybe_ff_singleton.name()
        await coin_store.new_block(
            height=uint32(0),
            timestamp=uint64(1),
            ...
            tx_additions=[
                ...
                (maybe_ff_singleton_id, maybe_ff_singleton, True),
            ],
            tx_removals=[],
        )
        async with db_wrapper.reader_no_transaction() as conn:
            ...
            assert await get_spent_index(conn, maybe_ff_singleton_id) == -1
...
async def test_rollback_to_block_spent_index_update() -> None:
    ...
    async with DBConnection(2) as db_wrapper:
        ...
        potential_ff_child = Coin(parent_coin_id, parent_coin.puzzle_hash, parent_coin.amount)
        reward_coin = Coin(bytes32([0] * 32), bytes32([0] * 32), uint64(1))
        await insert_coins(
            db_wrapper,
            # List of (coin, spent_index, coinbase) values
            [(parent_coin, 2, False), (normal_child, 3, False), (potential_ff_child, 3, False), (reward_coin, 3, True)],
        )
        # Let's roll back
        ...
        async with db_wrapper.reader_no_transaction() as conn:
            ...
            assert await get_spent_index(conn, potential_ff_child.name()) == -1
