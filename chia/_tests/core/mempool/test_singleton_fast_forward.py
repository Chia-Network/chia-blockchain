...
async def test_process_fast_forward_spends_nothing_to_do() -> None:
    ...
    resulting_mempool_item = await eligible_coin_spends.process_fast_forward_spends(
    ...
    assert resulting_mempool_item == original_version
...
async def test_mempool_items_immutability_on_ff() -> None:
    ...
    async with sim_and_client() as (sim, sim_client):
        ...
        assert status == MempoolInclusionStatus.SUCCESS
        assert error is None
        original_item = sim_client.service.mempool_manager.get_mempool_item(sb_name)
        ...
        # We should have processed our item without modifying it
        item = sim_client.service.mempool_manager.get_mempool_item(sb_name)
