...
async def valid_node_values_fixture(
    ...
    if node_type == NodeType.INTERNAL:
        ...
        node_b = await data_store.get_node_by_key(key=b"\x04", root=root)
        return create_valid_node_values(root=root, node_type=node_type, left_hash=node_a.hash, right_hash=node_b.hash)
    elif node_type == NodeType.TERMINAL:
        return create_valid_node_values(root=root, node_type=node_type)
    else:
