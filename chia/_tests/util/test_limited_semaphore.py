...
async def test_stuff() -> None:
    ...
    entered_tasks = [pit.create_task(acquire(entered_event=event)) for event in entered_events]
    waiting_tasks = [pit.create_task(acquire(entered_event=event)) for event in waiting_events]
    ...
    failure_tasks = [pit.create_task(acquire()) for _ in range(beyond_limit)]
