...
class TestPriorityMutex:
    ...
    async def test_priority_mutex(self) -> None:
        ...
        h = pit.create_task(do_high())
        l_tasks = []
        for i in range(50):
            l_tasks.append(pit.create_task(do_low(i)))
...
async def test_cancellation_while_waiting() -> None:
    ...
    block_task = pit.create_task(block())
    await blocker_acquired_event.wait()

    cancel_task = pit.create_task(to_be_cancelled(mutex=mutex))
    await wait_queued(mutex=mutex, task=cancel_task)

    queued_after_task = pit.create_task(queued_after())
...
async def create_acquire_tasks_in_controlled_order(
    ...
    for request in requests:
        task = pit.create_task(request.acquire(mutex=mutex, wait_for=release_event))
        tasks.append(task)
...
async def test_multiple_tasks_track_active_task_accurately() -> None:
    ...
    async with mutex.acquire(priority=MutexPriority.high):
        other_task = pit.create_task(other_task_function())
    ...
    another_task = pit.create_task(another_task_function())
