...
class TestPoolPuzzles(TestCase):
    def test_pool_lifecycle(self):
        ...
        p2_singleton_puz: Program = create_p2_singleton_puzzle(SINGLETON_MOD_HASH, launcher_id, DELAY_TIME, DELAY_PH)
        p2_singleton_ph: bytes32 = p2_singleton_puz.get_tree_hash()
        assert p2_singleton_ph == create_p2_singleton_puzzle_hash(SINGLETON_MOD_HASH, launcher_id, DELAY_TIME, DELAY_PH)
        ...
        last_coinsol: CoinSpend = next(filter(lambda e: e.coin.amount == START_AMOUNT, coin_sols))
        coin_sols: list[CoinSpend] = create_absorb_spend(
            last_coinsol,
            pool_state,
            launcher_coin,
        ...
        singleton_coinsol: CoinSpend = next(filter(lambda e: e.coin.amount != START_AMOUNT, coin_sols))
        ...
        travel_coinsol, _ = create_travel_spend(
            last_coinsol, launcher_coin, pool_state, target_pool_state, GENESIS_CHALLENGE, DELAY_TIME, DELAY_PH
        )
        # Test that we can retrieve the extra data
        ...
        sig: G2Element = AugSchemeMPL.sign(sk, (data + singleton.name() + DEFAULT_CONSTANTS.AGG_SIG_ME_ADDITIONAL_DATA))
        # Spend it!
        coin_db.update_coin_store_for_spend_bundle(
        ...
        return_coinsol, _ = create_travel_spend(
            travel_coinsol, launcher_coin, target_pool_state, pool_state, GENESIS_CHALLENGE, DELAY_TIME, DELAY_PH
        )
        ...
        # get the relevant coin solution
        return_coinsol, _ = create_travel_spend(
            singleton_coinsol, launcher_coin, target_pool_state, pool_state, GENESIS_CHALLENGE, DELAY_TIME, DELAY_PH
        )
        # Test that we can retrieve the extra data
