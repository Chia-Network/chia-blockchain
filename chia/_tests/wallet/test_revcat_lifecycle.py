...
async def test_revocable_cat_lifecycle(cost_logger):
    async with sim_and_client() as (sim, sim_client):
        ...
        conditions = [Program.to([52, cat_ph, starting_coin.amount])]
        delegated_puzzle: Program = p2_conditions.puzzle_for_conditions(conditions)
        inner_solution: Program = (
            p2_delegated_puzzle_or_hidden_puzzle.solution_for_conditions(conditions)
        )
        ...
        await spend_cat(
            ...
            cost_logger=cost_logger,
            cost_log_msg="Cat Eve Spend - create one child (TAIL: everything_with_singleton)",
        )
        ...
        signature: G2Element = sign_delegated_puz(
            delegated_puzzle, singleton_latest_coin
        )
        singleton_spend_bundle = WalletSpendBundle(
            [singleton_revoke_coinsol], signature
        )
        ...
        cat_melt_signature = sign_delegated_puz(delegated_puzzle, cat_coin_to_melt)
        ...
        singleton_spend_bundle = WalletSpendBundle([singleton_melt_coinsol], signature)
