...
class TestWalletSimulator:
    ...
    @pytest.mark.limit_consensus_modes(reason="irrelevant")
    @pytest.mark.anyio
    async def test_wallet_clawback_claim_auto(self, wallet_environments: WalletTestFramework) -> None:
        env = wallet_environments.environments[0]
    ...
    async def test_address_sliding_window(self, wallet_environments: WalletTestFramework) -> None:
        ...
        for i in range(211):
            pubkey = master_sk_to_wallet_sk(
                wallet.wallet_state_manager.get_master_private_key(), uint32(i)
            ).public_key()
