...
class WalletNode:
    ...
    def is_trusted(self, peer):
        ret = self.server.is_trusted_peer(peer, self.config["trusted_peers"])
        self.log.warning(f"Is trusted: {ret}")
        return ret
        ...
        async with self.new_peak_lock:
            if self.is_trusted(peer):
                ...
                if (
                    ...
                    and peak_block.prev_header_hash == self.wallet_state_manager.blockchain.get_peak_block().header_hash
                ):
                    pass
                else:
