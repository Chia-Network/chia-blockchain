...
class WalletNode:
    ...
    async def validate_block_inclusion(
        ...
        if await self.wallet_state_manager.blockchain.contains_height(block.height):
            stored_hash = await self.wallet_state_manager.blockchain.height_to_hash(block.height)
