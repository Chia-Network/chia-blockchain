...
class WalletNode:
    ...
    async def get_timestamp_for_height(self, height: uint32) -> uint64:
        ...
        If we are running in "light wallet" mode, we may not yet have this information.
        """
        ...
        if all([peak[0] < height for peak in self.node_peaks.values()]):
            raise PeerRequestException("Error fetching timestamp for height %s. No peers are that high.", height)
        ...
        raise PeerRequestException("Error fetching timestamp for height %s. Peer peaks: %s", height, self.node_peaks)
