...
class WalletSingletonStore:
    ...
    async def save_singleton(self, record: SingletonRecord) -> None:
        ...
        if singleton_id is None:  # pragma: no cover
            raise RuntimeError(
                "Failed to derive Singleton ID from puzzle reveal in parent spend %s",
                record.parent_coinspend,
            )
        pending_int = 0
        if record.pending:
            pending_int = 1
    ...
    async def add_spend(
        ...
        if len(lineage_bytes) == 2:
            ...
            lineage_proof = LineageProof(
                bytes32(lineage_bytes[0]),
                bytes32(lineage_bytes[1]),
                uint64(int_from_bytes(lineage_bytes[2])),
            )
    ...
    async def delete_singleton_by_coin_id(self, coin_id: bytes32, height: uint32) -> bool:
        ...
        async with self.db_wrapper.writer_maybe_transaction() as conn:
            cursor = await conn.execute(
                "UPDATE singletons SET removed_height=? WHERE coin_id=?",
