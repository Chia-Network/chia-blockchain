...
from chia.wallet.db_wallet.db_wallet_puzzles import create_graftroot_offer_puz, GRAFTROOT_DL_OFFERS, ACS_MU_PH
...
class RequestedPayment:
    ...
    def to_delegated_puzzle_and_solution(
        ...
        for fixed_typ, solved_typ in zip(self.asset_types, solver_types):
            if fixed_typ["type"] != solved_typ["type"]:
    ...
    def from_delegated_puzzle_and_solution(
        cls, mod: Program, curried_args: Program, delegated_solution: Program
    ) -> Tuple["RequestedPayment", Solver, Tuple[Program, Program]]:
        asset_types: List[Solver] = []
        ...
        for wrapper in wrappers.as_iter():
            ...
            type_args = {
                prop: disassemble(arg)
                for prop, arg in zip(props, committed_args_list.as_iter())
                if arg != Program.to("missing")
            }
            asset_type.update(type_args)
            asset_types.append(Solver(asset_type))

        nonce: Optional[bytes32] = (
            Program.to(None) if payments.first() == Program.to(None) else bytes32(payments.first().as_python())
        )
...
class DLDataInclusion:
                        ...
                        for value in values:
                            for proof_of_inclusion in dep_info["proofs_of_inclusion"]:
                                ...
                                if innerpuz is not None and new_innerpuz != innerpuz:
                                    raise ValueError(
                                        f"Received two conflicting new innerpuzs for launcher_id {launcher}"
                                    )
    ...
    def from_delegated_puzzle_and_solution(
        ...
        for launcher_id, launcher_proofs, root, innerpuz in zip(
            self.launcher_ids, proofs.as_iter(), roots.as_iter(), innerpuzs.as_iter()
        ):
            dl_dependencies_info.append(
                {
                    ...
                    "proofs_of_inclusion": [
                        [
                            "0x" + root.hex(),
                            str(int_from_bytes(proof[0].as_python())),
                            *["0x" + sib.hex() for sib in proof[1:]],
                        ]
                        for proof in launcher_proofs
                    ],
