...
class WalletTransactionStore:
    ...
    async def create(cls, db_wrapper: DBWrapper):
        ...
        # this index is not used by any queries, don't create it for new
        # installs, and remove it from existing installs in the future
        # await self.db.execute("DROP INDEX IF EXISTS tx_created_time on transaction_record(created_at_time)")

        await self.db_connection.execute("CREATE INDEX IF NOT EXISTS tx_type on transaction_record(type)")

        # this index is not used by any queries, don't create it for new
        # installs, and remove it from existing installs in the future
        # await self.db.execute("DROP INDEX IF EXISTS tx_to_puzzle_hash on transaction_record(to_puzzle_hash)")

        await self.db_connection.execute("CREATE INDEX IF NOT EXISTS tx_wallet_id on transaction_record(wallet_id)")
