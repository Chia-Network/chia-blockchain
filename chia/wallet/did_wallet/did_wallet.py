...
class DIDWallet:
    ...
    async def create_spend(self, puzhash: bytes32):
        ...
        # sign for AGG_SIG_ME
        message = puzhash + coin.name() + self.wallet_state_manager.constants.GENESIS_CHALLENGE
    ...
    async def create_attestment(
        ...
        # sign for AGG_SIG_ME
        message = innermessage + coin.name() + self.wallet_state_manager.constants.GENESIS_CHALLENGE
        pubkey = did_wallet_puzzles.get_pubkey_from_innerpuz(innerpuz)
        index = await self.wallet_state_manager.puzzle_store.index_for_pubkey(pubkey)
        private = master_sk_to_wallet_sk(self.wallet_state_manager.private_key, index)
        signature = AugSchemeMPL.sign(private, message)
    ...
    async def generate_eve_spend(self, coin: Coin, full_puzzle: Program, origin_id: bytes, innerpuz: Program):
        ...
        list_of_solutions = [CoinSolution(coin, full_puzzle, fullsol)]
        # sign for AGG_SIG_ME
        message = coin.puzzle_hash + coin.name() + self.wallet_state_manager.constants.GENESIS_CHALLENGE
        pubkey = did_wallet_puzzles.get_pubkey_from_innerpuz(innerpuz)
        index = await self.wallet_state_manager.puzzle_store.index_for_pubkey(pubkey)
        private = master_sk_to_wallet_sk(self.wallet_state_manager.private_key, index)
        signature = AugSchemeMPL.sign(private, message)
