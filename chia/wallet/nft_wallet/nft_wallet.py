...
class NFTWallet:
    ...
    async def get_did_approval_info(
        ...
        tx_config: TXConfig,
        did_id: Optional[bytes32] = None,
    ) -> Tuple[bytes32, WalletSpendBundle]:
    ...
    async def generate_new_nft(
        ...
        txs.append(dataclasses.replace(tx_record, spend_bundle=WalletSpendBundle.aggregate(bundles_to_agg)))
        return txs
    ...
    async def generate_unsigned_spendbundle(
        ...
        extra_conditions: Tuple[Condition, ...] = tuple(),
    ) -> Tuple[WalletSpendBundle, Optional[TransactionRecord]]:
        if nft_coin is None:
    ...
    async def mint_from_did(
        ...
        launcher_spends = []
        launcher_ids = []
        eve_spends: List[WalletSpendBundle] = []
        ...
        total_spend = WalletSpendBundle.aggregate([unsigned_spend_bundle, xch_spend, *eve_spends])
