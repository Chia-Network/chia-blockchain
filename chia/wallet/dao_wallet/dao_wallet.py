...
class DAOWallet:
    ...
    async def create_new_dao_and_wallet(
        ...
        std_wallet_id = self.standard_wallet.wallet_id
        self.singleton_store = self.wallet_state_manager.singleton_store
        bal = await wallet_state_manager.get_confirmed_balance_for_wallet(std_wallet_id)
    ...
    async def generate_new_dao(
        ...
        # add the treasury singleton (generated by eve spend) to unconfirmed_singleton store
        await self.wallet_state_manager.singleton_store.add_unconfirmed_singleton(
            coin_spend=eve_spend.coin_spends[0],
    ...
    async def generate_new_proposal(self, proposed_puzzle_hash, fee):
        ...
        # add proposal singleton to unconfirmed_singleton store
        await self.wallet_state_manager.singleton_store.add_unconfirmed_singleton(
            coin_spend=eve_spend.coin_spends[0],
            wallet_id=self.wallet_id,
        )
    ...
    async def create_add_money_to_treasury_spend(self, amount: uint64, fee: uint64 = uint64(0)) -> Optional[TransactionRecord]:
        ...
        await self.wallet_state_manager.add_interested_coin_ids([current_coin.name()], [self.wallet_id])
        await self.singleton_store.add_unconfirmed_singleton(
            coin_spend=treasury_sb.coin_spends[0],
            wallet_id=self.wallet_id,
        )
    ...
    async def apply_state_transition(self, coin_spend: CoinSpend, block_height: uint32) -> bool:
        """
        ...
        if not singleton_id:
            raise ValueError("DAO Wallet recieved a coin which is not a valid singleton")
        new_coin = get_most_recent_singleton_coin_from_coin_spend(coin_spend)
        confirmed_record = await self.singleton_store.get_record_by_coin_id(new_coin.name())
        unconfirmed_record = await self.singleton_store.get_unconfirmed_singleton_by_coin_id(new_coin.name())

        if confirmed_record:
            # TODO: Find out why we would be encountering this. Seems to happen during add_money spend
            self.log.info(f"Coin already confirmed: {new_coin.name().hex()}")
            return False
        elif unconfirmed_record:
            self.log.info(f"Confirming unconfirmed coin: {new_coin.name().hex()}")
            await self.singleton_store.confirm_unconfirmed_singleton(new_coin.name(), block_height)
        else:
            self.log.info(f"Received new coin for DAO Wallet {self.walllet_id}. coin_id: {new_coin.name().hex()}")
            record = await self.singleton_store.create_singleton_record_from_coin_spend(coin_spend, self.wallet_id)
            await self.singleton_store.add_confirmed_singleton(record, block_height)

        # Consume new DAOBlockchainInfo
        # Determine if this is a treasury spend or a proposal spend
        puzzle = get_innerpuzzle_from_puzzle(coin_spend.puzzle_reveal)
        try:
        ...
        if mod == DAO_TREASURY_MOD:
            await self.update_treasury_info(coin_spend, block_height)
        elif mod == DAO_PROPOSAL_MOD:
            await self.add_or_update_proposal_info(coin_spend, block_height)
        else:
