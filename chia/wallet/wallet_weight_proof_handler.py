...
class WalletWeightProofHandler:
    ...
    async def validate_weight_proof(
        ...
        #     executor, _validate_recent_blocks, constants, wp_recent_chain_bytes, summary_bytes, True
        # )
        ...
        valid_segments = _validate_sub_epoch_segments(constants, rng, wp_segment_bytes, summary_bytes)
        if not valid_segments:
            log.error("failed validating weight proof sub epoch segments")
            return False, uint32(0), [], []

        recent_block_records_obj = []
        for block_record in recent_block_records:
            recent_block_records_obj.append(BlockRecord.from_bytes(block_record))
        # TODO fix find fork point
        return True, 0, summaries, recent_block_records_obj
