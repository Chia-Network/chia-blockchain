...
def construct_cr_layer_hash(
    ...
    first_curry_hash: bytes32 = Program.to(CREDENTIAL_RESTRICTION_HASH).curry(
        CREDENTIAL_STRUCT_HASH,
        authorized_providers_hash,
        proofs_checker_hash,
    ).get_tree_hash_precalc(CREDENTIAL_RESTRICTION_HASH, CREDENTIAL_STRUCT_HASH, authorized_providers_hash, proofs_checker_hash)
    first_curry_hash_hash: bytes32 = Program.to(first_curry_hash).get_tree_hash()
    return Program.to(first_curry_hash).curry(first_curry_hash_hash, inner_puzzle_hash).get_tree_hash_precalc(first_curry_hash, first_curry_hash_hash, inner_puzzle_hash)
