...
import logging
import time
from typing import TYPE_CHECKING, Any, ClassVar, Dict, Iterable, List, Optional, Set, Tuple, cast
...
class Wallet:
    ...
    async def sum_hint_for_pubkey(self, pk: bytes) -> Optional[SumHint]:
        ...
        if dr is None:
            return None
        return SumHint(
            [Program.to(dr.pubkey.get_fingerprint().to_bytes(4, "big"))],
            Program.to(calculate_synthetic_offset(dr.pubkey, DEFAULT_HIDDEN_PUZZLE_HASH)),
        )
    ...
    async def path_hint_for_pubkey(self, pk: bytes) -> Optional[PathHint]:
        ...
        return PathHint(
            root_pubkey,
            [Program.to(12381), Program.to(8444), Program.to(2), Program.to(index)],
        )
    ...
    async def execute_signing_instructions(
        ...
        for sum_hint in signing_instructions.key_hints.sum_hints:
            ...
            for fingerprint in sum_hint.fingerprints:
                fingerprint_as_int = int.from_bytes(fingerprint.atom, "big")
                if fingerprint_as_int not in pk_lookup:
                    if not partial_allowed:
                        raise ValueError(
                            "No pubkey found (or path hinted to) for "
                            f"fingerprint {int.from_bytes(fingerprint.atom, 'big')}"
                        )
                    else:
                        break
                else:
                ...
                synthetic_secret_exponent = (secret_exponent + sum_hint.synthetic_offset.as_int()) % GROUP_ORDER
        ...
        for target in signing_instructions.targets:
            pk_fingerprint: int = G1Element.from_bytes(target.pubkey.atom).get_fingerprint()
            ...
            responses.append(
                SigningResponse(
                    Program.to(bytes(AugSchemeMPL.sign(sk_lookup[pk_fingerprint], target.message.atom))),
                    target.hook,
                )
            )
    ...
    async def apply_signatures(
        ...
        return SpendBundle(
            ...
            AugSchemeMPL.aggregate(
                [G2Element.from_bytes(signing_response.signature.atom) for signing_response in signing_responses]
            ),
        )
