...
async def start_rpc_server(
    ...
    try:
        ...
        routes = rpc_server.get_routes()
        app.add_routes([web.post(route, wrap_http_handler(func)) for (route, func) in routes.items()])
        ...
        log.error(f" ==== start_rpc_server() {rpc_port=} {routes=}")
