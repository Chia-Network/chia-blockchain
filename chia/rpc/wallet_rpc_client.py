...
class WalletRpcClient(RpcClient):
    ...
    async def get_offer_summary(self, offer: Offer) -> Dict[str, Dict[str, int]]:
        res = await self.fetch("get_offer_summary", {"offer": offer.compress().hex()})
        return res["summary"]

    async def check_offer_validity(self, offer: Offer) -> bool:
        res = await self.fetch("check_offer_validity", {"offer": offer.compress().hex()})
        return res["valid"]

    async def take_offer(self, offer: Offer, fee=uint64(0)) -> TradeRecord:
        res = await self.fetch("take_offer", {"offer": offer.compress().hex(), "fee": fee})
