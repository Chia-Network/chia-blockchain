...
class WalletRpcClient(RpcClient):
    ...
    async def create_offer_for_ids(
        ...
        offer: Optional[Offer] = None if validate_only else Offer.from_compressed(decode_offer_bytes(res["offer"]))
    ...
    async def get_offer(self, trade_id: bytes32, file_contents: bool = False) -> TradeRecord:
        ...
        offer_str = bytes(Offer.from_compressed(decode_offer_bytes(res["offer"]))).hex() if file_contents else ""
    ...
    async def get_all_offers(
        ...
        records = []
        if file_contents:
            optional_offers = [bytes(Offer.from_compressed(decode_offer_bytes(o))).hex() for o in res["offers"]]
        else:
