...
class WalletRpcApi:
    ...
    async def create_new_wallet(self, request: Dict[str, Any]) -> EndpointResult:
    ...
    async def send_transaction(self, request: Dict[str, Any]) -> EndpointResult:
        if await self.service.wallet_state_manager.synced() is False:
        ...
        excluded_coin_ids: Optional[List[str]] = request.get("excluded_coin_ids", request.get("exclude_coin_ids"))
    ...
    async def spend_clawback_coins(self, request: Dict[str, Any]) -> EndpointResult:
    ...
    async def send_notification(self, request: Dict[str, Any]) -> EndpointResult:
    ...
    async def cat_spend(self, request: Dict[str, Any], hold_lock: bool = True) -> EndpointResult:
        if await self.service.wallet_state_manager.synced() is False:
            raise ValueError("Wallet needs to be fully synced.")
        wallet_id = uint32(request["wallet_id"])
        wallet = self.service.wallet_state_manager.get_wallet(id=wallet_id, required_type=CATWallet)
    ...
    async def create_offer_for_ids(self, request: Dict[str, Any]) -> EndpointResult:
    ...
    async def take_offer(self, request: Dict[str, Any]) -> EndpointResult:
        offer_hex: str = request["offer"]
    ...
    async def cancel_offer(self, request: Dict[str, Any]) -> EndpointResult:
    ...
    async def cancel_offers(self, request: Dict[str, Any]) -> EndpointResult:
        secure = request["secure"]
    ...
    async def did_update_recovery_ids(self, request: Dict[str, Any]) -> EndpointResult:
        wallet_id = uint32(request["wallet_id"])
        wallet = self.service.wallet_state_manager.get_wallet(id=wallet_id, required_type=DIDWallet)
        recovery_list = []
        success: bool = False
    ...
    async def did_message_spend(self, request: Dict[str, Any]) -> EndpointResult:
        wallet_id = uint32(request["wallet_id"])
        wallet = self.service.wallet_state_manager.get_wallet(id=wallet_id, required_type=DIDWallet)
    ...
    async def did_update_metadata(self, request: Dict[str, Any]) -> EndpointResult:
        wallet_id = uint32(request["wallet_id"])
        wallet = self.service.wallet_state_manager.get_wallet(id=wallet_id, required_type=DIDWallet)
        metadata: Dict[str, str] = {}
    ...
    async def did_create_attest(self, request: Dict[str, Any]) -> EndpointResult:
        wallet_id = uint32(request["wallet_id"])
        wallet = self.service.wallet_state_manager.get_wallet(id=wallet_id, required_type=DIDWallet)
        async with self.service.wallet_state_manager.lock:
    ...
    async def did_transfer_did(self, request: Dict[str, Any]) -> EndpointResult:
        if await self.service.wallet_state_manager.synced() is False:
            raise ValueError("Wallet needs to be fully synced.")
        wallet_id = uint32(request["wallet_id"])
        did_wallet = self.service.wallet_state_manager.get_wallet(id=wallet_id, required_type=DIDWallet)
    ...
    async def nft_mint_nft(self, request: Dict[str, Any]) -> EndpointResult:
    ...
    async def nft_set_nft_did(self, request: Dict[str, Any]) -> EndpointResult:
        wallet_id = uint32(request["wallet_id"])
        nft_wallet = self.service.wallet_state_manager.get_wallet(id=wallet_id, required_type=NFTWallet)
    ...
    async def nft_set_did_bulk(self, request: Dict[str, Any]) -> EndpointResult:
        """
    ...
    async def nft_transfer_bulk(self, request: Dict[str, Any]) -> EndpointResult:
        """
    ...
    async def nft_transfer_nft(self, request: Dict[str, Any]) -> EndpointResult:
        wallet_id = uint32(request["wallet_id"])
        address = request["target_address"]
        if isinstance(address, str):
            puzzle_hash = decode_puzzle_hash(address)
    ...
    async def nft_add_uri(self, request: Dict[str, Any]) -> EndpointResult:
        wallet_id = uint32(request["wallet_id"])
    ...
    async def nft_mint_bulk(self, request: Dict[str, Any]) -> EndpointResult:
        if await self.service.wallet_state_manager.synced() is False:
            raise ValueError("Wallet needs to be fully synced.")
        wallet_id = uint32(request["wallet_id"])
        nft_wallet = self.service.wallet_state_manager.get_wallet(id=wallet_id, required_type=NFTWallet)
    ...
    async def create_signed_transaction(self, request: Dict[str, Any], hold_lock: bool = True) -> EndpointResult:
    ...
    # Pool Wallet
    ##########################################################################################
    async def pw_join_pool(self, request: Dict[str, Any]) -> EndpointResult:
        fee = uint64(request.get("fee", 0))
        wallet_id = uint32(request["wallet_id"])
    ...
    async def pw_self_pool(self, request: Dict[str, Any]) -> EndpointResult:
    ...
    async def pw_absorb_rewards(self, request: Dict[str, Any]) -> EndpointResult:
    ...
    # DataLayer Wallet
    ##########################################################################################
    async def create_new_dl(self, request: Dict[str, Any]) -> EndpointResult:
    ...
    async def dl_update_root(self, request: Dict[str, Any]) -> EndpointResult:
        """Get the singleton record for the latest singleton of a launcher ID"""
        if self.service.wallet_state_manager is None:
            raise ValueError("The wallet service is not currently initialized")
    ...
    async def dl_update_multiple(self, request: Dict[str, Any]) -> EndpointResult:
    ...
    async def dl_new_mirror(self, request: Dict[str, Any]) -> EndpointResult:
    ...
    async def dl_delete_mirror(self, request: Dict[str, Any]) -> EndpointResult:
    ...
    # Verified Credential
    ##########################################################################################
    async def vc_mint(self, request: Dict[str, Any]) -> EndpointResult:
        """
    ...
    async def vc_spend(self, request: Dict[str, Any]) -> EndpointResult:
        """
    ...
    async def vc_revoke(self, request: Dict[str, Any]) -> EndpointResult:
        """
