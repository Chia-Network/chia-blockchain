...
class WalletRpcApi:
        ...
        if request["wallet_type"] == "cat_wallet":
            ...
            if request["mode"] == "new":
                ...
                async with self.service.wallet_state_manager.lock:
                    ...
                    return {
                        "total_fee": action_scope.config.total_fee * 2,
    ...
    async def split_coins(
        ...
        async with action_scope.use() as interface:
            ...
            if wallet.type() == WalletType.STANDARD_WALLET:
                ...
                if coin.amount < total_amount + interface.side_effects.fee_left_to_pay:
                    async with action_scope.use() as interface:
                        interface.side_effects.selected_coins.append(coin)
                    coins = await wallet.select_coins(
                        uint64(total_amount + interface.side_effects.fee_left_to_pay - coin.amount),
                        action_scope,
                    )
                    coins.add(coin)
                else:
    ...
    async def combine_coins(
        ...
        async with action_scope.use() as interface:
            ...
            if isinstance(wallet, Wallet):
                fungible_amount_needed = uint64(fungible_amount_needed + interface.side_effects.fee_left_to_pay)
            ...
            primary_output_amount = (
                ...
                if request.target_coin_amount is None
                else request.target_coin_amount
            )

            if isinstance(wallet, Wallet):
                primary_output_amount = uint64(primary_output_amount - interface.side_effects.fee_left_to_pay)
                await wallet.generate_signed_transaction(
                    primary_output_amount,
    ...
    async def cat_spend(
        ...
        if additions is None and not isinstance(request["amount"], int):
            raise ValueError("An integer amount is required (too many decimals)")
    ...
    async def cancel_offer(
        ...
        async with self.service.wallet_state_manager.lock:
            await wsm.trade_manager.cancel_pending_offers(
                [bytes32(trade_id)], action_scope, secure=secure, extra_conditions=extra_conditions
            )
    ...
    async def cancel_offers(
        ...
        secure = request["secure"]
        # TODO: grumble grumble
        uint64(request.get("batch_fee", 0))
    ...
    async def did_update_recovery_ids(
        ...
        async with self.service.wallet_state_manager.lock:
            ...
            if update_success:
                await wallet.create_update_spend(action_scope, extra_conditions=extra_conditions)
                return {
                    "success": True,
                    "transactions": None,  # tx_endpoint wrapper will take care of this
                }
        ...
        for wallet_id, nft_list in nft_dict.items():
            ...
            if not first:
                ...
                await nft_wallet.set_bulk_nft_did(
                    nft_list, did_id, action_scope, nft_ids, extra_conditions=extra_conditions
                )
    ...
    async def nft_add_uri(
        ...
        uint64(request.get("fee", 0))
        await nft_wallet.update_metadata(nft_coin_info, key, uri, action_scope, extra_conditions=extra_conditions)
        return {
            "wallet_id": wallet_id,
            "success": True,
            "spend_bundle": None,  # tx_endpoint wrapper will take care of this
    ...
    async def pw_join_pool(
        ...
        async with self.service.wallet_state_manager.lock:
            total_fee = await wallet.join_pool(new_target_state, action_scope)
            return {
                "total_fee": total_fee,
                "transaction": None,  # tx_endpoint wrapper will take care of this
    ...
    async def pw_self_pool(
        ...
        async with self.service.wallet_state_manager.lock:
            total_fee = await wallet.self_pool(action_scope)
            return {
                "total_fee": total_fee,
                "transaction": None,  # tx_endpoint wrapper will take care of this
                "fee_transaction": None,  # tx_endpoint wrapper will take care of this
    ...
    async def pw_absorb_rewards(
        ...
        assert isinstance(wallet, PoolWallet)
        async with self.service.wallet_state_manager.lock:
            await wallet.claim_pool_rewards(max_spends_in_tx, action_scope)
    ...
    async def vc_mint(
        ...
        vc_record = await vc_wallet.launch_new_vc(did_id, action_scope, puzhash, extra_conditions=extra_conditions)
        return {
