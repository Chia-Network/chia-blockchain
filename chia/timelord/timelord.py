...
class Timelord:
    ...
    async def manage(self) -> AsyncIterator[None]:
        ...
        if not self.bluebox_mode:
            self.main_loop = pit.create_task(self._manage_chains())
        else:
            if os.name == "nt" or slow_bluebox:
                ...
                self.main_loop = pit.create_task(
                    self._start_manage_discriminant_queue_sanitizer_slow(self.bluebox_pool, workers)
                )
            else:
                self.main_loop = pit.create_task(self._manage_discriminant_queue_sanitizer())
    ...
    async def _start_manage_discriminant_queue_sanitizer_slow(self, pool: ThreadPoolExecutor, counter: int) -> None:
        ...
        for _ in range(counter):
            tasks.append(pit.create_task(self._manage_discriminant_queue_sanitizer_slow(pool)))
        for task in tasks:
            await task
