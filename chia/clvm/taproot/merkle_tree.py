...
def do_a_pair(left: bytes32, right: bytes32) -> bytes32:
    return std_hash(TWO + left + right)


def do_a_atom(atom: bytes32) -> bytes32:
...
class MerkleTree:
    ...
    def _root(self, puzzle_hashes: List[bytes32]) -> bytes32:
        if len(puzzle_hashes) == 1:
            return do_a_atom(puzzle_hashes[0])
        else:
            first, rest = self.split_list(puzzle_hashes)
            return do_a_pair(self._root(first), self._root(rest))
    ...
    def _proof(self, puzzle_hashes: List[bytes32], searching_for: bytes32):
        if len(puzzle_hashes) == 1:
            atom_hash = do_a_atom(puzzle_hashes[0])
            ...
            pair_hash = do_a_pair(first_hash[2], rest_hash[2])
