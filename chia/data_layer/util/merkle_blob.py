...
class MerkleBlob:
    ...
    def insert(
        ...
        if len(self.blob) == 0:
            ...
            self.leaf_hash_to_index[hash] = TreeIndex(uint32(0))
            self.free_indexes = []
        ...
        if len(self.key_to_index) == 1:
            ...
            for index, leaf in enumerate([leaf_1, leaf_2], start=1):
                ...
                self.leaf_hash_to_index[leaf.hash] = TreeIndex(uint32(index))
            self.free_indexes = []
    ...
    def delete(self, key: KeyId) -> None:
        ...
        if grandparent_index is None:
            ...
            if isinstance(sibling, RawLeafMerkleNode):
                ...
                self.leaf_hash_to_index[sibling.hash] = TreeIndex(uint32(0))
            else:
                assert isinstance(sibling, RawInternalMerkleNode)
