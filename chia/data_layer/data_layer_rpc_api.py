...
class DataLayerRpcApi:
    ...
    async def get_row(self, request: Dict) -> bytes:
        row = b""
        if "hash" in request:
            row = await self.service.data_store.get_row_by_hash(request["hash"])
        elif "index" in request:
            row = await self.service.data_store.get_row_by_index(request["index"])
        return row
    ...
    async def update(self, request: Dict):
        ...
        for change in changelist:
            if change["action"] == "insert":
                ...
                index = await self.service.data_store.insert_row(table, row)
            else:
                assert change["action"] == "delete"
                row, index = await self.service.data_store.delete_row_by_hash(table, change["row"])
