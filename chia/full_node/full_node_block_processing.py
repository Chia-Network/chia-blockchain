...
from chia.full_node.full_node_mempool_protocol import MempoolProtocolMixin
...
class BlockProcessingMixin(MempoolProtocolMixin):
    # Attribute stubs - provided by the class that uses this mixin (e.g. FullNode).
    # Do NOT declare: server, blockchain, block_store, mempool_manager,
    # coin_store, hint_store, timelord_lock, db_wrapper (properties on FullNode).
    _bls_cache: BLSCache
    constants: Any  # ConsensusConstants
    full_node_store: FullNodeStore
    log: logging.Logger
    profile_block_validation: bool
    root_path: Path
    _segment_task_list: list[asyncio.Task[None]]
    signage_point_times: list[float]
    subscriptions: Any  # PeerSubscriptions
    sync_store: Any  # SyncStore
