...
STANDARD_TRANSACTION_PUZZLE_PREFIX_PATTERN = re.compile(STANDARD_TRANSACTION_PUZZLE_PREFIX)
...
def match_standard_transaction_at_any_index(generator_body: bytes) -> Optional[Tuple[int, int]]:
    ...
    m = STANDARD_TRANSACTION_PUZZLE_PREFIX_PATTERN.search(generator_body.hex())
    if m:
