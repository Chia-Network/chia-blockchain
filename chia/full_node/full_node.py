...
class FullNode:
    ...
    async def _start(self) -> None:
        ...
        if peak is None:
            ...
            async with self.blockchain.lock_queue.acquire(priority=BlockchainLockPriority.high):
    ...
    async def peak_post_processing(
        ...
        """
        Must be called under self.blockchain.lock_queue. This updates the internal state of the full node with the
    ...
    async def add_transaction(
        ...
        if self.sync_store.get_sync_mode() or self.mempool_manager.peak is None:
            ...
            try:
                ...
                self.mempool_manager.remove_seen(spend_name)
                raise
            async with self.blockchain.lock_queue.acquire(priority=BlockchainLockPriority.low):
                if self.mempool_manager.get_spendbundle(spend_name) is not None:
                    self.mempool_manager.remove_seen(spend_name)
                    return MempoolInclusionStatus.SUCCESS, None
