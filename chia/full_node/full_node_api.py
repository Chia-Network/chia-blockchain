    ...
    try:
        while True:
            ...
            full_node.log.warning(
                f"tx_request_and_timeout attempt for tx {transaction_id.hex()[:8]}, loop index {counter}, "
                f"dynamic peers_with_tx has {len(full_node.full_node_store.peers_with_tx.get(transaction_id, {}))} "
                f"peers and the copy has {len(peers_with_tx)} peers"
            )
            if counter == 5:
                full_node.log.warning(f"reached 5 tries for tx {transaction_id.hex()[:8]}, giving up")
                break
            ...
            if len(peers_with_tx) == 0:
                full_node.log.warning(f"peers_with_tx got 0 for tx {transaction_id.hex()[:8]}.. breaking")
                break
            ...
            if full_node.mempool_manager.seen(transaction_id):
                ...
                full_node.log.warning(
                    ...
                    f"peer is {peer_id.hex()[:8]} and loop index is {counter}"
                )
