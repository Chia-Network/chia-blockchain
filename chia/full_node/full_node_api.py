...
class FullNodeAPI:
    ...
    async def new_peak(self, request: full_node_protocol.NewPeak, peer: WSChiaConnection) -> None:
        ...
        try:
            ...
            self.log.debug(f"Ignoring NewPeak, limited semaphore full: %s %s", peer.get_peer_logging(), request)
            return None
    ...
    async def new_compact_vdf(
        ...
        if name in self.full_node.compact_vdf_requests:
            self.log.debug(f"Ignoring NewCompactVDF, already requested: %s %s", peer.get_peer_logging(), request)
            return None
        ...
        try:
            ...
            self.log.debug(f"Ignoring NewCompactVDF, limited semaphore full: %s %s", peer.get_peer_logging(), request)
            return None
