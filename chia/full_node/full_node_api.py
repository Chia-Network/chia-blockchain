    ...
    try:
        ...
        for i in range(5):
            full_node.log.warning(
                ...
                f"peers_with_tx has {len(full_node.full_node_store.peers_with_tx.get(transaction_id, {}))} peers"
            )
            peers_with_tx = full_node.full_node_store.peers_with_tx.get(transaction_id)
            if peers_with_tx is None:
                full_node.log.warning(f"tx {transaction_id.hex()[:8]} not in peers_with_tx.. breaking")
                break
            peers_to_try = set(peers_with_tx) - tried_peers
            if len(peers_to_try) == 0:
                full_node.log.warning(f"peers_to_try is 0 for tx {transaction_id.hex()[:8]}.. breaking")
                break
