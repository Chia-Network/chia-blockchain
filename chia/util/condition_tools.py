...
def pkm_pairs(conditions: SpendBundleConditions, additional_data: bytes) -> Tuple[List[PublicKeyBytes], List[bytes]]:
    ret: Tuple[List[PublicKeyBytes], List[bytes]] = ([], [])

    for pk, msg in conditions.agg_sig_unsafe:
        ret[0].append(bytes_to_PublicKeyBytes(pk))
        ret[1].append(msg)
...
def pkm_pairs_for_conditions_dict(
...
) -> List[Tuple[PublicKeyBytes, bytes]]:
    assert coin_name is not None
    ret: List[Tuple[PublicKeyBytes, bytes]] = []
    ...
    for cwa in conditions_dict.get(ConditionOpcode.AGG_SIG_UNSAFE, []):
        ...
        ret.append((bytes_to_PublicKeyBytes(cwa.vars[0]), cwa.vars[1]))

    for cwa in conditions_dict.get(ConditionOpcode.AGG_SIG_ME, []):
        assert len(cwa.vars) == 2
        assert len(cwa.vars[0]) == 48 and len(cwa.vars[1]) <= 1024
        assert cwa.vars[0] is not None and cwa.vars[1] is not None
        ret.append((bytes_to_PublicKeyBytes(cwa.vars[0]), cwa.vars[1] + coin_name + additional_data))
    return ret
