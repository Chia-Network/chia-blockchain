...
from typing import Any, AsyncIterator, Dict, Sequence, Optional, TextIO
from psycopg.rows import Row, AsyncRowFactory
...
async def execute_fetchone(
    c: psycopg.AsyncConnection[Row], sql: str, parameters: Optional[Sequence[Any]] = None
) -> Optional[Row]:
...
async def manage_connection(
    ...
    name: Optional[str] = None,
    row_factory: Optional[AsyncRowFactory[Row]] = None,
) -> AsyncIterator[psycopg.AsyncConnection[Row]]:
...
def get_host_parameter_limit() -> int:
    ...
    if sys.version_info >= (3, 11):
        connection = psycopg.connect(":memory:")

        # sqlite3.SQLITE_LIMIT_VARIABLE_NUMBER exists in 3.11, pylint
        limit_number = psycopg.SQLITE_LIMIT_VARIABLE_NUMBER  # pylint: disable=E1101
        ...
        if psycopg.sqlite_version_info >= (3, 32, 0):
...
class DBWrapper2:
    ...
    async def add_connection(self, c: psycopg.AsyncConnection[Row]) -> None:
    ...
    @contextlib.asynccontextmanager
    async def writer(self) -> AsyncIterator[psycopg.AsyncConnection[Row]]:
        """
    ...
    @contextlib.asynccontextmanager
    async def writer_maybe_transaction(self) -> AsyncIterator[psycopg.AsyncConnection[Row]]:
        """
