...
def announcements_for_solution(coin: Coin, puzzle_reveal: Program, solution: Program) -> List[Announcement]:
    """
    ...
    if err or dic is None:
        return []
    return created_announcements_for_conditions_dict(dic, coin)


def announcement_names_for_solution(coin: Coin, puzzle_reveal: Program, solution: Program) -> List[bytes32]:
    """
    Checks the conditions created by CoinSolution and returns the list of announcements
    """
    err, dic, cost = conditions_dict_for_solution(puzzle_reveal, solution)
    if err or dic is None:
        return []
    return created_announcement_names_for_conditions_dict(dic, coin)
