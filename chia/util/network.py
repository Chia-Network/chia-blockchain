...
def class_for_type(type: NodeType) -> Any:
    if type is NodeType.FULL_NODE:
        ...
        from deafwave.introducer.introducer_api import IntroducerAPI

        return IntroducerAPI
    elif type is NodeType.TIMELORD:
        from deafwave.timelord.timelord_api import TimelordAPI

        return TimelordAPI
    elif type is NodeType.FARMER:
        from deafwave.farmer.farmer_api import FarmerAPI

        return FarmerAPI
    elif type is NodeType.HARVESTER:
        from deafwave.harvester.harvester_api import HarvesterAPI
