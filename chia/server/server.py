    ...
    async def incoming_connection(self, request):
        ...
        try:
            ...
            if not self.accept_inbound_connections(connection.connection_type) and not is_in_network(
                ...
                self.log.info(
                    f"Not accepting inbound connection: {connection.get_peer_info_or_host()}.Inbound limit reached."
                )
                await connection.close()
                close_event.set()
            else:
                ...
                try:
                    ...
                    async def wrapped_coroutine() -> Optional[Message]:
                        try:
                            ...
                            connection.log.error(f"Exception: {e}, {connection.get_peer_info_or_host()}. {tb}")
                            raise e
                        return None
                    ...
                    if self.connection_close_task is None:
                        ...
                        connection.log.error(
                            f"Exception: {e} {type(e)}, closing connection {connection.get_peer_info_or_host()}. {tb}"
                        )
                    else:
