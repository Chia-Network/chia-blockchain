...
class AddressManagerSQLiteStore:
    """
    Supports creation of an AddressManager deserialized from a SQLite database.
    """

    @classmethod
    async def create(cls, db_path: Path) -> Optional[AddressManager]:
        """ """
        async with aiosqlite.connect(db_path) as connection:
            await connection.execute("pragma journal_mode=wal")
            await connection.execute("pragma synchronous=OFF")

            metadata: Dict[str, str] = await cls._get_metadata(connection)
            address_manager: Optional[AddressManager] = None

            if not await cls._is_empty(metadata):
                nodes: List[Node] = await cls._get_nodes(connection)
                new_table_entries: List[Table] = await cls._get_new_table(connection)
                address_manager = cls._create_address_manager(metadata, nodes, new_table_entries)

            return address_manager

    @classmethod
    async def _get_metadata(cls, connection: aiosqlite.Connection) -> Dict[str, str]:
    ...
    @classmethod
    async def _get_nodes(cls, connection: aiosqlite.Connection) -> List[Node]:
    ...
    @classmethod
    async def _get_new_table(cls, connection: aiosqlite.Connection) -> List[Table]:
    ...
    @classmethod
    async def _is_empty(cls, metadata: Dict[str, str]) -> bool:
    ...
    @classmethod
    def _create_address_manager(
        cls, metadata: Dict[str, str], nodes: List[Node], new_table_entries: List[Table]
    ) -> AddressManager:
