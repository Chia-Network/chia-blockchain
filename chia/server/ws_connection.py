...
class WSChiaConnection:
    ...
    async def _send_message(self, message: Message):
        ...
        rate_limit_exception_reason = None
        if is_localhost(self.peer_host):
            rate_limit_exception_reason = "peer is localhost"
        ...
        if ProtocolMessageTypes(message.type) == ProtocolMessageTypes.respond_peers:
            rate_limit_exception_reason = "we are gossiping peers"

        if not self.outbound_rate_limiter.process_msg_and_check(message):
            if rate_limit_exception_reason is None:
                self.log.debug(
                ...
                self.log.debug(
                    f"Not rate limiting ourselves because {rate_limit_exception_reason}. "
