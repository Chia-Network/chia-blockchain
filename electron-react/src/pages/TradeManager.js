...
import { Box, Typography, Grid, Paper, FormControl, InputLabel, MenuItem, Select, TextField, Button } from '@material-ui/core';
import { makeStyles } from '@material-ui/core/styles';
import { newBuy, newSell, addTrade, resetTrades } from '../modules/TradeReducer';
    ...
    return (
        <Grid item xs={12}>
            ...
            <Box display="flex" style={{ minWidth: '100%' }}>
                        ...
                        <Box style={{marginRight: 10, width:'40%', textAlign: 'right', overflowWrap: 'break-word',}}>
                            Colour
                        </Box>
                    </Box>
                {trades.map(trade => (
                    <Box display="flex" style={{ minWidth: '100%' }}>
                        ...
                        <Box flexGrow={1}>
                            {mojo_to_chia_string(trade.amount)}
                        </Box>
                        <Box style={{marginRight: 10, width:'40%', textAlign: 'right', overflowWrap: 'break-word',}}>
...
const CreateOffer = () => {
    ...
    async function save() {
        ...
        if (isElectron()) {
            ...
            dispatch(openDialog("", "This feature is available only from electron app"))
        }
    }
    function cancel() {
            ...
            <Grid container spacing={0}>
                        ...
                        <Grid container spacing={2}>
                            <Grid item xs={6}>
                                <FormControl fullWidth variant="outlined" className={classes.formControl}>
                                    ...
                                    <Select
                                        inputRef={(input) => { buy_or_sell = input }}
                                        label="Buy Or Sell"
                                    >
                            ...
                            <Grid item xs={6}>
                                <FormControl fullWidth variant="outlined" className={classes.formControl}>
                                    ...
                                    <Select
                                        inputRef={(input) => { wallet_id = input; }}
                                        label="Colour"
                                    >
                                        {wallets.map(wallet => (<MenuItem value={wallet.id}>{wallet.name}</MenuItem>))}
                                    </Select>
                                </FormControl>
                            </Grid>
                        </Grid>
                        ...
                        <Box display="flex">
                            <Box flexGrow={1} >
                                <TextField className={classes.input}
                                    fullWidth id="outlined-basic" inputRef={(input) => { amount_input = input; }} label="Amount" variant="outlined" />
                            </Box>
                            <Box>
                                <Button onClick={add} className={classes.send} variant="contained" color="primary">
                                    Add
                            </Button>
                            </Box>
                        </Box>
                ...
                <Grid item xs={12}>
                    <div className={classes.cardSubSection} >
                        <Grid container spacing={2}>
                            <Grid item xs={6}>
                                <Button onClick={save} className={classes.saveButton} variant="contained" color="primary">
                                    Save
                            </Button>
                            </Grid>
                            <Grid item xs={6}>
                                <Button onClick={cancel} className={classes.cancelButton} variant="contained" color="primary">
                                    Cancel
                            </Button>
                            </Grid>
                        </Grid>
