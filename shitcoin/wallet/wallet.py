...
from shitcoin.consensus.cost_calculator import calculate_cost_of_program, NPCResult
...
from shitcoin.types.announcement import Announcement
...
from shitcoin.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import (
    DEFAULT_HIDDEN_PUZZLE_HASH,
    calculate_synthetic_secret_key,
    puzzle_for_pk,
    solution_for_conditions,
)
from shitcoin.wallet.puzzles.puzzle_utils import (
...
from shitcoin.wallet.secret_key_store import SecretKeyStore
from shitcoin.wallet.sign_coin_spends import sign_coin_spends
...
from shitcoin.wallet.wallet_info import WalletInfo
...
class Wallet:
    ...
    async def create_spend_bundle_relative_shitcoin(self, shitcoin_amount: int, exclude: List[Coin]) -> SpendBundle:
        list_of_solutions = []
        utxos = None

        # If we're losing value then get coins with at least that much value
        # If we're gaining value then our amount doesn't matter
        if shitcoin_amount < 0:
            utxos = await self.select_coins(abs(shitcoin_amount), exclude)
        else:
        ...
        shitcoin_amount = spend_value + shitcoin_amount
        ...
        for coin in utxos:
            ...
            if output_created is None:
                ...
                primaries = [{"puzzlehash": newpuzhash, "amount": shitcoin_amount}]
