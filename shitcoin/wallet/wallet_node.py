...
from shitcoin.consensus.block_record import BlockRecord
from shitcoin.consensus.blockchain_interface import BlockchainInterface
from shitcoin.consensus.constants import ConsensusConstants
from shitcoin.consensus.multiprocess_validation import PreValidationResult
from shitcoin.daemon.keychain_proxy import (
    KeychainProxy,
...
from shitcoin.pools.pool_puzzles import SINGLETON_LAUNCHER_HASH
from shitcoin.protocols import wallet_protocol
from shitcoin.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
from shitcoin.protocols.protocol_message_types import ProtocolMessageTypes
from shitcoin.protocols.wallet_protocol import (
...
from shitcoin.server.node_discovery import WalletPeers
from shitcoin.server.outbound_message import Message, NodeType, make_msg
from shitcoin.server.server import shitcoinServer
from shitcoin.server.ws_connection import WSshitcoinConnection
from shitcoin.types.blockchain_format.coin import Coin, hash_coin_list
from shitcoin.types.blockchain_format.sized_bytes import bytes32
from shitcoin.types.coin_spend import CoinSpend
from shitcoin.types.header_block import HeaderBlock
...
from shitcoin.util.check_fork_next_block import check_fork_next_block
from shitcoin.util.errors import Err, ValidationError
from shitcoin.util.ints import uint32, uint128
from shitcoin.util.keychain import Keychain
from shitcoin.util.lru_cache import LRUCache
from shitcoin.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
from shitcoin.util.path import mkdir, path_from_root
from shitcoin.wallet.block_record import HeaderBlockRecord
from shitcoin.wallet.derivation_record import DerivationRecord
from shitcoin.wallet.settings.settings_objects import BackupInitialized
from shitcoin.wallet.transaction_record import TransactionRecord
from shitcoin.wallet.util.backup_utils import open_backup_file
from shitcoin.wallet.util.wallet_types import WalletType
from shitcoin.wallet.wallet_action import WalletAction
from shitcoin.wallet.wallet_blockchain import ReceiveBlockResult
from shitcoin.wallet.wallet_state_manager import WalletStateManager
from shitcoin.util.profiler import profile_task
...
class WalletNode:
    ...
    server: Optional[shitcoinServer]
    log: logging.Logger
    ...
    async def get_key_for_fingerprint(self, fingerprint: Optional[int]) -> Optional[PrivateKey]:
        ...
        try:
            ...
            self.log.warning("No keys present. Create keys with the UI, or with the 'shitcoin keys' program.")
            return None
    ...
    def set_server(self, server: shitcoinServer):
        self.server = server
    ...
    async def on_connect(self, peer: WSshitcoinConnection):
        if self.wallet_state_manager is None or self.backup_initialized is False:
            return None
    ...
    async def complete_blocks(self, header_blocks: List[HeaderBlock], peer: WSshitcoinConnection):
    ...
    async def new_peak_wallet(self, peak: wallet_protocol.NewPeakWallet, peer: WSshitcoinConnection):
        if self.wallet_state_manager is None:
            return
    ...
    async def _sync(self) -> None:
        ...
        peers: List[WSshitcoinConnection] = self.server.get_full_node_connections()
        if len(peers) == 0:
    ...
    async def get_additions(
        self, peer: WSshitcoinConnection, block_i, additions: Optional[List[bytes32]], get_all_additions: bool = False
    ) -> Optional[List[Coin]]:
    ...
    async def get_removals(
        self, peer: WSshitcoinConnection, block_i, additions, removals, request_all_removals=False
    ) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
...
async def wallet_next_block_check(
    peer: WSshitcoinConnection, potential_peek: uint32, blockchain: BlockchainInterface
) -> bool:
