...
from shitcoin.plotters.bladebit import get_bladebit_install_info, plot_bladebit
from shitcoin.plotters.shitcoinpos import get_shitcoinpos_install_info, plot_shitcoin
from shitcoin.plotters.madmax import get_madmax_install_info, plot_madmax
from shitcoin.plotters.install_plotter import install_plotter
...
def build_parser(subparsers, root_path, option_list, name, plotter_desc):
    ...
    for option in option_list:
        ...
        u_default = 0 if name == "shitcoinpos" else 256
...
def call_plotters(root_path: Path, args):
    # Add `plotters` section in shitcoin_ROOT.
    shitcoin_root_path = root_path
    ...
    if not os.path.exists(root_path):
        print(f"Creating plotters folder within shitcoin_ROOT: {root_path}")
        try:
            os.mkdir(root_path)
        except Exception as e:
    ...
    build_parser(subparsers, root_path, shitcoin_plotter, "shitcoinpos", "shitcoinpos Plotter")
    ...
    if args.plotter == "shitcoinpos":
        plot_shitcoin(args, shitcoin_root_path)
    if args.plotter == "madmax":
        plot_madmax(args, shitcoin_root_path, root_path)
    if args.plotter == "bladebit":
        plot_bladebit(args, shitcoin_root_path, root_path)
...
def get_available_plotters(root_path) -> Dict[str, Any]:
    ...
    shitcoinpos: Optional[Dict[str, Any]] = get_shitcoinpos_install_info()
    ...
    if shitcoinpos is not None:
        plotters["shitcoinpos"] = shitcoinpos
