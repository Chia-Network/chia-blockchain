...
import shitcoin.server.ws_connection as ws
from shitcoin.consensus.block_creation import create_unfinished_block
...
from shitcoin.full_node.bundle_tools import best_solution_generator_from_template, simple_solution_generator
from shitcoin.full_node.full_node import FullNode
from shitcoin.full_node.mempool_check_conditions import get_puzzle_and_solution_for_coin
from shitcoin.full_node.signage_point import SignagePoint
from shitcoin.protocols import farmer_protocol, full_node_protocol, introducer_protocol, timelord_protocol, wallet_protocol
from shitcoin.protocols.full_node_protocol import RejectBlock, RejectBlocks
...
from shitcoin.util.api_decorators import api_request, peer_required, bytes_required, execute_task, reply_type
from shitcoin.util.generator_tools import get_block_header
...
class FullNodeAPI:
    ...
    async def request_peers(self, _request: full_node_protocol.RequestPeers, peer: ws.WSshitcoinConnection):
    ...
    async def respond_peers(
        self, request: full_node_protocol.RespondPeers, peer: ws.WSshitcoinConnection
    ) -> Optional[Message]:
    ...
    async def respond_peers_introducer(
        self, request: introducer_protocol.RespondPeersIntroducer, peer: ws.WSshitcoinConnection
    ) -> Optional[Message]:
    ...
    @peer_required
    @api_request
    async def new_peak(self, request: full_node_protocol.NewPeak, peer: ws.WSshitcoinConnection) -> Optional[Message]:
        """
    ...
    async def new_transaction(
        self, transaction: full_node_protocol.NewTransaction, peer: ws.WSshitcoinConnection
    ) -> Optional[Message]:
        """
    ...
    async def new_signage_point_or_end_of_sub_slot(
        self, new_sp: full_node_protocol.NewSignagePointOrEndOfSubSlot, peer: ws.WSshitcoinConnection
    ) -> Optional[Message]:
        # Ignore if syncing
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def respond_signage_point(
        self, request: full_node_protocol.RespondSignagePoint, peer: ws.WSshitcoinConnection
    ) -> Optional[Message]:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def respond_end_of_sub_slot(
        self, request: full_node_protocol.RespondEndOfSubSlot, peer: ws.WSshitcoinConnection
    ) -> Optional[Message]:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def declare_proof_of_space(
        self, request: farmer_protocol.DeclareProofOfSpace, peer: ws.WSshitcoinConnection
    ) -> Optional[Message]:
        """
    ...
    async def signed_values(
        self, farmer_request: farmer_protocol.SignedValues, peer: ws.WSshitcoinConnection
    ) -> Optional[Message]:
        """
    ...
    async def new_infusion_point_vdf(
        self, request: timelord_protocol.NewInfusionPointVDF, peer: ws.WSshitcoinConnection
    ) -> Optional[Message]:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def new_signage_point_vdf(
        self, request: timelord_protocol.NewSignagePointVDF, peer: ws.WSshitcoinConnection
    ) -> None:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def new_end_of_sub_slot_vdf(
        self, request: timelord_protocol.NewEndOfSubSlotVDF, peer: ws.WSshitcoinConnection
    ) -> Optional[Message]:
        if self.full_node.sync_store.get_sync_mode():
            return None
        if (
    ...
    async def new_compact_vdf(
        self, request: full_node_protocol.NewCompactVDF, peer: ws.WSshitcoinConnection, request_bytes: bytes = b""
    ):
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def request_compact_vdf(self, request: full_node_protocol.RequestCompactVDF, peer: ws.WSshitcoinConnection):
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    @peer_required
    @api_request
    async def respond_compact_vdf(self, request: full_node_protocol.RespondCompactVDF, peer: ws.WSshitcoinConnection):
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def register_interest_in_puzzle_hash(
        self, request: wallet_protocol.RegisterForPhUpdates, peer: ws.WSshitcoinConnection
    ):
    ...
    async def register_interest_in_coin(
        self, request: wallet_protocol.RegisterForCoinUpdates, peer: ws.WSshitcoinConnection
    ):
