...
import shitcoin.server.ws_connection as ws  # lgtm [py/import-and-import-from]
from shitcoin.consensus.block_creation import unfinished_block_to_full_block
...
from shitcoin.consensus.make_sub_epoch_summary import next_sub_epoch_summary
from shitcoin.consensus.multiprocess_validation import PreValidationResult
from shitcoin.consensus.pot_iterations import calculate_sp_iters
...
from shitcoin.protocols import farmer_protocol, full_node_protocol, timelord_protocol, wallet_protocol
from shitcoin.protocols.full_node_protocol import (
    RequestBlocks,
    RespondBlock,
    RespondBlocks,
    RespondSignagePoint,
...
from shitcoin.protocols.wallet_protocol import CoinState, CoinStateUpdate
from shitcoin.server.node_discovery import FullNodePeers
...
from shitcoin.types.blockchain_format.vdf import CompressibleVDFField, VDFInfo, VDFProof
...
from shitcoin.util.safe_cancel_task import cancel_task_safe
...
class FullNode:
    ...
    def set_server(self, server: shitcoinServer):
        ...
        if "dns_servers" in self.config:
            ...
            dns_servers.append("dns-introducer.shitcoin.net")
        try:
    ...
    async def short_sync_batch(self, peer: ws.WSshitcoinConnection, start_height: uint32, target_height: uint32) -> bool:
        """
    ...
    async def short_sync_backtrack(
        self, peer: ws.WSshitcoinConnection, peak_height: uint32, target_height: uint32, target_unf_hash: bytes32
    ):
        """
    ...
    async def new_peak(self, request: full_node_protocol.NewPeak, peer: ws.WSshitcoinConnection):
        """
    ...
    async def send_peak_to_timelords(
        self, peak_block: Optional[FullBlock] = None, peer: Optional[ws.WSshitcoinConnection] = None
    ):
        """
    ...
    async def on_connect(self, connection: ws.WSshitcoinConnection):
        """
    ...
    def on_disconnect(self, connection: ws.WSshitcoinConnection):
    ...
    def remove_subscriptions(self, peer: ws.WSshitcoinConnection):
    ...
    async def sync_from_fork_point(
        ...
        async def fetch_block_batches(batch_queue, peers_with_peak: List[ws.WSshitcoinConnection]):
            try:
        ...
        batch_queue: asyncio.Queue[Tuple[ws.WSshitcoinConnection, List[FullBlock]]] = asyncio.Queue(
    ...
    async def update_wallets(
        ...
        for peer, changes in changes_for_peer.items():
            if peer not in self.server.all_connections:
                continue
            ws_peer: ws.WSshitcoinConnection = self.server.all_connections[peer]
    ...
    async def peak_post_processing(
        ...
        record: BlockRecord,
        fork_height: uint32,
        peer: Optional[ws.WSshitcoinConnection],
    ...
    async def respond_block(
        ...
        peer: Optional[ws.WSshitcoinConnection] = None,
    ) -> Optional[Message]:
        """
    ...
    async def new_infusion_point_vdf(
        self, request: timelord_protocol.NewInfusionPointVDF, timelord_peer: Optional[ws.WSshitcoinConnection] = None
    ) -> Optional[Message]:
    ...
    async def new_compact_vdf(self, request: full_node_protocol.NewCompactVDF, peer: ws.WSshitcoinConnection):
...
async def node_next_block_check(
    peer: ws.WSshitcoinConnection, potential_peek: uint32, blockchain: BlockchainInterface
) -> bool:
