...
from shitcoin.cmds.passphrase_funcs import get_current_passphrase
from shitcoin.daemon.client import DaemonProxy, connect_to_daemon_and_validate
from shitcoin.util.keychain import KeyringMaxUnlockAttempts
from shitcoin.util.service_groups import services_for_groups


def launch_start_daemon(root_path: Path) -> subprocess.Popen:
    os.environ["shitcoin_ROOT"] = str(root_path)
    # TODO: use startupinfo=subprocess.DETACHED_PROCESS on windows
    shitcoin = sys.argv[0]
    process = subprocess.Popen(f"{shitcoin} run_daemon --wait-for-unlock".split(), stdout=subprocess.PIPE)
    return process
...
async def async_start(root_path: Path, group: str, restart: bool) -> None:
    ...
    if daemon is None:
        print("Failed to create the shitcoin daemon")
        return None
