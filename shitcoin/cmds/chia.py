...
from shitcoin.cmds.configure import configure_cmd
from shitcoin.cmds.farm import farm_cmd
from shitcoin.cmds.init import init_cmd
from shitcoin.cmds.keys import keys_cmd
from shitcoin.cmds.netspace import netspace_cmd
from shitcoin.cmds.passphrase import passphrase_cmd
from shitcoin.cmds.plots import plots_cmd
from shitcoin.cmds.show import show_cmd
from shitcoin.cmds.start import start_cmd
from shitcoin.cmds.stop import stop_cmd
from shitcoin.cmds.wallet import wallet_cmd
from shitcoin.cmds.plotnft import plotnft_cmd
from shitcoin.cmds.plotters import plotters_cmd
from shitcoin.util.default_root import DEFAULT_KEYS_ROOT_PATH, DEFAULT_ROOT_PATH
from shitcoin.util.keychain import set_keys_root_path, supports_keyring_passphrase
from shitcoin.util.ssl_check import check_ssl
...
@click.group(
    help=f"\n  Manage shitcoin blockchain infrastructure ({__version__})\n",
    epilog="Try 'shitcoin start node', 'shitcoin netspace -d 192', or 'shitcoin show -s'",
...
def cli(
    ...
    if passphrase_file is not None:
        from shitcoin.cmds.passphrase_funcs import cache_passphrase, read_passphrase_from_file
...
@cli.command("version", short_help="Show shitcoin version")
def version_cmd() -> None:
    print(__version__)


@cli.command("run_daemon", short_help="Runs shitcoin daemon")
@click.option(
    "--wait-for-unlock",
    help="If the keyring is passphrase-protected, the daemon will wait for an unlock command before accessing keys",
    default=False,
    is_flag=True,
    hidden=True,  # --wait-for-unlock is only set when launched by shitcoin start <service>
)
@click.pass_context
def run_daemon_cmd(ctx: click.Context, wait_for_unlock: bool) -> None:
    import asyncio
    from shitcoin.daemon.server import async_run_daemon
