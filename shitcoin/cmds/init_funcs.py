...
from shitcoin.ssl.create_ssl import (
    ensure_ssl_dirs,
    generate_ca_signed_cert,
    get_shitcoin_ca_crt_key,
    make_ca_cert,
    write_ssl_cert_and_key,
)
from shitcoin.util.bech32m import encode_puzzle_hash
from shitcoin.util.config import (
    create_default_shitcoin_config,
    initial_config_file,
    load_config,
    save_config,
...
from shitcoin.util.path import mkdir
from shitcoin.util.ssl_check import (
...
from shitcoin.wallet.derive_keys import master_sk_to_pool_sk, master_sk_to_wallet_sk
...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    if len(all_sks) == 0:
        print("No keys are present in the keychain. Generate them with 'shitcoin keys generate'")
        return None
...
def create_all_ssl(root_path: Path):
    ...
    shitcoin_ca_crt, shitcoin_ca_key = get_shitcoin_ca_crt_key()
    shitcoin_ca_crt_path = ca_dir / "shitcoin_ca.crt"
    shitcoin_ca_key_path = ca_dir / "shitcoin_ca.key"
    write_ssl_cert_and_key(shitcoin_ca_crt_path, shitcoin_ca_crt, shitcoin_ca_key_path, shitcoin_ca_key)
    ...
    shitcoin_ca_crt, shitcoin_ca_key = get_shitcoin_ca_crt_key()
    generate_ssl_for_nodes(ssl_dir, shitcoin_ca_crt, shitcoin_ca_key, False, overwrite=False)
...
def init(create_certs: Optional[Path], root_path: Path, fix_ssl_permissions: bool = False):
    if create_certs is not None:
        if root_path.exists():
            ...
            if shitcoin_init(root_path, fix_ssl_permissions=fix_ssl_permissions) == 0 and root_path.exists():
                return init(create_certs, root_path, fix_ssl_permissions)

            print(f"** {root_path} was not created. Exiting **")
            return -1
    else:
        return shitcoin_init(root_path, fix_ssl_permissions=fix_ssl_permissions)


def shitcoin_version_number() -> Tuple[str, str, str, str]:
...
def shitcoin_minor_release_number():
    res = int(shitcoin_version_number()[2])
    print(f"Install release number: {res}")
    return res


def shitcoin_full_version_str() -> str:
    major, minor, patch, dev = shitcoin_version_number()
    return f"{major}.{minor}.{patch}{dev}"


def shitcoin_init(root_path: Path, *, should_check_keys: bool = True, fix_ssl_permissions: bool = False):
    """
    ...
    """
    if os.environ.get("shitcoin_ROOT", None) is not None:
        print(
            f"warning, your shitcoin_ROOT is set to {os.environ['shitcoin_ROOT']}. "
            f"Please unset the environment variable and run shitcoin init again\n"
            f"or manually migrate config.yaml"
        )

    print(f"shitcoin directory {root_path}")
    if root_path.is_dir() and Path(root_path / "config" / "config.yaml").exists():
        # This is reached if shitcoin_ROOT is set, or if user has run shitcoin init twice
        # before a new update.
    ...
    create_default_shitcoin_config(root_path)
    create_all_ssl(root_path)
    if fix_ssl_permissions:
        fix_ssl(root_path)
    if should_check_keys:
        check_keys(root_path)
    print("")
    print("To see your keys, run 'shitcoin keys show --show-mnemonic-seed'")
