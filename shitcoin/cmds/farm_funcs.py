...
from shitcoin.cmds.units import units
from shitcoin.consensus.block_record import BlockRecord
from shitcoin.rpc.farmer_rpc_client import FarmerRpcClient
from shitcoin.rpc.full_node_rpc_client import FullNodeRpcClient
from shitcoin.rpc.wallet_rpc_client import WalletRpcClient
from shitcoin.util.config import load_config
...
from shitcoin.util.network import is_localhost
...
async def summary(
    ...
    if amounts is not None:
        print(f"Total shitcoin farmed: {amounts['farmed_amount'] / units['shitcoin']}")
        print(f"User transaction fees: {amounts['fee_amount'] / units['shitcoin']}")
        print(f"Block rewards: {(amounts['farmer_reward_amount'] + amounts['pool_reward_amount']) / units['shitcoin']}")
    ...
    if amounts is None:
        if wallet_not_running:
            print("For details on farmed rewards and fees you should run 'shitcoin start wallet' and 'shitcoin wallet show'")
        elif wallet_not_ready:
            print("For details on farmed rewards and fees you should run 'shitcoin wallet show'")
    else:
        print("Note: log into your key using 'shitcoin wallet show' to see rewards for each key")
