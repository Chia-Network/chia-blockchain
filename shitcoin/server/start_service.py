...
from shitcoin.daemon.server import singleton, service_launch_lock_path
from shitcoin.server.ssl_context import shitcoin_ssl_ca_paths, private_ssl_ca_paths
...
from shitcoin.server.upnp import UPnP
from shitcoin.types.peer_info import PeerInfo
from shitcoin.util.shitcoin_logging import initialize_logging
from shitcoin.util.config import load_config, load_config_cli
...
class Service:
    def __init__(
        ...
        proctitle_name = f"shitcoin_{service_name}"
        ...
        shitcoin_ca_crt, shitcoin_ca_key = shitcoin_ssl_ca_paths(root_path, self.config)
        inbound_rlp = self.config.get("inbound_rate_limit_percent")
        outbound_rlp = self.config.get("outbound_rate_limit_percent")
        assert inbound_rlp and outbound_rlp
        self._server = shitcoinServer(
            advertised_port,
            node,
            peer_api,
            node_type,
            ...
            (shitcoin_ca_crt, shitcoin_ca_key),
    ...
    async def wait_closed(self) -> None:
        ...
        self._log.info("Waiting for shitcoinServer to be closed")
