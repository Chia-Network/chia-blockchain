...
from shitcoin.server.server import NodeType, ssl_context_for_client
from shitcoin.server.ssl_context import private_ssl_ca_paths
...
class RpcClient:
    """
    Client to shitcoin RPC, connects to a local service. Uses HTTP/JSON, and converts back from
    JSON into native python objects before returning. All api calls use POST requests.
    Note that this is not the same as the peer protocol, or wallet protocol (which run shitcoin's
