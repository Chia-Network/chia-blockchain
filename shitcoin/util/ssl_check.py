...
import stat
import sys
from shitcoin.util.config import load_config, traverse_dict
from shitcoin.util.permissions import octal_mode_string, verify_file_permissions
...
def get_all_ssl_file_paths(root_path: Path) -> Tuple[List[Path], List[Path]]:
    ...
    from shitcoin.ssl.create_ssl import get_mozilla_ca_crt
...
def check_ssl(root_path: Path) -> None:
    ...
    if len(invalid_files):
        ...
        print("Run `shitcoin init --fix-ssl-permissions` to fix issues.")
