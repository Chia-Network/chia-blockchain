...
from shitcoin.pools.pool_config import PoolWalletConfig, load_pool_config
...
from shitcoin.util.config import load_config, save_config, config_path_for_filename
...
from shitcoin.wallet.puzzles.singleton_top_layer import SINGLETON_MOD
...
class Farmer:
    ...
    async def setup_keys(self):
        ...
        if len(self.get_public_keys()) == 0:
            error_str = "No keys exist. Please run 'shitcoin keys generate' or open the UI."
    ...
    async def get_cached_harvesters(self, connection: WSshitcoinConnection) -> HarvesterCacheEntry:
