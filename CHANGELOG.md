...
- When plotting, instead of 1 monolithic temp file, there are now 7 files - one for each table. These files are deleted as the `-2` or `-d` final file is written so the final file can fit within the footprint of the temporary files on the same filesystem.
