...
- Configuration directories have been moved to a default location of HomeDirectory/.chives/release/config, plots/ db/, wallet/ etc. This can be overridden by `export CHIA_ROOT=~/.chia` for example which would then put the plots directory in `HomeDirectory/.chives/plots`.
