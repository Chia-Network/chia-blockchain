...
- Portable pooled plots are now available using our new plot NFT. These allow you to plot new plots to an NFT that can either self farm or join and leave pools. During development there were changes to the plot NFT so portable pool plots (those made with `-c` option to `hddcoin plots create`) using code from before June 25th are invalid on mainnet.
OG plots made before this release can continue to be farmed side by side with the new portable pool plots but can not join pools using the official pooling protocol. You can learn more as a farmer by checking out the [pool user guide](https://github.com/HDDcoin-Network/hddcoin-blockchain/wiki/Pooling-User-Guide). Pool operators and those wanting to understand how the official pooling protocol operates should check out our [pooling implementation reference repository](https://github.com/HDDcoin-Network/pool-reference). If you plan to use plot NFT, all your farmers and harvesters must be on 1.2.0 to function properly for portable pool plots.
...
- `hddcoin farm summary` and the GUI now use a new RPC endpoint to properly show plots for local and remote harvesters. This should address issues #6563, #5881, #3875, #1461.
- `hddcoin configure` now supports command line updates to peer count and target peer count.
...
- New CLI command `hddcoin plotnft` to manage pools.
...
- We have increased the strictness of validating HDDcoinlisp in the mempool and clvm.
...
- Correctly display private key in `hddcoin keys show`.
- Thanks to @gldecurtins for removing a default printout of the private key mnemonic in `hddcoin keys show`.
...
- `hddcoin farm summary` better handles wallet errors.
...
- We added a simple profiler to track performance times in the application (see /hddcoin/util/profiler.py for instructions).
...
- Added configuration options for moving an install to testnet and back (use `hddcoin configure -t true|false`).
...
- Secret wallet key is hidden by default in `hddcoin keys show`. Use `hddcoin keys show --show-mnemonic-seed` for private keys.
...
- We changed hddcoinlisp singletons to take a puzzlehash as its origin. We also updated the DID wallet to use this.
...
- HDDcoinlisp programs now remained serialized in Node for better performance.
...
- This is the 1.0 release of HDDcoinlisp. Much has been massaged and finalized. We will be putting a focus on updating and expanding the documentation on [hddcoinlisp.com](https://hddcoinlisp.com) shortly.
...
- The `src` directory in hddcoin-blockchain has been changed to `hddcoin` to avoid namespace collisions.
- GUI install builds have been simplified to rely on one `.spec` file in `hddcoin/`
...
- You should review the [release notes for v1.0.2](https://github.com/HDDcoin-Network/hddcoin-blockchain/releases/tag/1.0.2) but we especially want to point out that wallet sync is much faster than in 1.0.1 and earlier versions.
...
- @jespino changed `hddcoin keys show` to require the `--show-mnemonic-seed` before it displays your 24 work private key mnemonic.
- We decreased the size of the block cache in node to perform better with longer chains.
- You can now add a private key mnemonic from a file with `hddcoin keys show`.
...
- Users must now explicitly set the `--show-mnemonic-seed` flag to see their private keys when running `hddcoin keys show`.
- We are now building Linux GUI installers. These should be considered beta quality for now.
- Translations now available for German, Traditional Chinese, and Danish. Thanks to @Dravenex, @MaestroOnICe, @loudsyncro, @loppefaaret, @thirteenthd, @wong8888, @N418, and @swjz for all the translation help. You to can translate at our [Crowdin project](https://crowdin.com/project/hddcoin-blockchain/).
...
- Users can now pip install e.g. hddcoin-blockchain==1.0.1 on most platforms.
...
- Thank you to @L3Sota for adding a Japanese translation via our [Crowdin project](https://crowdin.com/project/hddcoin-blockchain).
...
- Migration with HDDCOIN_ROOT set does not crash hddcoin init

## 1.0rc8 aka Release Candidate 8 - 2021-03-15

### Added

- This is a hard fork/breaking change from RC6/7. TXCH Coins will **not** be moved forward but your plots and keys and parts of your configuration do. When you install this version before 10AM PDST on 3/16/2021 it will load up, start finding peers, and otherwise wait for the flag drop at that time to start farming. This is likely to be the last dress rehearsal for mainnet launch. Our [3/15/2021 blog post](https://www.hddcoin.org/2021/03/15/mainnet-update.html) has more details on the current mainnet launch plan.
...
- `hddcoin farm summary` will now display TXCH or XCH as appropriate.
...
- `hddcoin farm summary` displayed some incorrect amounts.
...
- `hddcoin netspace` now defaults to 1000 blocks to mirror the GUI.
...
- There was a typo in `hddcoin netspace`. Thanks @altendky.
- There was a typo in `hddcoin plots`. Thanks @adamfiddler.

### Known Issues

- Some users can't plot in the GUI in MacOS Big Sur - especially on M1. See issue [1189](https://github.com/HDDcoin-Network/hddcoin-blockchain/issues/1189)
...
- Generators have been re-worked in CLVM. We added a hddcoinlisp deserialization puzzle and improved the low-level generator. We reduce the accepted atom size to 1MB during HDDcoinLisp native deserialization.
...
- We have moved to a new and much higher scalability download.hddcoin.org to support the mainnet launch flag and additional download demand.
- To always get the latest testnet and then mainnet installers you can now use a latest URL: [Windows](https://download.hddcoin.org/latest/Setup-Win64.exe) and [MacOS x86_64](https://download.hddcoin.org/latest/Setup-MacOS.dmg).
- HDDcoin wheels not on Pypi and some dependecies not found there also are now on pypi.hddcoin.org.
...
- PRs and commits to the hddcoin-blockchain-gui repository will automatically have their locales updated.

## Fixed

- The Farm page will now no longer get stuck at 50 TXCH farmed.
- `hddcoin farm` has had multiple bugs and spelling issues addressed. Thanks to @alfonsoperez, @soulmerge and @olivernyc for your contributions.
- `hddcoin wallet` had various bugs.
...
- We now support a "green flag" chain launch process. A new version of the software will poll download.hddcoin.org/notify/ for a signed json file that will be the genesis block of the chain for that version. This will allow unattended start at mainnet.
...
- Thanks to @jespino there is now `hddcoin farm summary`. You can now get almost exactly the same farming information on the CLI as the GUI.
- We have added Romanian to the GUI translations. Thank you to @bicilis on [Crowdin](https://crowdin.com/project/hddcoin-blockchain). We also added a couple of additional target languages. Klingon anyone?
- `hddcoin wallet` now takes get_address to get a new wallet receive address from the CLI.
- `hddcoin plots check` will list out all the failed plot filenames at the end of the report. Thanks for the PR go to @eFishCent.
- HDDcoinlisp and the clvm have had the standard puzzle updated and we replaced `((c P A))` with `(a P A)`.
...
- Thanks to @xdustinface of Dash, the BlS Signature library has been updated to 0.9 with clean ups and some speed ups. This changed how the G2 infinity element was handled and we now manage it inside of hddcoin-blockchain, etc., instead of in blspy.
...
- Adding Click changed the behavior of `hddcoin keys add -m`. The help now makes it clear that the 24 word mnemonic needs to be surrounded by a pair of quotes.
- Python root CA certificates have issues so we have added the Mozilla certificate store via curl.se and use that to connect to backup.hddcoin.org via https, for example.
- The difficulty adjustment calculation was simplified.
- All of the hddcoin sub repositories that were attempting to build MacOS Universal wheels were only generating x86_64 wheels internally. We have moved back to only generating x86_64 MacOS wheels on CI.
...
- Various typos and style clean ups were made to the Click CLI implementation. `hddcoin -upnp f` was added to disable uPnP.
- `hddcoin plots check` shouldn't crash when encountering plots that cause RuntimeError. PR again thanks to @eFishCent.
...
- RC3 is a new chain to support the last major hddcoinlisp changes. TXCH from the RC1/2 chain do not come forward to this chain but plots and keys continue to work as usual.
...
- A new RPC api: /push_tx. Using this RPC, you can spend custom hddcoinlisp programs. You need to make a SpendBundle, which includes the puzzle reveal (hddcoinlisp), a solution (hddcoinlisp) and a signature.
- You can now use the RPC apis to query the mempool.
- There are now Swedish, Spanish, and Slovak translations. Huge thanks to @ordtrogen (Swedish), @jespino and @dvd101x (Spanish), and our own @seeden (Slovak). Also thanks were due to @f00b4r (Finnish), @A-Caccese (Italian), and @Bibop182 and @LeonidShamis (Russian). Quite a few more are almost complete and ready for inclusion. You can help translate and review translations at our [crowdin project](https://crowdin.com/project/hddcoin-blockchain).
- You can obtain a new wallet receive address on the command line with `hddcoin wallet new_address`. Thanks to @jespino for this and a lot more in the next section below.
- You will now see Your Harvester Network in the GUI even if you have no plots.

### Changed

- All hddcoinlisp opcodes have been renumbered. This should be the last major breaking change for hddcoinlisp and the clvm. There are a couple minor enhancements still needed for mainnet launch, but they may or may not require minor breaking changes. We will be restarting testnet chains on a mostly weekly basis either way.
- Node batch syncing performance was increased, and it now avoids re-validating blocks that node had already validated.
- The entire CLI has been ported to [Click](https://click.palletsprojects.com/en/7.x/). Huge thanks to @jespino for the big assist and @unparalleled-js for the [recommendation and the initial start](https://github.com/HDDcoin-Network/hddcoin-blockchain/issues/464). This will make building out the CLI much easier. There are some subtle changes and some shortcuts are not there anymore. `hddcoin -h` and `hddcoin SUBCOMMAND -h` can be your guide.
...
- `hddcoin plots check` now will continue checking after it finds an error in a plot to the total number of checks you specified.
...
- Interface improvements for `hddcoin netspace`.
- Now that aiosqlite included our upstream improvements we install version 0.17.0.
- `hddcoin init` only migrates release candidate directories. The versioned sub directories under `~/hddcoin` will be going away before mainnet.
...
- `hddcoin configure -log-level` was broken.
...
- This is an errata release for Release Candidate 1. There were a couple of things that did not smoothly migrate from the Beta versions. Please make sure you also consult the [release notes for RC-1](https://github.com/HDDcoin-Network/hddcoin-blockchain/releases/tag/1.0rc1) was well.
...
- We have integrated with crowdin to make it easier to translate the GUI. Check out [HDDcoin Blockchain GUI](https://crowdin.com/project/hddcoin-blockchain) there.
- We have added Italian, Russian, and Finnish. More to come soon.
- There is now remote UI support. [Documents](https://github.com/HDDcoin-Network/hddcoin-blockchain-gui/blob/main/remote.md) will temporarily live in the repository but have moved to the [wiki](https://github.com/HDDcoin-Network/hddcoin-blockchain/wiki/Connecting-the-UI-to-a-remote-daemon). Thanks to @dkackman for this excellent addition!
...
- Added `hddcoin plots check --challenge-start [start]` that begins at a different `[start]` for `-n [challenges]`. Useful when you want to do more detailed checks on plots without restarting from lower challenge values you already have done. Huge thanks to @eFishCent for this and all of the debugging work behind the scenes confirming that plot failures were machine errors and not bugs!
...
- 'hddcoin wallet send' now takes in TXCH or XCH as units instead of mojos.
...
- The GUI is now in a separate submodule repository from hddcoin-blockchain, [hddcoin-blockchain-gui](https://github.com/HDDcoin-Network/hddcoin-blockchain-gui). The installers and install scripts have been updated and it continues to follow the same install steps. Note that the GUI directory will now be `hddcoin-blockchain-gui`. The workflow for this may be "touch and go" for people who use the git install methods over the short term.
...
- The cli wallet, `hddcoin wallet`, was incorrectly displaying TXCH balances as if they were Coloured Coins.
...
- We have made the "Help Translate" button in the GUI open in your default web browser and added instructions for adding new translations and more phrases in existing translations at that [URL](https://github.com/HDDcoin-Network/hddcoin-blockchain/tree/main/electron-react/src/locales). Try the "Help Translate" option on the language selection pull down to the left of the dark/light mode selection at the top right of the GUI.
- Sync store now tracks all connected peers and removes them as they get removed.
- The Rate Limited Wallet has been ported to new consensus and updated HDDcoinlisp methods.
- We are down to only one sub dependency that does not ship binary wheels for all four platforms. The only platform still impacted is ARM64 (generally Raspberry Pi) but that only means that you still need the minor build tools as outlined on the [wiki](https://github.com/HDDcoin-Network/hddcoin-blockchain/wiki/Raspberry-Pi).
...
- Last Attempted Proof is now fixed. This will show you the last time one of your plots passed the [plot filter](https://github.com/HDDcoin-Network/hddcoin-blockchain/wiki/FAQ#what-is-the-plot-filter-and-why-didnt-my-plot-pass-it).
...
- `hddcoin plots check` enforces a minimum of `-n 5` to decrease false negatives. Thanks to @eFishCent for these ongoing pull requests!
...
- We have removed the '-r' flag from `hddcoin` as it was being overridden in most cases by the `-r` for restart flag to `hddcoin start`. Use `hddcoin --root-path` instead.
- `hddcoin -h` now recommends `hddcoin netspace -d 192` which is approximately one hours worth of sub blocks. Use `-d 1000` to get the same estimate of netspace as the RPC and GUI.
- `hddcoin show -c` now displays in MiB and the GUI has been changed to MiB to match.
- `hddcoin configure` now accepts the shorter `-upnp` and `-log-level` arguments also.
- `hddcoin plots check` now defaults to `-n 30` instead of `-n 1` - HT @eFishCent.
- `hddcoin plots create` now enforces a minimum of k=22. As a reminder, anything less than k=32 is just for testing and be careful extrapolating performance of a k less than 30 to a k=32 or larger.
...
- `hddcoin show -c` was showing upload MiB in the download column and vice versa. @pyl and @psydafke deserves credit for insisting it was broken and @kd637xx for the PR assist.
- `hddcoin show` handles sub block 0 better.
...
- `hddcoin plots create -x` skips adding [final dir] to harvester for farming
...
- Removed loading plot file names when starting `hddcoin plots create`; decreases plotter time when there are a lot of plots on the machine. Huge thanks to @eFishCent for this PR!
...
- `hddcoin show -h` now correctly states that you use the first 8 characters of the node id to remove a node on the cli.
- Thank you to @wallentx for adding better help for `hddcoin configure --enable-upnp`.
...
- `hddcoin configure` now lets you enable or disable uPnP.
...
- `hddcoin netspace -s` would not correctly look up the start block height by block hash. Additionally netspace now flips to PiB above 1024 TiB. To compare netspace to `hddcoin show` of the GUI use `hddcoin netspace -d 1000` as `hddcoin netspace` defaults to `-d 192` which is one hour.
...
- There is a lot more information and explanation of the new consensus algorithm in the New Consensus Working Document linked from [hddcoin.org](https://hddcoin.org/). Among the improvements this gives the HDDcoin blockchain are a much higher security level against all attacks, more frequent transaction blocks that have less time variation between them and are then buried under confirmations (sub blocks also count towards re-org security) much more quickly.
...
- RPCs and daemon now communicate over TLS with certificates that are generated into `~/.hddcoin/VERSION/config/`
...
- `hddcoin show -s` now is less brain dead when a node is initially starting to sync.
...
- A segfault caused by memory leaks in bls-library has been fixed. This should end the random farmer and harvester crashes over time as outlined in [Issue 500](https://github.com/HDDcoin-Network/hddcoin-blockchain/issues/500).
...
- FreeBSD and OpenBSD should be able to build and install hddcoin-blockchain and its dependencies again.
...
- In the GUI there was [a regression](https://github.com/HDDcoin-Network/hddcoin-blockchain/issues/484) that removed the scroll bar on the Plot page. The scroll bar has returned!
...
- Logging had [a regression](https://github.com/HDDcoin-Network/hddcoin-blockchain/issues/485) where log level could no longer be set by service.
...
- The GUI now supports translations and localizations. If you'd like to add your language you can see the examples in [the locales directory](https://github.com/HDDcoin-Network/hddcoin-blockchain/tree/dev/electron-react/src/locales) of the hddcoin-blockchain repository.
- `hddcoin check plots` now takes a `-g` option that allows you to specify a matching path string to only check a single plot file, a wild card list of plot files, or all plots in a single directory instead of the default behavior of checking every directory listed in your config.yaml. A big thank you to @eFishCent for this pull request!
...
- The rpc interfaces of all hddcoin services have been refactored, simplified, and had various additional functionality added.
- Block timestamps are now stored in the wallet database. Both database versions were incremented and databases from previous versions will not work with Beta 14. However, upon re-sync all test hddcoin since Beta 12 should appear in your wallet.
...
- Long_description_content_type is now set to improve hddcoin-blockhddcoinn's Pypi entry. Thanks to @altendky for this pull request.
...
- You can now backup your wallet related metadata in an encrypted and signed file to a free service from HDDcoin Network at backup.hddcoin.org. Simply having a backup of your private key will allow you to fully restore the state of your wallet including coloured coins, rate limited wallets, distributed identity wallets and many more. Your private key is used to automatically restore the last backup you saved to the HDDcoin backup cloud service. This service is open source and ultimately you will be able to configure your backups to go to backup.hddcoin.org, your own installation, or a third party's version of it.
- Added a Code of Conduct in CODE_OF_CONDUCT.md.
- Added a bug report template in `.github/ISSUE_TEMPLATE/bug_report.md`.

### Changed

- This is a new blockchain as we changed how the default puzzle hashes are generated and previous coins would not be easy to spend. Plots made with Beta 8 and newer continue to work, but all previous test hddcoin are left on the old chain and do not migrate over. Configuration data like plot directories automatically migrate in your `~/.hddcoin` directory.
...
- `hddcoin init` now automatically discovers previous releases in each new release.

### Fixed

- `hddcoin show -w` should now more reliably work. Wallet balances should be more often correct.
...
- Fixed a typo in `hddcoin show -w` with thanks to @pyl on Keybase.
...
- Ability to run only the farmer, wallet, or timelord services, for more advanced configurations (hddcoin run farmer-only, wallet-only, timelord-only)
...
- `hddcoin show -w` now correctly outputs all wallet types and balances from the local wallet.
...
- `hddcoin show -b` now returns plot public key and pool public key for each block.
...
- See wallet balances in command line: `hddcoin show -w`
- Retry opening invalid plots every 20 minutes (so you can copy a large plot into a plot directory.)
- We've added `hddcoin keys sign` and `hddcoin keys verify` to allow farmers to certify their ownership of keys.
...
- `hddcoin start wallet-server` changed to `hddcoin start wallet`, for consistency.
...
- Removed legacy scripts such as hddcoin-stop-server, hddcoin-restart-harvester, etc.
...
are read.
- Updated [blspy](https://github.com/HDDcoin-Network/bls-signatures) to use the
...
- The hddcoin executable is now available if installing from the Windows or MacOS
...
"hddcoin plots create" and "hddcoin plots check".
- Add plot directories to config.yaml from the cli with "hddcoin plots add".
...
- Test plots for the simulation and testing harness now go into `~/.hddcoin/test-plots/`
...
- We added additional improvements and more RPCs to the start daemon and various services to continue to make hddcoin start/stop reliable cross platform.
...
- Starting hddcoin now waits for the full node to be active before contacting the introducer.
...
- You can now create or import private keys with a 24 word mnemonic, both in the UI and 'hddcoin keys' command line.
...
- We added a `hddcoin keys` command line program, to see, add, and remove private keys.
...
- `hddcoin netspace` has been refactored to use the `/get_network_space` RPC. The command
...
- `hddcoin-check-plots` uses the plot root and checks the plots in the same manner as harvester.
- `hddcoin-check-plots` now does not override plots.yaml, which means concurrent plots will properly be added to plots.yaml.
...
- We have added an option to `hddcoin-create-plots` to specify the second temporary directory. Creating a plot is a three step process. First a working file ending in `.dat.tmp` is created. This file is usually 5 times larger than the final plot file. In the later stages of plotting a second temp file is created ending in `.dat.2.tmp` which will grow to the size of the final plot file. In the final step, the `.dat.2.tmp` is copied to the final `.dat` plot file. You can now optionally set the directory for the `.dat.2.tmp` file with the `-2` flag. An example use case is plotting on a ramdisk and writing both the second temp file and the final file out to an SSD - `hddcoin-create-plots -n 1 -k 30 -t /mnt/ramdisk -2 /mnt/SSD -d /mnt/SSD`.

### Changed

- `hddcoin init` properly migrates from previous versions including the k>=32 workaround. Additionally, the farming target key is checked to make sure that it is the valid and correct public key format.
- We have implemented a workaround for the `hddcoin start` issues some were having upon crash or reboot. We will be rebuilding start and stop to be robust across platforms.
- This release re-includes `hddcoin-start-harvester`.
...
- The user interface now refers to hddcoin coins with their correct currency code of XCH.
...
- Now you can run estimates of the total space currently farming the network. Try `hddcoin netspace -d 12` to run an estimate over the last 12 blocks which is approximately 1 hour.
- We’ve added TLS authentication for incoming farmer connections. TLS certs and keys are generated during hddcoin init and only full nodes with your keys will be able to connect to your Farmer. Also, Harvester, Timelord, and Wallet will now not accept incoming connections which reduces the application attack surface.
- The node RPC has a new endpoint get_header_by_height which allows you to retrieve the block header from a block height. Try `hddcoin show -bh 1000` to see the block header hash of block 1000. You can then look up the block details with `hddcoin show -b f655e1a9f7f8c89a703e40d9ce82ae33508badaf7b37fa1a56cad27926b5e936` which will look up a block by it's header hash.
...
- Most of the content of README.md and INSTALL.md have been moved to the [repository wiki](https://github.com/HDDcoin-Network/hddcoin-blockchain/wiki) and placed in [INSTALL](https://github.com/HDDcoin-Network/hddcoin-blockchain/wiki/INSTALL) and [Quick Start Guide](https://github.com/HDDcoin-Network/hddcoin-blockchain/wiki/Quick-Start-Guide)
...
- Most scripts have been removed in favor of hddcoin action commands. You can run `hddcoin version` or `hddcoin start node` for example. Just running `hddcoin` will show you more options. However `hddcoin-create-plots` continues to use the hyphenated form. Also it's now `hddcoin generate keys` as another example.
- HDDcoin start commands like `hddcoin start farmer` and `hddcoin stop node` now keep track of process IDs in a run/ directory in your configuration directory. `hddcoin stop` is unlikely to work on Windows native for now. If `hddcoin start -r node` doesn't work you can force the run/ directory to be reset with `hddcoin start -f node`.
- We suggest you take a look at our [Upgrading documentation](https://github.com/HDDcoin-Network/hddcoin-blockchain/wiki/Updating-beta-software) if you aren't performing a new install.
...
- The script `hddcoin start wallet-gui` has been chaned to `hddcoin start wallet` which launches but the GUI and server on MacOS and Linux. `hddcoin start wallet-server` remains for WSL 2 and Windows native.
...
- Plots of k>=32 are not working for farming, and some broken plots can cause a memory leak. A [workaround is available](https://github.com/HDDcoin-Network/hddcoin-blockchain/wiki/Beta-1.4-k=32-or-larger-work-around).
...
- Currently, there is no way to restore a Coloured Coin Wallet.
- The `hddcoin stop all` command sometimes fails, use `hddcoin-stop-all` instead. In windows, use the task manager to stop the servers.
...
- There is a new command `hddcoin init` that will migrate files and generate your initial configuration. If you want to use the Wallet or farm, you will also have to `hddcoin-generate-keys`. You can read step by step instructions for [upgrading from a previous beta release](https://github.com/HDDcoin-Network/hddcoin-blockchain/wiki/Updating-beta-software). If you've set `$HDDCOIN_ROOT` you will have to make sure your existing configuration remains compatible manually.
...
- We added full block lookup to `hddcoin show`.

### Changed

- `hddcoin-restart-harvester` has been renamed from `hddcoin-start-harvester` to better reflect its functionality. Use it to restart a harvester that's farming so that it will pick up newly finished plots.
...
- If you previously used the plot_root variable in config, your plot directory names might not migrate correctly. Please double check the filenames in `~/.hddcoin/beta-1.0b3/config/plots.yaml` after migrating
...
- There is now full transaction support on the HDDcoin blockchain. In this initial Beta 1.0 release, all transaction types are supported though the wallets and UIs currently only directly support basic transactions like coinbase rewards and sending coins while paying fees. UI support for our [smart transactions](https://github.com/HDDcoin-Network/wallets/blob/main/README.md) will be available in the UIs shortly.
...
- HDDcoin fullnode, farmer and harvester now default to logging to hddcoin.log in the hddcoin-blockchain directory. This is configured in config.yaml and due to config.yaml changes it is recommended to edit the new template config instead of using older config.yaml’s from previous versions.
...
- Added color logging
- Now hddcoin_harvester will periodically announce which plots it is currently farming and their k sizes.

### Changed

- Moved the ssh UI to use RPC.
- Changed the displayed process names for harvester, farmer, fullnode, timelords, and VDFs to to hddcoin_full node, hddcoin_harvester, etc. Fixed a bug that could cause inadvertent shutdown of other processes like an ongoing plotting session when new hddcoin services were started.
...
- More details on the release at [https://www.hddcoin.org/developer/](https://www.hddcoin.org/developer/)

[unreleased]: https://github.com/HDDcoin-Network/hddcoin-blockchain/compare/1.0beta5...dev
[1.0beta5]: https://github.com/HDDcoin-Network/hddcoin-blockchain/compare/1.0beta4...1.0beta5
[1.0beta4]: https://github.com/HDDcoin-Network/hddcoin-blockchain/compare/1.0beta3...1.0beta4
[1.0beta3]: https://github.com/HDDcoin-Network/hddcoin-blockchain/compare/1.0beta2...1.0beta3
[1.0beta2]: https://github.com/HDDcoin-Network/hddcoin-blockchain/compare/1.0beta1...1.0beta2
[1.0beta1]: https://github.com/HDDcoin-Network/hddcoin-blockchain/compare/alpha-1.5.1...1.0beta1
[alpha 1.5.1]: https://github.com/HDDcoin-Network/hddcoin-blockchain/compare/alpha-1.5...alpha-1.5.1
[alpha 1.5]: https://github.com/HDDcoin-Network/hddcoin-blockchain/compare/alpha-1.4.1...alpha-1.5
[alpha 1.4.1]: https://github.com/HDDcoin-Network/hddcoin-blockchain/compare/alpha-1.4...alpha-1.4.1
[alpha 1.4]: https://github.com/HDDcoin-Network/hddcoin-blockchain/compare/alpha-1.3...alpha-1.4
[alpha 1.3]: https://github.com/HDDcoin-Network/hddcoin-blockchain/compare/alpha-1.2...alpha-1.3
[alpha 1.2]: https://github.com/HDDcoin-Network/hddcoin-blockchain/compare/alpha-1.1.1...alpha-1.2
[alpha 1.1.1]: https://github.com/HDDcoin-Network/hddcoin-blockchain/compare/alpha-1.1...alpha-1.1.1
[alpha 1.1]: https://github.com/HDDcoin-Network/hddcoin-blockchain/compare/alpha-1.0...alpha-1.1
[alpha 1.0]: https://github.com/HDDcoin-Network/hddcoin-blockchain/releases/tag/Alpha-1.0
