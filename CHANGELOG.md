...
Portable pooled plots are now available using our new plot NFT. These allow you to plot new plots to an NFT that can either self farm or join and leave pools. During development there were changes to the plot NFT so portable pool plots (those made with `-c` option to `chia plots create`) using code from before June 25th are likely invalid on mainnet.
...
- The exact commit after which Plot NFTs are guaranteed to be valid is the `89f7a4b3d6329493cd2b4bc5f346a819c99d3e7b` commit (in which `pools.testnet9` branch was merged to main) or `5d62b3d1481c1e225d8354a012727ab263342c0a` within the `pools.testnet9` branch.
...
- We have fixed an error where the wallet could get corrupted, which required restarting the application before.
