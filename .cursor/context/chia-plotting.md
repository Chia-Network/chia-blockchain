...
Read this for plot creation, plot IDs, memo parsing, V1/V2 prover abstraction, plot cache/refresh behavior, plot directory config helpers, and diagnostics such as `chia plots check`. For live signage-point handling, read `chia-harvester.md`; for pool singleton state, read `chia-pools.md`.
