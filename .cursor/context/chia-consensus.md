...
Header validation depends on `ValidationState` (`ssi`, difficulty, previous SES block). `pre_validate_block()` mutates the caller-provided `ValidationState` while queuing batch validation, then passes a copy into the worker. Full node batch sync relies on this: the mutable `ValidationState` advances for subsequent block scheduling, while `add_block()` later receives the preserved state needed for commit.

Difficulty and SSI transitions must flow through a validated sub-epoch summary. `validate_finished_header_block()` rejects raw `new_difficulty` or `new_sub_slot_iters` fields unless the finished sub-slot also carries a valid `subepoch_summary_hash`. `block_to_block_record()` converts that validated hash into `block_rec.sub_epoch_summary_included`, and that computed block-record field is the authoritative source for advancing shared `ValidationState`; do not treat peer-provided `block.finished_sub_slots[0].challenge_chain.new_*` fields as trusted state on their own.
