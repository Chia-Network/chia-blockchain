...
Read this for farmer/full-node/harvester proof flow, pool partial submission, reward-target config, plot-sync receiver state, solver connection management, and farmer RPC. For local plot discovery or signatures, read `chia-harvester.md` and `chia-plotting.md`; for final block acceptance, read `chia-full-node.md` and `chia-consensus.md`.
