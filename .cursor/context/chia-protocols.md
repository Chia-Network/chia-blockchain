...
- Import-time coverage currently matters: every live `ProtocolMessageTypes` entry needs the appropriate sender/reply/no-reply coverage, and reply/no-reply classifications must not overlap. Preserve this as a static invariant.
...
- The old/new harvester signage-point transition is a live compatibility pattern: `NewSignagePointHarvester2` intentionally uses the same message ID as `new_signage_point_harvester`, with selection based on protocol version. Do not assume one message ID maps to exactly one Python payload shape forever. `NewSignagePointHarvester2` also carries `peak_height`, `last_tx_height`, and an optional `filter_challenge`; filter/strength gating is keyed on the transaction-block height (`last_tx_height`), not peak height, and validation-side filter checks must mirror the harvester-side computation.
...
For exact message ids, sender maps, reply/no-reply maps, handshake fields, and capabilities, read the files in the Landmarks table above. For API stub metadata and outbound call shape, read `chia-apis.md`, `chia/server/api_protocol.py`, and the relevant `chia/apis/*_stub.py`. For rate-limit compatibility, read `chia/server/rate_limit_numbers.py` and `chia/server/rate_limits_v3.py`.
