...
Read this for wallet sync, key/derivation state, transaction construction, wallet RPC transaction endpoints, asset wallets, offers, signing, and wallet-owned persistence. For consensus acceptance or full-node mempool policy, read `chia-consensus.md` or `chia-full-node.md`.
...
- `WalletTransactionStore` has primary serialized transaction blobs plus side tables for condition-valid-times and in-memory unconfirmed caches. Confirmation, rollback, and reorg reset paths must keep the serialized tx, valid-time rows, and unconfirmed cache in sync.
