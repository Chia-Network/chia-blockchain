...
- `Err` is a shared negative/positive enum across consensus, protocol, mempool,
  and API errors. Negative values are temporary/non-blacklisting categories;
  positive values include permanent validation and protocol failures. Renumbering
  or reclassifying values changes behavior and wire/user-visible diagnostics.
