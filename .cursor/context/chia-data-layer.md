...
Read this for DataLayer store mutations, root publication, mirror sync, `.dat` file generation/validation, proof verification, DataLayer offers, subscriptions, and DataLayer RPC. For singleton wallet authority and offer signing, also read `chia-wallet.md`; for RPC transport shape, read `chia-rpc.md`.
