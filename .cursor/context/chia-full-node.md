...
Read this for full-node sync, block propagation, mempool coupling, peak processing, wallet/timelord/farmer notifications, full-node RPC reads, and `FullNodeStore` behavior. For pure block validity, read `chia-consensus.md`; for wire schema or connection mechanics, read `chia-protocols.md` or `chia-server.md`.
...
- `/push_tx`/wallet `send_transaction` enqueue into the same transaction pipeline as peer transactions, but trusted peers get high-priority queue treatment and wait on the configured ack path before returning `PENDING`.
