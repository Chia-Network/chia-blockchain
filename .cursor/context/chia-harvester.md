...
Read this for farmer-harvester handshake behavior, local plot refresh, signage-point proof lookup, harvester signatures, plot-sync sender behavior, and harvester RPC/config mutation. For plot creation/cache/prover details, read `chia-plotting.md`; for farmer-side proof handling and pool partials, read `chia-farmer.md`.
...
- The signage-point message has a live compatibility split. Old farmers/harvesters use `NewSignagePointHarvester` with precomputed `filter_prefix_bits`; newer peers use `NewSignagePointHarvester2` with `peak_height`, `last_tx_height`, and an optional `filter_challenge` (used with the signage point index for V2 filter eligibility, derived from completed sub-slot history), letting the harvester compute filter bits per plot version and enforce fork-height rules. V2 gating (skip before `HARD_FORK2_HEIGHT`) and V1 phase-out (`v1_cut_off_height()`) are keyed on `last_tx_height`, not peak height.
...
- V2 partial proof lookup returns only partial proofs that pass the same required-iterations threshold. It must carry plot id, plot index, meta group, strength, plot public key, and pool identifiers because the farmer/solver path needs those fields to build the final `ProofOfSpace`.
- V2 filter eligibility uses `calculate_effective_plot_filter_bits()`: base filter bits plus the plot's strength above the network minimum, capped at a maximum. Plots below `calculate_min_plot_strength()` are skipped locally and rejected by `verify_and_get_quality_string()`; harvester-side and validation-side filter math must stay identical or farmers produce proofs full nodes reject. See `chia/types/blockchain_format/proof_of_space.py` for exact constants.
