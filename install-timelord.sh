...
THE_PATH=$(python -c 'import pkg_resources; print( pkg_resources.get_distribution("shitcoinvdf").location)' 2>/dev/null)/vdf_client
shitcoinVDF_VERSION=$(python -c 'from setup import dependencies; t = [_ for _ in dependencies if _.startswith("shitcoinvdf")][0]; print(t)')
...
if [ -e "$THE_PATH" ]; then
	...
	if [ -e venv/bin/python ] && test $UBUNTU_DEBIAN; then
		echo "Installing shitcoinvdf from source on Ubuntu/Debian"
		...
		echo venv/bin/python -m pip install --force --no-binary shitcoinvdf "$shitcoinVDF_VERSION"
		venv/bin/python -m pip install --force --no-binary shitcoinvdf "$shitcoinVDF_VERSION"
		symlink_vdf_bench "$PYTHON_VERSION"
	elif [ -e venv/bin/python ] && test $RHEL_BASED; then
		echo "Installing shitcoinvdf from source on RedHat/CentOS/Fedora"
		# Install remaining needed development tools - assumes venv and prior run of install.sh
		...
		echo "Installing shitcoinvdf requirement boost for MacOS."
		brew install boost
		echo "Installing shitcoinvdf from source."
