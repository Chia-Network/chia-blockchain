...
class KeyTool(dict):
    ...
    def signature_for_solution(self, coin_solution: CoinSolution, genesis_challenge: bytes32) -> AugSchemeMPL:
        signatures = []
        ...
        for public_key, message_hash in pkm_pairs_for_conditions_dict(
            conditions_dict, coin_solution.coin.name(), genesis_challenge
        ):
