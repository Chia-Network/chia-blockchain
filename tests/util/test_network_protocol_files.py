...
def test_protocol_bytes() -> None:
    ...
    message_bytes, input_bytes = parse_blob(input_bytes)
    message_94 = type(error_without_data).from_bytes(message_bytes)
    assert message_94 == error_without_data
    assert bytes(message_94) == bytes(error_without_data)

    message_bytes, input_bytes = parse_blob(input_bytes)
    message_95 = type(error_with_data).from_bytes(message_bytes)
    assert message_95 == error_with_data
    assert bytes(message_95) == bytes(error_with_data)
