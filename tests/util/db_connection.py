...
class DBConnection:
    def __init__(self, db_version: int, foreign_keys: bool = False) -> None:
    ...
    async def __aenter__(self) -> DBWrapper2:
        ...
        self._db_wrapper = await DBWrapper2.create(
            database=self.db_path, reader_count=4, db_version=self.db_version, foreign_keys=self.foreign_keys
        )
