...
async def setup_farmer(
    ...
    config["xchi_target_address"] = encode_puzzle_hash(b_tools.farmer_ph, "xchi")
    config["pool_public_keys"] = [bytes(pk).hex() for pk in b_tools.pool_pubkeys]
    config["port"] = port
    config_pool["xchi_target_address"] = encode_puzzle_hash(b_tools.pool_ph, "xchi")
