...
from sector.daemon.server import WebSocketServer, create_server_for_daemon, daemon_launch_lock_path, singleton
from sector.full_node.full_node_api import FullNodeAPI
from sector.server.start_farmer import service_kwargs_for_farmer
from sector.server.start_full_node import service_kwargs_for_full_node
from sector.server.start_harvester import service_kwargs_for_harvester
from sector.server.start_introducer import service_kwargs_for_introducer
from sector.server.start_service import Service
from sector.server.start_timelord import service_kwargs_for_timelord
from sector.server.start_wallet import service_kwargs_for_wallet
from sector.simulator.start_simulator import service_kwargs_for_full_node_simulator
from sector.timelord.timelord_launcher import kill_processes, spawn_process
...
async def setup_farmer(
    ...
    config["xsc_target_address"] = encode_puzzle_hash(b_tools.farmer_ph, "xsc")
    config["pool_public_keys"] = [bytes(pk).hex() for pk in b_tools.pool_pubkeys]
    config["port"] = port
    config_pool["xsc_target_address"] = encode_puzzle_hash(b_tools.pool_ph, "xsc")
