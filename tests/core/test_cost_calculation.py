...
from deafwave.consensus.condition_costs import ConditionCost
...
from deafwave.full_node.mempool_check_conditions import get_name_puzzle_conditions, get_puzzle_and_solution_for_coin
...
from deafwave.wallet.puzzles import p2_delegated_puzzle_or_hidden_puzzle
...
def large_block_generator(size):
    ...
    try:
        ...
        #  TODO: Re-enable large-block*.hex but cache in ~/.deafwave/subdir
...
class TestCostCalculation:
    ...
    async def test_basics(self):
        ...
        npc_result: NPCResult = get_name_puzzle_conditions(
            program, test_constants.MAX_BLOCK_COST_CLVM, False)
    ...
    async def test_strict_mode(self):
        ...
        npc_result: NPCResult = get_name_puzzle_conditions(
            generator, test_constants.MAX_BLOCK_COST_CLVM, True)
        assert npc_result.error is not None
        npc_result: NPCResult = get_name_puzzle_conditions(
            generator, test_constants.MAX_BLOCK_COST_CLVM, False)
        assert npc_result.error is None
