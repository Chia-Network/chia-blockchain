...
class TestRpc:
    ...
    async def test1(self, simulation):
        ...
        try:
            farmer_rpc_client = await FarmerRpcClient.create(self_hostname, test_rpc_port, bt.root_path, config)
            harvester_rpc_client = await HarvesterRpcClient.create(self_hostname, test_rpc_port_2, bt.root_path, config)
            ...
            async def have_plots():
                return len((await harvester_rpc_client.get_plots())["plots"]) > 0
            ...
            res_2 = await harvester_rpc_client.get_plots()
            assert len(res_2["plots"]) == num_plots

            assert len(await harvester_rpc_client.get_plot_directories()) == 1

            # import pdb; pdb.set_trace()
            await harvester_rpc_client.add_plot_directory(str(plot_dir))
            await harvester_rpc_client.add_plot_directory(str(plot_dir_sub))

            assert len(await harvester_rpc_client.get_plot_directories()) == 3

            res_2 = await harvester_rpc_client.get_plots()
            assert len(res_2["plots"]) == num_plots + 2

            await harvester_rpc_client.delete_plot(str(plot_dir / filename))
            await harvester_rpc_client.delete_plot(str(plot_dir / filename_2))
            res_3 = await harvester_rpc_client.get_plots()
            assert len(res_3["plots"]) == num_plots

            await harvester_rpc_client.remove_plot_directory(str(plot_dir))
            assert len(await harvester_rpc_client.get_plot_directories()) == 2
