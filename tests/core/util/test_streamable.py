...
def test_basic_optional() -> None:
    assert is_type_SpecificOptional(type(Optional[int]))
    assert is_type_SpecificOptional(type(Optional[Optional[int]]))
    assert not is_type_SpecificOptional(type(List[int]))
