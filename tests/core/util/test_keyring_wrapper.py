...
def test_empty_file_keyring_doesnt_have_master_passphrase(empty_temp_file_keyring: TempKeyring):
    """
...
def test_populated_file_keyring_has_master_passphrase(populated_temp_file_keyring: TempKeyring):
    """
...
def test_default_cached_master_passphrase(empty_temp_file_keyring: TempKeyring):
    """
...
def test_set_cached_master_passphrase(empty_temp_file_keyring: TempKeyring):
    """
...
# When: using a populated file keyring
def test_master_passphrase_is_valid(populated_temp_file_keyring: TempKeyring):
    """
...
def test_set_master_passphrase_on_empty_keyring(empty_temp_file_keyring: TempKeyring):
    """
...
def test_set_master_passphrase_on_keyring(populated_temp_file_keyring: TempKeyring):
    """
    Setting a master passphrase should cache the passphrase and be usable to unlock
    the keyring. Using an old passphrase should not unlock the keyring.
    """
...
def test_remove_master_passphrase_from_empty_keyring(empty_temp_file_keyring: TempKeyring):
    """
...
# When: using a populated keyring
def test_remove_master_passphrase_from_populated_keyring(populated_temp_file_keyring: TempKeyring):
    """
...
# When: using a new empty keyring
def test_get_passphrase(empty_temp_file_keyring: TempKeyring):
    """
...
# When: using a new empty keyring
def test_set_passphrase_overwrite(empty_temp_file_keyring: TempKeyring):
    """
    Overwriting a previously-set passphrase should work
    """
    # When: initially setting the passphrase
    KeyringWrapper.get_shared_instance().set_passphrase("service-xyz", "user-123", "initial passphrase".encode().hex())
    ...
    KeyringWrapper.get_shared_instance().set_passphrase("service-xyz", "user-123", "updated passphrase".encode().hex())
...
# When: using a new empty keyring
def test_delete_passphrase(empty_temp_file_keyring: TempKeyring):
    """
...
def test_emoji_master_passphrase(empty_temp_file_keyring: TempKeyring):
    """
...
def test_japanese_master_passphrase(empty_temp_file_keyring: TempKeyring):
    """
...
# When: using a new empty keyring
def test_set_master_passphrase_with_hint(empty_temp_file_keyring: TempKeyring):
    """
...
def test_passphrase_hint(empty_temp_file_keyring: TempKeyring):
    """
...
def test_passphrase_hint_removal(empty_temp_file_keyring: TempKeyring):
    """
    ...
    # Expect: to retrieve the passphrase hint that was just set
    assert KeyringWrapper.get_shared_instance().get_master_passphrase_hint() == "President Skroob's luggage combination"
...
def test_passphrase_hint_update(empty_temp_file_keyring: TempKeyring):
    """
...
def test_get_label(empty_temp_file_keyring: TempKeyring):
...
def test_set_label(empty_temp_file_keyring: TempKeyring):
    keyring_wrapper = KeyringWrapper.get_shared_instance()
    # Set and validate a label for 1
    keyring_wrapper.set_label(1, "one")
    assert keyring_wrapper.get_label(1) == "one"
...
def test_set_special_labels(label: str, empty_temp_file_keyring: TempKeyring):
    keyring_wrapper = KeyringWrapper.get_shared_instance()
...
def test_set_label_failures(
    label: str, exception: Type[KeychainLabelError], message: str, empty_temp_file_keyring: TempKeyring
) -> None:
    keyring_wrapper = KeyringWrapper.get_shared_instance()
    keyring_wrapper.set_label(1, "one")
...
def test_delete_label(empty_temp_file_keyring: TempKeyring) -> None:
    keyring_wrapper = KeyringWrapper.get_shared_instance()
