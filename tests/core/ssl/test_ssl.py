...
async def establish_connection(server: DeafwaveServer, dummy_port: int, ssl_context) -> bool:
...
class TestSSL:
    ...
    async def test_public_connections(self, wallet_node):
        ...
        server_1: DeafwaveServer = full_node_api.full_node.server
    ...
    async def test_full_node(self, wallet_node):
        ...
        generate_ca_signed_cert(
            full_node_server.deafwave_ca_crt_path.read_bytes(),
            full_node_server.deafwave_ca_key_path.read_bytes(),
            pub_crt,
            pub_key,
        )
        ssl_context = ssl_context_for_client(
            full_node_server.deafwave_ca_crt_path, full_node_server.deafwave_ca_crt_path, pub_crt, pub_key
        )
    ...
    async def test_harvester(self, harvester_farmer):
        ...
        generate_ca_signed_cert(
            harvester_server.deafwave_ca_crt_path.read_bytes(),
            harvester_server.deafwave_ca_key_path.read_bytes(),
            pub_crt,
            pub_key,
        )
        ssl_context = ssl_context_for_client(
            harvester_server.deafwave_ca_crt_path, harvester_server.deafwave_ca_crt_path, pub_crt, pub_key
        )
    ...
    async def test_introducer(self, introducer):
        ...
        # Create not authenticated cert
        pub_crt = introducer_server.deafwave_ca_key_path.parent / "p2p.crt"
        pub_key = introducer_server.deafwave_ca_key_path.parent / "p2p.key"
        generate_ca_signed_cert(
            introducer_server.deafwave_ca_crt_path.read_bytes(),
            introducer_server.deafwave_ca_key_path.read_bytes(),
            pub_crt,
            pub_key,
        )
        ssl_context = ssl_context_for_client(
            introducer_server.deafwave_ca_crt_path, introducer_server.deafwave_ca_crt_path, pub_crt, pub_key
        )
    ...
    async def test_timelord(self, timelord):
        ...
        generate_ca_signed_cert(
            timelord_server.deafwave_ca_crt_path.read_bytes(),
            timelord_server.deafwave_ca_key_path.read_bytes(),
            pub_crt,
            pub_key,
        )
        ssl_context = ssl_context_for_client(
            timelord_server.deafwave_ca_crt_path, timelord_server.deafwave_ca_crt_path, pub_crt, pub_key
        )
