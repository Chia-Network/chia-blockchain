...
@pytest.mark.asyncio
async def test_height_to_hash_long_chain(tmp_dir: Path, db_version: int) -> None:
    async with DBConnection(db_version) as db_wrapper:
        await setup_db(db_wrapper)
...
@pytest.mark.asyncio
async def test_restore_extend(tmp_dir: Path, db_version: int) -> None:
...
@pytest.mark.asyncio
async def test_height_to_hash_with_orphans(tmp_dir: Path, db_version: int) -> None:
    async with DBConnection(db_version) as db_wrapper:
        await setup_db(db_wrapper)
        await setup_chain(db_wrapper, 10)
...
@pytest.mark.asyncio
async def test_height_to_hash_update(tmp_dir: Path, db_version: int) -> None:
    async with DBConnection(db_version) as db_wrapper:
        await setup_db(db_wrapper)
        await setup_chain(db_wrapper, 10)
...
@pytest.mark.asyncio
async def test_update_ses(tmp_dir: Path, db_version: int) -> None:
    async with DBConnection(db_version) as db_wrapper:
        await setup_db(db_wrapper)
        await setup_chain(db_wrapper, 10)
...
@pytest.mark.asyncio
async def test_height_to_ses(tmp_dir: Path, db_version: int) -> None:
    async with DBConnection(db_version) as db_wrapper:
        await setup_db(db_wrapper)
...
@pytest.mark.asyncio
async def test_rollback(tmp_dir: Path, db_version: int) -> None:
    async with DBConnection(db_version) as db_wrapper:
        await setup_db(db_wrapper)
        await setup_chain(db_wrapper, 10, ses_every=2)
...
@pytest.mark.asyncio
async def test_rollback2(tmp_dir: Path, db_version: int) -> None:
    async with DBConnection(db_version) as db_wrapper:
        await setup_db(db_wrapper)
        await setup_chain(db_wrapper, 10, ses_every=2)
...
@pytest.mark.asyncio
async def test_cache_file_nothing_to_write(tmp_dir: Path, db_version: int) -> None:
...
@pytest.mark.asyncio
async def test_cache_file_replace_everything(tmp_dir: Path, db_version: int) -> None:
...
@pytest.mark.asyncio
async def test_cache_file_extend(tmp_dir: Path, db_version: int) -> None:
    # Test the case where the cache has fewer blocks than the DB, and that
    # we correctly load all the missing blocks from the DB to update the
    # cache
    async with DBConnection(db_version) as db_wrapper:
