...
def test_has_ph_sub() -> None:
    ...
    assert sub.has_puzzle_hash_subscription(ph1) is False
    assert sub.has_puzzle_hash_subscription(ph2) is False

    sub.add_puzzle_hash_subscriptions(peer1, [ph1], 100)

    assert sub.has_puzzle_hash_subscription(ph1) is True
    assert sub.has_puzzle_hash_subscription(ph2) is False

    sub.add_puzzle_hash_subscriptions(peer1, [ph1, ph2], 100)

    assert sub.has_puzzle_hash_subscription(ph1) is True
    assert sub.has_puzzle_hash_subscription(ph2) is True

    # note that this is technically a type error as well.
    # we can remove these asserts once we have type checking
    assert sub.has_coin_id_subscription(coin1) is False
    assert sub.has_coin_id_subscription(coin2) is False
...
def test_has_coin_sub() -> None:
    ...
    sub.add_coin_id_subscriptions(peer1, [coin1], 100)

    assert sub.has_coin_id_subscription(coin1) is True
    assert sub.has_coin_id_subscription(coin2) is False

    sub.add_coin_id_subscriptions(peer1, [coin1, coin2], 100)

    assert sub.has_coin_id_subscription(coin1) is True
    assert sub.has_coin_id_subscription(coin2) is True

    # note that this is technically a type error as well.
    # we can remove these asserts once we have type checking
    assert sub.has_puzzle_hash_subscription(coin1) is False
    assert sub.has_puzzle_hash_subscription(coin2) is False
...
def test_overlapping_coin_subscriptions() -> None:
    ...
    sub.add_coin_id_subscriptions(peer2, [coin2], 100)
    ...
    sub.add_coin_id_subscriptions(peer1, [coin2], 100)
...
def test_overlapping_ph_subscriptions() -> None:
    ...
    sub.add_puzzle_hash_subscriptions(peer2, [ph2], 100)
    ...
    sub.add_puzzle_hash_subscriptions(peer1, [ph2], 100)
...
def test_ph_sub_limit() -> None:
    ...
    assert sub.has_puzzle_hash_subscription(ph2) is False
    assert sub.has_puzzle_hash_subscription(ph2) is False
    assert sub.has_puzzle_hash_subscription(ph3) is False

    sub.add_puzzle_hash_subscriptions(peer1, [ph1, ph2, ph3, ph4], 3)

    assert sub.has_puzzle_hash_subscription(ph1) is True
    assert sub.has_puzzle_hash_subscription(ph2) is True
    assert sub.has_puzzle_hash_subscription(ph3) is True
    assert sub.has_puzzle_hash_subscription(ph4) is False
    ...
    sub.add_puzzle_hash_subscriptions(peer1, [ph4], 3)

    assert sub.has_puzzle_hash_subscription(ph4) is False
    assert sub.peers_for_puzzle_hash(ph4) == set()

    # peer1 is also limied on coin subscriptions
    sub.add_puzzle_hash_subscriptions(peer1, [coin1], 3)

    assert sub.has_coin_id_subscription(coin1) is False
    assert sub.peers_for_coin_id(coin1) == set()

    # peer2 is has its own limit
    sub.add_puzzle_hash_subscriptions(peer2, [ph4], 3)

    assert sub.has_puzzle_hash_subscription(ph4) is True
...
def test_ph_sub_limit_incremental() -> None:
    ...
    sub.add_puzzle_hash_subscriptions(peer1, [ph1], 2)
    ...
    sub.add_puzzle_hash_subscriptions(peer1, [ph2, ph3], 2)
...
def test_coin_sub_limit() -> None:
    ...
    assert sub.has_coin_id_subscription(coin2) is False
    assert sub.has_coin_id_subscription(coin2) is False
    assert sub.has_coin_id_subscription(coin3) is False

    sub.add_coin_id_subscriptions(peer1, [coin1, coin2, coin3, coin4], 3)

    assert sub.has_coin_id_subscription(coin1) is True
    assert sub.has_coin_id_subscription(coin2) is True
    assert sub.has_coin_id_subscription(coin3) is True
    assert sub.has_coin_id_subscription(coin4) is False
    ...
    # peer1 should still be limited
    sub.add_coin_id_subscriptions(peer1, [coin4], 3)

    assert sub.has_coin_id_subscription(coin4) is False
    assert sub.peers_for_coin_id(coin4) == set()

    # peer1 is also limied on ph subscriptions
    sub.add_puzzle_hash_subscriptions(peer1, [ph1], 3)

    assert sub.has_puzzle_hash_subscription(ph1) is False
    assert sub.peers_for_puzzle_hash(ph1) == set()

    # peer2 is has its own limit
    sub.add_coin_id_subscriptions(peer2, [coin4], 3)

    assert sub.has_coin_id_subscription(coin4) is True
...
def test_coin_sub_limit_incremental() -> None:
    ...
    sub.add_coin_id_subscriptions(peer1, [coin1], 2)
    ...
    sub.add_coin_id_subscriptions(peer1, [coin2, coin3], 2)
