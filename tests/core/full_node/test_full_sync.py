...
class TestFullSync:
    ...
    async def test_long_sync_from_zero(self, five_nodes, default_400_blocks):
        ...
        await time_out_assert(
            120, node_height_exactly, True, full_node_2, test_constants.WEIGHT_PROOF_RECENT_BLOCKS + 5 - 1
        )
        await time_out_assert(
            120, node_height_exactly, True, full_node_3, test_constants.WEIGHT_PROOF_RECENT_BLOCKS + 5 - 1
        )
        ...
        for block in blocks_node_5:
            await full_node_5.full_node.respond_sub_block(full_node_protocol.RespondSubBlock(block))
