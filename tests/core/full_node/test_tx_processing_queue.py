...
class TestTransactionQueue:
    ...
    @pytest.mark.asyncio
    async def test_one_peer_and_await(self, transaction_queue: TransactionQueue) -> None:
        num_txs = 100
    ...
    @pytest.mark.asyncio
    async def test_lots_of_peers(self, transaction_queue: TransactionQueue) -> None:
        num_peers = 1000
        num_txs = 100
    ...
    @pytest.mark.asyncio
    async def test_full_queue(self, transaction_queue: TransactionQueue) -> None:
        num_peers = 100
        num_txs = 1000
        total_txs = num_txs * num_peers
        peer_ids: List[bytes32] = [get_peer_id() for _ in range(num_peers)]
    ...
    @pytest.mark.asyncio
    async def test_queue_cleanup_and_fairness(self, transaction_queue: TransactionQueue) -> None:
        num_peers = 1000
        num_txs = 100
        total_txs = num_txs * num_peers
        peer_ids: List[bytes32] = [get_peer_id() for _ in range(num_peers)]
