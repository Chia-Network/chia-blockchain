...
class TestCoinStoreWithBlocks:
    ...
    async def test_basic_coin_store(
        self, db_version: int, softfork_height: uint32, bt: BlockTools, consensus_mode: Mode, tmp_path: Path
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_set_spent(self, db_version: int, bt: BlockTools, consensus_mode: Mode, tmp_path: Path) -> None:
        if consensus_mode != Mode.PLAIN:
            pytest.skip("only run in PLAIN mode to save time")
    ...
    @pytest.mark.asyncio
    async def test_num_unspent(self, bt: BlockTools, db_version: int, consensus_mode: Mode, tmp_path: Path) -> None:
        if consensus_mode != Mode.PLAIN:
            pytest.skip("only run in PLAIN mode to save time")
    ...
    @pytest.mark.asyncio
    async def test_rollback(self, db_version: int, bt: BlockTools, consensus_mode: Mode, tmp_path: Path) -> None:
        if consensus_mode != Mode.PLAIN:
            pytest.skip("only run in PLAIN mode to save time")
    ...
    @pytest.mark.asyncio
    async def test_basic_reorg(self, tmp_dir: Path, db_version: int, bt: BlockTools, tmp_path: Path) -> None:
        async with DBConnection(db_version=db_version, tmp_path=tmp_path) as db_wrapper:
    ...
    @pytest.mark.asyncio
    async def test_get_puzzle_hash(
        self,
        tmp_dir: Path,
        db_version: int,
        bt: BlockTools,
    ...
    @pytest.mark.asyncio
    async def test_get_coin_states(self, db_version: int, consensus_mode: Mode, tmp_path: Path) -> None:
        if consensus_mode != Mode.PLAIN:
            pytest.skip("only run in PLAIN mode to save time")
