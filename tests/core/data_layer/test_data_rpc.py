...
async def test_create_insert_get(one_wallet_node_and_rpc: nodes) -> None:
    ...
    async for data_layer in init_data_layer(root_path):
        ...
        await time_out_assert(
            15, is_transaction_confirmed, True, "this is unused", wallet_rpc_api, update_tx_rec0
        )
        ...
        for i in range(0, num_blocks):
            ...
            await full_node_api.farm_new_transaction_block(FarmNewBlockProtocol(ph))
        await time_out_assert(
            15, is_transaction_confirmed, True, "this is unused", wallet_rpc_api, update_tx_rec1
        )
        with pytest.raises(Exception):
            val = await data_rpc_api.get_value({"id": store_id.hex(), "key": key.hex()})


# todo uncomment after ssl issue is resolved
# @pytest.mark.asyncio
# async def test_create_double_insert(chia_root: ChiaRoot, one_wallet_node_and_rpc: nodes) -> None:
#     root = chia_root.path
