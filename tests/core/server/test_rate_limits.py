...
class TestRateLimits:
    ...
    async def test_too_many_messages(self):
        ...
        for i in range(3000):
            assert r.message_received(new_tx_message)

        saw_disconnect = False
        for i in range(3000):
            response = r.message_received(new_tx_message)
            if not response:
                saw_disconnect = True
        ...
        for i in range(20):
            assert r.message_received(new_peak_message)

        saw_disconnect = False
        for i in range(200):
            response = r.message_received(new_peak_message)
            if not response:
                saw_disconnect = True
        assert saw_disconnect
    ...
    async def test_large_message(self):
        ...
        r = RateLimiter()
        assert r.message_received(small_tx_message)
        assert r.message_received(small_tx_message)
        assert not r.message_received(large_tx_message)
        ...
        r = RateLimiter()
        assert r.message_received(small_vdf_message)
        assert r.message_received(small_vdf_message)
        assert not r.message_received(large_vdf_message)
    ...
    async def test_too_much_data(self):
        ...
        for i in range(10):
            assert r.message_received(tx_message)

        saw_disconnect = False
        for i in range(300):
            response = r.message_received(tx_message)
            if not response:
                saw_disconnect = True
        assert saw_disconnect
        ...
        for i in range(10):
            assert r.message_received(block_message)

        saw_disconnect = False
        for i in range(40):
            response = r.message_received(block_message)
            if not response:
                saw_disconnect = True
        assert saw_disconnect
    ...
    async def test_non_tx_aggregate_limits(self):
        ...
        for i in range(450):
            assert r.message_received(message_1)
        for i in range(450):
            assert r.message_received(message_2)

        saw_disconnect = False
        for i in range(450):
            response = r.message_received(message_3)
            if not response:
                saw_disconnect = True
        assert saw_disconnect
        ...
        for i in range(2):
            assert r.message_received(message_4)

        saw_disconnect = False
        for i in range(2):
            response = r.message_received(message_5)
            if not response:
                saw_disconnect = True
        assert saw_disconnect
    ...
    async def test_periodic_reset(self):
        ...
        for i in range(300):
            ...
            if not response:
                saw_disconnect = True
        assert saw_disconnect
        assert not r.message_received(tx_message)
        ...
        for i in range(3000):
            ...
            if not response:
                saw_disconnect = True
        assert saw_disconnect
        assert not r.message_received(new_tx_message)
        await asyncio.sleep(6)
        assert r.message_received(new_tx_message)
