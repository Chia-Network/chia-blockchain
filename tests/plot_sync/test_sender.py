...
from chia.protocols.plot_sync_protocol import Identifier, Response
...
def test_set_response() -> None:
    ...
    def new_response_message(sync_id: int, message_id: int, message_type: ProtocolMessageTypes) -> Response:
        return Response(plot_sync_identifier(uint64(sync_id), uint64(message_id)), int16(int(message_type.value)), None)
    ...
    expired_message = Response(expired_identifier, int16(int(ProtocolMessageTypes.plot_sync_start.value)), None)
