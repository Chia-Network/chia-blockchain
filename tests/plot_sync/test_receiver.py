...
from chia.protocols.plot_sync_protocol import Done, Identifier, PathList, Plot, PlotList, Response, Start
...
def plot_sync_identifier(current_sync_id: uint64, message_id: uint64) -> Identifier:
    return Identifier(uint64(0), current_sync_id, message_id)
...
def assert_error_response(plot_sync: Receiver, error_code: ErrorCodes) -> None:
    ...
    response: Response = Response.from_bytes(message.data)
...
async def run_sync_step(receiver: Receiver, sync_step: SyncStepData, expected_state: State) -> None:
    ...
    if sync_step.payload_type == PlotList or sync_step.payload_type == PathList:
...
def plot_sync_setup():
    ...
    sync_steps: List[SyncStepData] = [
        SyncStepData(State.idle, receiver.sync_started, Start, False, uint64(0), uint32(len(plot_info_list))),
        SyncStepData(State.loaded, receiver.process_loaded, PlotList, plot_info_list[10:20], True),
        SyncStepData(State.removed, receiver.process_removed, PathList, path_list[0:10], True),
        SyncStepData(State.invalid, receiver.process_invalid, PathList, path_list[20:30], True),
        SyncStepData(State.keys_missing, receiver.process_keys_missing, PathList, path_list[30:40], True),
        SyncStepData(State.done, receiver.sync_done, Done, uint64(0)),
    ]
...
async def test_invalid_ids() -> None:
    ...
    for state in State:
        ...
        if receiver.state() == State.idle:
            ...
            invalid_last_sync_id_param = Start(plot_sync_identifier(uint64(0), uint64(0)), False, uint64(2), uint32(0))
            ...
            invalid_sync_id_match_param = Start(plot_sync_identifier(uint64(1), uint64(0)), False, uint64(1), uint32(0))
