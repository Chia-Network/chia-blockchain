...
from chia.protocols.plot_sync_protocol import Error, Response
    ...
    async def run(
        ...
        *,
        ...
        await self.test_data[index].run(
            loaded=loaded, removed=removed, invalid=invalid, keys_missing=keys_missing, initial=initial
        )
...
async def skip_processing(self, _, message_type, message) -> bool:
    ...
    if self.simulate_error == 5 and self.message_counter > 1:  # Non recoverable error
        await self.connection().send_message(
            make_msg(
                ProtocolMessageTypes.plot_sync_response,
                Response(message.identifier, int16(message_type.value), Error(int16(0), "non recoverable", None)),
            )
        )
...
async def test_sync_simulated(farmer_multi_harvester, event_loop) -> None:
    ...
    await test_runner.run(0, loaded=plots[0:10000], removed=[], invalid=[], keys_missing=[], initial=True)
    await test_runner.run(
        1, loaded=plots[10000:20000], removed=[], invalid=plots[30000:30100], keys_missing=[], initial=True
    )
    await test_runner.run(
        2, loaded=plots[20000:30000], removed=[], invalid=[], keys_missing=plots[30100:30200], initial=True
    )
    await test_runner.run(
        0, loaded=[], removed=[], invalid=plots[30300:30400], keys_missing=plots[30400:30453], initial=False
    )
    await test_runner.run(0, loaded=[], removed=[], invalid=[], keys_missing=[], initial=False)
    await test_runner.run(0, loaded=[], removed=plots[5000:10000], invalid=[], keys_missing=[], initial=False)
    await test_runner.run(1, loaded=[], removed=plots[10000:20000], invalid=[], keys_missing=[], initial=False)
    await test_runner.run(2, loaded=[], removed=plots[20000:29000], invalid=[], keys_missing=[], initial=False)
    await test_runner.run(0, loaded=[], removed=plots[0:5000], invalid=[], keys_missing=[], initial=False)
    await test_runner.run(
        2,
    ...
    await test_runner.run(2, loaded=[], removed=plots[5000:10000], invalid=[], keys_missing=[], initial=False)
