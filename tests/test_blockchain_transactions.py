...
class TestBlockchainTransactions:
    ...
    async def test_basic_blockchain_tx(self, two_nodes):
        ...
        next_spendbundle = await full_node_1.mempool.create_bundle_for_tip(last_block.header_block)
        ...
        for coin in added_coins:
            unspent = await full_node_1.unspent_store.get_unspent(coin.name(), next_block.header_block)
            assert unspent is not None

        full_tips = await full_node_1.blockchain.get_full_tips()
        in_full_tips = False
        farmed_block: FullBlock = None
