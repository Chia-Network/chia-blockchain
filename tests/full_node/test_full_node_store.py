...
class TestFullNodeStore:
    ...
    async def test_basic_store(self, empty_blockchain):
        ...
        for block in blocks:
            ...
            sb = blockchain.__sub_blocks[block.header_hash]
            sp_sub_slot, ip_sub_slot = await blockchain.get_sp_and_ip_sub_slots(block.header_hash)
            res = store.new_peak(sb, sp_sub_slot, ip_sub_slot, False, blockchain.__sub_blocks)
        ...
        for block in blocks_reorg:
            ...
            if res == ReceiveBlockResult.NEW_PEAK:
                sb = blockchain.__sub_blocks[block.header_hash]
                sp_sub_slot, ip_sub_slot = await blockchain.get_sp_and_ip_sub_slots(block.header_hash)
                res = store.new_peak(sb, sp_sub_slot, ip_sub_slot, True, blockchain.__sub_blocks)
                assert res[0] is None
        ...
        for slot in blocks_2[-1].finished_sub_slots:
            store.new_finished_sub_slot(slot, blockchain.__sub_blocks, blockchain.get_peak())
        ...
        for i in range(1, test_constants.NUM_SPS_SUB_SLOT - test_constants.NUM_SP_INTERVALS_EXTRA):
            ...
            assert store.new_signage_point(i, blockchain.__sub_blocks, peak, peak.sub_slot_iters, sp)
        ...
        while True:
            ...
            if res == ReceiveBlockResult.NEW_PEAK:
                sb = blockchain.__sub_blocks[blocks[-1].header_hash]
        ...
        for i in range(1, test_constants.NUM_SPS_SUB_SLOT - test_constants.NUM_SP_INTERVALS_EXTRA):
            ...
            assert not store.new_signage_point(i, blockchain.__sub_blocks, peak, peak.sub_slot_iters, sp)
        ...
        assert not store.new_signage_point(
            ...
            {},
            peak,
            blockchain.__sub_blocks[blocks[1].header_hash].sp_sub_slot_total_iters(test_constants),
            sp,
        )
        ...
        sb = blockchain.__sub_blocks[blocks_4[-1].header_hash]
        store.new_peak(sb, None, None, False, blockchain.__sub_blocks)
        for i in range(
            ...
            assert store.new_signage_point(i, empty_blockchain.__sub_blocks, sb, peak.sub_slot_iters, sp)
        ...
        assert store.new_finished_sub_slot(dependant_sub_slots[0], blockchain.__sub_blocks, peak) is None
        block = blocks[-2]
        sb = blockchain.__sub_blocks[block.header_hash]
        sp_sub_slot, ip_sub_slot = await blockchain.get_sp_and_ip_sub_slots(block.header_hash)
        res = store.new_peak(sb, sp_sub_slot, ip_sub_slot, False, blockchain.__sub_blocks)
        ...
        for i in range(5, len(blocks) - 1):
            ...
            sb = blockchain.__sub_blocks[prev_block.header_hash]
            res = store.new_peak(sb, sp_sub_slot, ip_sub_slot, False, blockchain.__sub_blocks)
            if len(block.finished_sub_slots) == 0:
                ...
                for ss in block.finished_sub_slots:
                    found_ips += store.new_finished_sub_slot(ss, blockchain.__sub_blocks, sb)
