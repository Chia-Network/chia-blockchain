...
from shitcoin.cmds.init_funcs import create_all_ssl, create_default_shitcoin_config
from shitcoin.daemon.keychain_proxy import connect_to_keychain_and_validate, wrap_local_keychain
from shitcoin.full_node.bundle_tools import (
    best_solution_generator_from_template,
    detect_potential_template_generator,
    simple_solution_generator,
)
from shitcoin.util.errors import Err
from shitcoin.full_node.generator import setup_generator_args
from shitcoin.full_node.mempool_check_conditions import GENERATOR_MOD
from shitcoin.plotting.create_plots import create_plots, PlotKeys
...
from shitcoin.consensus.coinbase import create_puzzlehash_for_pk, create_farmer_coin, create_pool_coin
...
from shitcoin.plotting.util import PlotsRefreshParameter, PlotRefreshResult, PlotRefreshEvents, parse_plot_info
from shitcoin.plotting.manager import PlotManager
from shitcoin.server.server import ssl_context_for_server
...
from shitcoin.types.blockchain_format.program import INFINITE_COST
...
from shitcoin.util.ints import uint8, uint16, uint32, uint64, uint128
from shitcoin.util.keychain import Keychain, bytes_to_mnemonic
...
from shitcoin.wallet.derive_keys import (
    ...
    async def setup_keys(self):
        if self.local_keychain:
            ...
            self.keychain_proxy = await connect_to_keychain_and_validate(
                self.root_path, log, user="testing-1.8.0", service="shitcoin-testing-1.8.0"
            )
        ...
        if len(self.pool_pubkeys) == 0 or len(self.farmer_pubkeys) == 0:
            raise RuntimeError("Keys not generated. Run `shitcoin generate keys`")
...
def get_plot_dir() -> Path:
    cache_path = Path(os.path.expanduser(os.getenv("shitcoin_ROOT", "~/.shitcoin/"))) / "test-plots"
    mkdir(cache_path)
    return cache_path
