...
def test_notifications_send(capsys: object, get_test_cli_clients: Tuple[TestRpcClients, Path]) -> None:
    ...
    class NotificationsSendRpcClient(TestWalletRpcClient):
        async def send_notification(
            ...
            return cast(TransactionRecord, FakeTransactionRecord(bytes32([2] * 32).hex()))

    inst_rpc_client = NotificationsSendRpcClient()  # pylint: disable=no-value-for-parameter
    test_rpc_clients.wallet_rpc_client = inst_rpc_client
    target_ph = bytes32([1] * 32)
...
def test_notifications_get(capsys: object, get_test_cli_clients: Tuple[TestRpcClients, Path]) -> None:
    ...
    class NotificationsGetRpcClient(TestWalletRpcClient):
        async def get_notifications(
            ...
            return [Notification(bytes32([1] * 32), bytes("hello", "utf8"), uint64(1000000000), uint32(50))]
    ...
    run_cli_command_and_assert(capsys, root_dir, command_args, assert_list)
    expected_calls: logType = {"get_notifications": [([bytes32([1] * 32)], (10, 10))]}
    test_rpc_clients.wallet_rpc_client.check_log(expected_calls)
...
def test_vcs_mint(capsys: object, get_test_cli_clients: Tuple[TestRpcClients, Path]) -> None:
    ...
    test_rpc_clients.wallet_rpc_client = inst_rpc_client
    did_bytes = bytes32([1] * 32)
    did_id = encode_puzzle_hash(did_bytes, "did:chia:")
    target_bytes = bytes32([2] * 32)
    ...
    # these are various things that should be in the output
    assert_list = [
        f"New VC with launcher ID minted: {bytes32([3] * 32).hex()}",
        f"Transaction {bytes32([2] * 32).hex()}",
    ]
    run_cli_command_and_assert(capsys, root_dir, command_args, assert_list)
...
def test_vcs_get(capsys: object, get_test_cli_clients: Tuple[TestRpcClients, Path]) -> None:
    ...
    class VcsGetRpcClient(TestWalletRpcClient):
        async def vc_get_list(self, start: int = 0, count: int = 50) -> Tuple[List[VCRecord], Dict[str, Any]]:
            class FakeVC:
                def __init__(self) -> None:
                    self.launcher_id = bytes32([3] * 32)
                    self.coin = Coin(bytes32([1] * 32), bytes32([2] * 32), uint64(12345678))
                    self.inner_puzzle_hash = bytes32([3] * 32)
                    self.proof_hash = bytes32([4] * 32)
            ...
            proofs = {bytes32([1] * 32).hex(): ["proof here"]}
    ...
    assert_list = [
        f"Proofs:\n- {bytes32([1] * 32).hex()}\n  - proof here",
        f"Launcher ID: {bytes32([3] * 32).hex()}",
        f"Inner Address: {encode_puzzle_hash(bytes32([3] * 32), 'xch')}",
    ]
    run_cli_command_and_assert(capsys, root_dir, command_args, assert_list)
...
def test_vcs_update_proofs(capsys: object, get_test_cli_clients: Tuple[TestRpcClients, Path]) -> None:
    ...
    test_rpc_clients.wallet_rpc_client = inst_rpc_client
    vc_bytes = bytes32([1] * 32)
    target_ph = bytes32([2] * 32)
    new_proof = bytes32([3] * 32)
    command_args = [
        "wallet",
        "vcs",
        "update_proofs",
...
def test_vcs_revoke(capsys: object, get_test_cli_clients: Tuple[TestRpcClients, Path]) -> None:
    ...
    # these are various things that should be in the output
    assert_list = ["VC successfully revoked!", f"Transaction {bytes32([2] * 32).hex()}"]
...
def test_vcs_approve_r_cats(capsys: object, get_test_cli_clients: Tuple[TestRpcClients, Path]) -> None:
    ...
    # these are various things that should be in the output
    assert_list = ["VC successfully approved R-CATs!", f"Transaction {bytes32([2] * 32).hex()}"]
    run_cli_command_and_assert(capsys, root_dir, command_args, assert_list)
    expected_calls: logType = {
        "crcat_approve_pending": [
