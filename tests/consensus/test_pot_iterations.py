...
class TestPotIterations:
    ...
    def test_is_overflow_sub_block(self):
        ...
        assert not is_overflow_sub_block(test_constants, ips, uint64(0))

    def test_calculate_icp_iters(self):
        ips: uint64 = uint64(100001)
        with raises(ValueError):
            calculate_icp_iters(test_constants, ips, uint64(test_constants.SLOT_TIME_TARGET * ips + 1))
        with raises(ValueError):
            calculate_icp_iters(test_constants, ips, uint64(test_constants.SLOT_TIME_TARGET * ips))
        one_checkpoint_iters = test_constants.SLOT_TIME_TARGET * ips // 32
        assert (
            calculate_icp_iters(test_constants, ips, uint64(test_constants.SLOT_TIME_TARGET * ips - 1))
            == 31 * one_checkpoint_iters
        )
        assert calculate_icp_iters(test_constants, ips, uint64(20 * one_checkpoint_iters)) == 20 * one_checkpoint_iters
        assert (
            calculate_icp_iters(test_constants, ips, uint64(20 * one_checkpoint_iters) - 1) == 19 * one_checkpoint_iters
        )
        assert (
            calculate_icp_iters(test_constants, ips, uint64(20 * one_checkpoint_iters) + 1) == 20 * one_checkpoint_iters
        )
        assert calculate_icp_iters(test_constants, ips, uint64(1)) == 0 * one_checkpoint_iters
        assert calculate_icp_iters(test_constants, ips, uint64(0)) == 0 * one_checkpoint_iters
    ...
    def test_calculate_ip_iters(self):
        ...
        assert (
            calculate_ip_iters(test_constants, ips, uint64(5 * one_checkpoint_iters))
        ...
        assert calculate_ip_iters(test_constants, ips, uint64(0)) == extra_iters
        assert calculate_ip_iters(test_constants, ips, uint64(1)) == extra_iters + 1
