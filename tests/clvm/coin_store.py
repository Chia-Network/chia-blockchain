...
class CoinStore:
    ...
    def farm_coin(self, puzzle_hash: bytes32, birthday: CoinTimestamp, amount: int = 1024) -> Coin:
        parent = (birthday.height | self._reward_mask).to_bytes(32, "big")
    ...
    def validate_spend_bundle(
        ...
        for coin_solution in spend_bundle.coin_solutions:
            ...
            coin_announcements.update(coin_announcement_names_for_conditions_dict(conditions_dict, coin_solution.coin))
        ...
        for coin_solution, conditions_dict in zip(spend_bundle.coin_solutions, conditions_dicts):
            ...
            if coin_record is None:
                raise BadSpendBundleError(f"coin not found for id 0x{coin_solution.coin.name().hex()}")
    ...
    def update_coin_store_for_spend_bundle(
        self, spend_bundle: SpendBundle, now: CoinTimestamp, max_cost: int = MAX_COST
    ):
