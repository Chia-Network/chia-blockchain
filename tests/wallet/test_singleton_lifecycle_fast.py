...
ANYONE_CAN_SPEND_WITH_PADDING_PUZZLE_HASH = Program.to(binutils.assemble("(a (q . 1) 3)")).get_tree_hash()  # type: ignore[attr-defined]  # noqa E501
...
def solve_pool_member(solver: Solver, puzzle_db: PuzzleDB, args: List[Program], kwargs: Dict) -> Program:
    ...
    if to_waiting_room:
        ...
        # TODO: address hint error and remove ignore
        #       error: Incompatible return value type (got "SExp", expected "Program")  [return-value]
        return Program.to([0, 1, 0, 0, key_value_list])  # type: ignore[return-value]
...
def solve_pool_waiting_room(solver: Solver, puzzle_db: PuzzleDB, args: List[Program], kwargs: Dict) -> Program:
    ...
    if exit_waiting_room:
        ...
        # TODO: address hint error and remove ignore
        #       error: Incompatible return value type (got "SExp", expected "Program")  [return-value]
        return Program.to([0, 1, key_value_list, destination_puzzle_hash])  # type: ignore[return-value]
    # it's an "absorb_pool_reward" type
    pool_reward_amount = from_kwargs(kwargs, "pool_reward_amount", int)
    pool_reward_height = from_kwargs(kwargs, "pool_reward_height", int)
...
# TODO: address hint error and remove ignore
#       error: "SExp" has no attribute "get_tree_hash"  [attr-defined]
SOLVER.register_solver(ANYONE_CAN_SPEND_PUZZLE.get_tree_hash(), solve_anyone_can_spend)  # type: ignore[attr-defined]
...
class SingletonWallet:
    ...
    def coin_spend_for_conditions(self, puzzle_db: PuzzleDB, **kwargs) -> CoinSpend:
        ...
        #       [arg-type]
        return CoinSpend(coin, puzzle_reveal, solution)  # type: ignore[arg-type]
            ...
            if coin_spend.coin.name() == current_coin_name:
                for coin in coin_spend.additions():
                    if coin.amount & 1 == 1:
                        ...
                        # TODO: address hint error and remove ignore
                        #       error: Incompatible types in assignment (expression has type "SExp", variable has type
                        #       "Program")  [assignment]
                        self.lineage_proof = lineage_proof  # type: ignore[assignment]
...
def launcher_conditions_and_spend_bundle(
    ...
    coin_spend = CoinSpend(launcher_coin, SerializedProgram.from_program(launcher_puzzle), solution)  # type: ignore[arg-type]  # noqa E501
    spend_bundle = SpendBundle([coin_spend], G2Element())
    # TODO: address hint error and remove ignore
    #       error: Incompatible return value type (got "Tuple[Any, List[SExp], SpendBundle]", expected
    #       "Tuple[Any, List[Program], SpendBundle]")  [return-value]
    return launcher_coin.name(), expected_conditions, spend_bundle  # type: ignore[return-value]
...
def claim_p2_singleton(
    ...
    p2_singleton_coin_spend = CoinSpend(
        ...
        p2_singleton_solution,  # type: ignore[arg-type]
    )
    ...
    # TODO: address hint error and remove ignore
    #       error: Incompatible return value type (got "Tuple[CoinSpend, List[SExp]]", expected
    #       "Tuple[CoinSpend, List[Program]]")  [return-value]
    return p2_singleton_coin_spend, singleton_conditions  # type: ignore[return-value]
...
def lineage_proof_for_coin_spend(coin_spend: CoinSpend) -> Program:
    ...
    if coin.puzzle_hash == LAUNCHER_PUZZLE_HASH:
        # TODO: address hint error and remove ignore
        #       error: Incompatible return value type (got "SExp", expected "Program")  [return-value]
        return Program.to([parent_name, amount])  # type: ignore[return-value]
    ...
    # TODO: address hint error and remove ignore
    #       error: Incompatible return value type (got "SExp", expected "Program")  [return-value]
    return Program.to([parent_name, inner_puzzle_hash, amount])  # type: ignore[return-value]
...
def spend_coin_to_singleton(
    ...
    # TODO: address hint error and remove ignore
    #       error: "SExp" has no attribute "get_tree_hash"  [attr-defined]
    farmed_coin = coin_store.farm_coin(ANYONE_CAN_SPEND_PUZZLE.get_tree_hash(), now, amount=farmed_coin_amount)  # type: ignore[attr-defined]  # noqa E501
    now.seconds += 500
    now.height += 1
    ...
    launcher_puzzle_hash = launcher_puzzle.get_tree_hash()
    # TODO: address hint error and remove ignore
    #       error: Argument 1 to "adaptor_for_singleton_inner_puzzle" has incompatible type "SExp"; expected "Program"
    #       [arg-type]
    initial_singleton_puzzle = adaptor_for_singleton_inner_puzzle(ANYONE_CAN_SPEND_PUZZLE)  # type: ignore[arg-type]
    ...
    coin_spend = CoinSpend(farmed_coin, ANYONE_CAN_SPEND_PUZZLE, conditions)  # type: ignore[arg-type]
