...
async def test_p2dohp_wallet_signer_protocol(wallet_environments: WalletTestFramework) -> None:
    ...
    assert utx.signing_instructions.key_hints.sum_hints == [
        SumHint(
            [Program.to(pubkey.get_fingerprint().to_bytes(4, "big"))],
            Program.to(calculate_synthetic_offset(pubkey, DEFAULT_HIDDEN_PUZZLE_HASH)),
        )
    ]
    ...
    assert utx.signing_instructions.targets[0].pubkey == Program.to(synthetic_pubkey)
    assert utx.signing_instructions.targets[0].message == Program.to(message)
    ...
    assert AugSchemeMPL.verify(synthetic_pubkey, message, G2Element.from_bytes(signing_responses[0].signature.atom))
            ...
            key_hints=dataclasses.replace(
                ...
                sum_hints=[
                    *not_our_signing_instructions.key_hints.sum_hints,
                    SumHint([Program.to(not_our_pubkey)], Program.to(None)),
                ],
            ),
        )
        await wallet_state_manager.execute_signing_instructions(not_our_signing_instructions)
            ...
            key_hints=dataclasses.replace(
                ...
                path_hints=[
                    *not_our_signing_instructions.key_hints.path_hints,
                    PathHint(Program.to(not_our_pubkey), [Program.to(0)]),
                ],
            ),
        )
        await wallet_state_manager.execute_signing_instructions(not_our_signing_instructions)
