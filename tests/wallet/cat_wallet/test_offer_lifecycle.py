...
class TestOfferLifecycle:
    ...
    # This method takes a dictionary of strings mapping to amounts and generates the appropriate CAT/XCHI coins
    ...
    async def test_complex_offer(self, setup_sim):
        ...
        try:
            ...
            chinilla_coins: List[Coin] = all_coins[None]
            red_coins: List[Coin] = all_coins["red"]
            blue_coins: List[Coin] = all_coins["blue"]

            # Create an XCHI Offer for RED
            chinilla_requested_payments: Dict[Optional[bytes32], List[Payment]] = {
            ...
            new_offer = Offer.aggregate([chinilla_offer, red_offer])
            ...
            # Create yet another offer of BLUE for XCHI and RED
            ...
            assert new_offer.summary() == (
                ...
                {"xchi": 900, str_to_tail_hash("red").hex(): 350},
            )
