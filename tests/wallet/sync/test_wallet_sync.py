...
class TestWalletSync:
    ...
    async def test_wallet_reorg_sync(self, two_wallet_nodes, default_400_blocks, self_hostname):
        ...
        for wallet_node, wallet_server in wallets:
            wallet = wallet_node.wallet_state_manager.main_wallet
            await time_out_assert(120, wallet.get_confirmed_balance, funds)
            await time_out_assert(120, get_tx_count, 2 * (num_blocks - 1), wallet_node.wallet_state_manager, 1)
        ...
        for wallet_node, wallet_server in wallets:
            wallet = wallet_node.wallet_state_manager.main_wallet
            await time_out_assert(120, get_tx_count, 0, wallet_node.wallet_state_manager, 1)
            await time_out_assert(120, wallet.get_confirmed_balance, 0)
