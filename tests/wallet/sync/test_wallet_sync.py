...
@pytest.mark.asyncio
async def test_request_block_headers(simulator_and_wallet, default_1000_blocks, consensus_mode: Mode):
...
@pytest.mark.asyncio
async def test_basic_sync_wallet(two_wallet_nodes, default_400_blocks, self_hostname, consensus_mode: Mode):
    if consensus_mode != Mode.PLAIN:
        pytest.skip("only run in PLAIN mode to save time")
...
async def test_almost_recent(
    two_wallet_nodes, default_400_blocks, self_hostname, blockchain_constants, consensus_mode: Mode
):
    # Tests the edge case of receiving funds right before the recent blocks  in weight proof
    if consensus_mode != Mode.PLAIN:
        pytest.skip("only run in PLAIN mode to save time")
...
async def test_long_sync_wallet(
    two_wallet_nodes, default_1000_blocks, default_400_blocks, self_hostname, consensus_mode: Mode
):
    if consensus_mode != Mode.PLAIN:
        pytest.skip("only run in PLAIN mode to save time")
...
@pytest.mark.asyncio
async def test_wallet_reorg_sync(two_wallet_nodes, default_400_blocks, self_hostname, consensus_mode: Mode):
    if consensus_mode != Mode.PLAIN:
        pytest.skip("only run in PLAIN mode to save time")
...
@pytest.mark.asyncio
async def test_wallet_reorg_get_coinbase(two_wallet_nodes, default_400_blocks, self_hostname, consensus_mode: Mode):
    if consensus_mode != Mode.PLAIN:
        pytest.skip("only run in PLAIN mode to save time")
...
async def test_bad_peak_mismatch(
    two_wallet_nodes, default_1000_blocks, self_hostname, blockchain_constants, consensus_mode
):
    if consensus_mode != Mode.PLAIN:
        pytest.skip("only run in PLAIN mode to save time")
