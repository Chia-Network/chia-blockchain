...
class TestNFTRPC:
    ...
    async def test_create_rl_coin(self, three_wallet_nodes):
        ...
        val = await api_1.create_new_wallet(
            {"wallet_type": "did_wallet", "did_type": "new", "backup_dids": []}
        )
        ...
        if "success" in val:
            assert val["success"]
        assert val["id"]
        assert val["type"] == WalletType.NFT.value
        user_wallet_id = val["id"]
        pubkey = val["pubkey"]

        api_2 = WalletRpcApi(wallet_node)
        val = await api_2.create_new_wallet(
            {
        ...
        assert (await api_1.get_wallet_balance({"wallet_id": user_wallet_id}))["wallet_balance"][
        ...
        assert await check_balance(api_1, user_wallet_id) == 100
        ...
        val = await api_1.send_transaction({"wallet_id": user_wallet_id, "amount": 3, "fee": 2, "address": address})
        await asyncio.sleep(2)
        assert "transaction_id" in val
        await time_out_assert(15, is_transaction_in_mempool, True, user_wallet_id, api_1, val["transaction_id"])
        for i in range(0, num_blocks):
            await full_node_api.farm_new_transaction_block(FarmNewBlockProtocol(32 * b"\0"))
        await time_out_assert(15, wallet_height_at_least, True, wallet_node, 18)
        assert await is_transaction_confirmed(user_wallet_id, api_1, val["transaction_id"])
        assert await check_balance(api_1, user_wallet_id) == 95
        assert await receiving_wallet.get_spendable_balance() == 3

        val = await api_2.add_rate_limited_funds({"wallet_id": admin_wallet_id, "amount": 100, "fee": 7})
        assert val["status"] == "SUCCESS"
        await asyncio.sleep(2)
        for i in range(0, 50):
            await full_node_api.farm_new_transaction_block(FarmNewBlockProtocol(32 * b"\0"))
        await time_out_assert(15, wallet_height_at_least, True, wallet_node, 68)
        assert await check_balance(api_1, user_wallet_id) == 195
        # test spending
        puzzle_hash = encode_puzzle_hash(await receiving_wallet.get_new_puzzlehash(), "xch")
        val = await api_1.send_transaction(
        ...
        assert await is_transaction_confirmed(user_wallet_id, api_1, val["transaction_id"])
        assert await check_balance(api_1, user_wallet_id) == 90
        assert await receiving_wallet.get_spendable_balance() == 108

        val = await api_2.send_clawback_transaction({"wallet_id": admin_wallet_id, "fee": 11})
        await asyncio.sleep(2)
        await time_out_assert(15, is_transaction_in_mempool, True, user_wallet_id, api_2, val["transaction_id"])
        for i in range(0, num_blocks):
            await full_node_api.farm_new_transaction_block(FarmNewBlockProtocol(32 * b"\0"))
        await time_out_assert(15, wallet_height_at_least, True, wallet_node, 76)
        assert await is_transaction_confirmed(user_wallet_id, api_2, val["transaction_id"])
        assert await check_balance(api_1, user_wallet_id) == 0
