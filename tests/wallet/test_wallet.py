...
from deafwave.protocols.full_node_protocol import RespondBlock
from deafwave.server.server import DeafwaveServer
from deafwave.simulator.simulator_protocol import FarmNewBlockProtocol, ReorgProtocol
...
class TestWalletSimulator:
    ...
    #         outbound: List[WSDeafwaveConnection] = wallet.server.get_outgoing_connections()
    ...
    async def test_wallet_tx_reorg(self, two_wallet_nodes):
        ...
        await full_node_api.reorg_from_index_to_new_index(
            ReorgProtocol(uint32(peak_height - 3),
                          uint32(peak_height + 3), 32 * b"0")
        )
