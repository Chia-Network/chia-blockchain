...
class TestWalletSimulator:
    ...
    async def test_wallet_coinbase(self, wallet_node_sim_and_wallet, trusted, self_hostname):
        ...
        if trusted:
            assert wallet_node.is_trusted(list(wallet_node.server.all_connections.values())[0])
        else:
            assert not wallet_node.is_trusted(list(wallet_node.server.all_connections.values())[0])
    ...
    async def test_wallet_send_to_three_peers(self, three_sim_two_wallets, trusted, self_hostname):
        ...
        wallet_node, wallet_server_0 = wallets[0]
        ...
        tx = await wallet_node.wallet_state_manager.main_wallet.generate_signed_transaction(10, 32 * b"0", 0)
        await wallet_node.wallet_state_manager.main_wallet.push_transaction(tx)

        await time_out_assert_not_none(5, full_node_0.mempool_manager.get_spendbundle, tx.spend_bundle.name())

        # wallet0 <-> sever1
        await wallet_server_0.start_client(PeerInfo(self_hostname, uint16(server_1._port)), wallet_node.on_connect)

        await time_out_assert_not_none(15, full_node_1.mempool_manager.get_spendbundle, tx.spend_bundle.name())

        # wallet0 <-> sever2
        await wallet_server_0.start_client(PeerInfo(self_hostname, uint16(server_2._port)), wallet_node.on_connect)
