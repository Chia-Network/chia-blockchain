...
class TestWalletSimulator:
    ...
    @pytest.mark.asyncio
    async def test_wallet_make_transaction_autofarm(
        self,
        two_wallet_nodes: Tuple[List[FullNodeSimulator], List[Tuple[WalletNode, ChiaServer]]],
        trusted: bool,
        self_hostname: str,
        ...
        await time_out_assert(
            5,
            wallet.get_confirmed_balance,
            funds + calculate_pool_reward(uint32(1)) + calculate_base_farmer_reward(uint32(1)) - 10,
        )
        await time_out_assert(
            5, full_node_api.full_node.mempool_manager.get_spendbundle, None, tx.name
        )  # make sure not in mempool
