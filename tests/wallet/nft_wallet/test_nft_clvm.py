...
def test_transfer_program() -> None:
    ...
    assert conditions.rest().rest().rest().rest().first().rest().rest().first().as_int() == 1
    ...
    #     Program.to((1, [[60, bytes([0] * 32)]])),
    #     Program.to(trade_prices_list),
    # ).run(Program.to([new_pk, STANDARD_PUZZLE_MOD.curry(new_pk).get_tree_hash(), 1, Program.to([])]))
...
def test_ownership_layer() -> None:
    ...
    assert res.rest().rest().rest().rest().rest().first().rest().rest().first().as_int() == 1
    ...
    assert (
        res.rest().rest().rest().rest().rest().first().rest().first().as_atom()
        == curried_ownership_layer.get_tree_hash()
    )
    assert (
...
def test_full_stack() -> None:
    ...
    assert res.rest().rest().rest().rest().rest().rest().first().rest().rest().first().as_int() == 1
    curried_inner = STANDARD_PUZZLE_MOD.curry(destination)
    curried_ownership_layer = NFT_OWNERSHIP_LAYER.curry(
        NFT_OWNERSHIP_LAYER.get_tree_hash(),
        new_did,
    ...
    assert (
        res.rest().rest().rest().rest().rest().rest().first().rest().first().as_atom()
        == curried_state_layer.get_tree_hash()
    )
    assert (
        res.rest().rest().rest().rest().rest().rest().first().rest().rest().rest().first().first().as_atom()
        == curried_inner.get_tree_hash()
    )
