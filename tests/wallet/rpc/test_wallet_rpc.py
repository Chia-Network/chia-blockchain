...
class TestWalletRpc:
    ...
    async def test_wallet_rpc(self, two_wallet_nodes, trusted):
        ...
        try:
            ...
            addr = encode_puzzle_hash(await wallet_node_2.wallet_state_manager.main_wallet.get_new_puzzlehash(), "xchi")
            tx_amount = 15600000
            try:
            ...
            for i in range(0, 5):
                await client.farm_block(encode_puzzle_hash(ph_2, "xchi"))
            ...
            # Test RELEVANCE
            await client.send_transaction("1", 1, encode_puzzle_hash(ph_2, "xchi"))  # Create a pending tx
            ...
            # Creates a wallet and a CAT with 20 chins
            ...
            assert summary == {"offered": {"xchi": 5}, "requested": {col.hex(): 1}}
            ...
            test_config["farmer"]["xchi_target_address"] = encode_puzzle_hash(test_ph, "txchi")
            # set pool to second private key
            sk = await wallet_node.get_key_for_fingerprint(pks[1])
            test_ph = create_puzzlehash_for_pk(master_sk_to_wallet_sk(sk, uint32(0)).get_g1())
            test_config["pool"]["xchi_target_address"] = encode_puzzle_hash(test_ph, "txchi")
