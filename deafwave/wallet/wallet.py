...
from deafwave.consensus.cost_calculator import calculate_cost_of_program, NPCResult
...
from deafwave.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import (
    DEFAULT_HIDDEN_PUZZLE_HASH,
    calculate_synthetic_secret_key,
    puzzle_for_pk,
    solution_for_conditions,
)
from deafwave.wallet.puzzles.puzzle_utils import (
...
from deafwave.wallet.secret_key_store import SecretKeyStore
from deafwave.wallet.sign_coin_solutions import sign_coin_solutions
...
class Wallet:
    ...
    async def get_max_send_amount(self, records=None):
        ...
        max_cost = self.wallet_state_manager.constants.MAX_BLOCK_COST_CLVM / \
            5  # avoid full block TXs
        current_cost = 0
        total_amount = 0
        total_coin_count = 0
    ...
    def make_solution(
        ...
        if primaries:
            for primary in primaries:
                condition_list.append(make_create_coin_condition(
    ...
    async def _generate_unsigned_transaction(
        ...
        if primaries is not None:
            all_primaries_list = [(p["puzzlehash"], p["amount"])
                                  for p in primaries] + [(newpuzzlehash, amount)]
        ...
        for coin in coins:
            ...
            if primary_announcement_hash is None and origin_id in (None, coin.name()):
                if primaries is None:
                    primaries = [
                        {"puzzlehash": newpuzzlehash, "amount": amount}]
                else:
                    primaries.append(
                        {"puzzlehash": newpuzzlehash, "amount": amount})
                if change > 0:
                    ...
                    primaries.append(
                        {"puzzlehash": change_puzzle_hash, "amount": change})
                ...
                solution = self.make_solution(coin_announcements_to_assert=[
                                              primary_announcement_hash])

            spends.append(
                CoinSolution(
                    coin, SerializedProgram.from_bytes(
                        bytes(puzzle)), SerializedProgram.from_bytes(bytes(solution))
                )
            )
    ...
    async def generate_signed_transaction(
        ...
        if primaries is None:
            ...
            non_change_amount = uint64(
                amount + sum(p["amount"] for p in primaries))
        ...
        assert sum(a.amount for a in add_list) + \
            fee == sum(r.amount for r in rem_list)
    ...
    async def create_spend_bundle_relative_deafwave(self, deafwave_amount: int, exclude: List[Coin]) -> SpendBundle:
        list_of_solutions = []
        utxos = None

        # If we're losing value then get coins with at least that much value
        # If we're gaining value then our amount doesn't matter
        if deafwave_amount < 0:
            utxos = await self.select_coins(abs(deafwave_amount), exclude)
        else:
        ...
        deafwave_amount = spend_value + deafwave_amount
        ...
        for coin in utxos:
            ...
            if output_created is None:
                ...
                primaries = [
                    {"puzzlehash": newpuzhash, "amount": deafwave_amount}]
