...
from deafwave.protocols.wallet_protocol import PuzzleSolutionResponse
...
from deafwave.wallet.cc_wallet.cc_info import CCInfo
from deafwave.wallet.cc_wallet.cc_utils import (
    CC_MOD,
    SpendableCC,
...
class CCWallet:
    ...
    async def create_wallet_for_cc(
        ...
        self.cc_info = CCInfo(Program.from_bytes(
            bytes.fromhex(genesis_checker_hex)), [])
        info_as_string = bytes(self.cc_info).hex()
        self.wallet_info = await wallet_state_manager.user_store.create_wallet(
            "CC Wallet", WalletType.COLOURED_COIN, info_as_string
        )
    ...
    async def create(
        ...
        self.standard_wallet = wallet
        self.cc_info = CCInfo.from_bytes(
    ...
    async def get_confirmed_balance(self, record_list: Optional[Set[WalletCoinRecord]] = None) -> uint64:
        ...
        self.log.info(
            f"Confirmed balance for cc wallet {self.id()} is {amount}")
    ...
    async def get_unconfirmed_balance(self, unspent_records=None) -> uint128:
        ...
        self.log.info(
            f"Unconfirmed balance for cc wallet {self.id()} is {result}")
    ...
    async def puzzle_solution_received(self, response: PuzzleSolutionResponse, action_id: int):
        ...
        r = uncurry_cc(puzzle)
        header_hash = self.wallet_state_manager.blockchain.height_to_hash(
            height)
        block: Optional[
            HeaderBlockRecord
    ...
    async def generate_signed_transaction(
        ...
        for coin in selected_coins:
            ...
            spendable_cc_list.append(SpendableCC(
                coin, genesis_id, inner_puzzle, lineage_proof))
    ...
    async def create_spend_bundle_relative_amount(self, cc_amount, zero_coin: Coin = None) -> Optional[SpendBundle]:
        ...
        for coin in cc_spends:
            if output_created is None:
                ...
                innersol = self.standard_wallet.make_solution(
                    primaries=[
                        {"puzzlehash": newinnerpuzhash, "amount": cc_amount}]
                )
                output_created = coin
            else:
                innersol = self.standard_wallet.make_solution(
            ...
            list_of_solutions.append(CoinSolution(
                coin, puzzle_reveal, Program.to(solution)))
