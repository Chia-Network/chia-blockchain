...
from flange.cmds.units import units
...
from flange.rpc.wallet_rpc_client import WalletRpcClient
...
from flange.util.misc import format_minutes
from flange.util.network import is_localhost
...
async def summary(
    ...
    if amounts is not None:
        print(f"Total flange farmed: {amounts['farmed_amount'] / units['flange']}")
        print(f"User transaction fees: {amounts['fee_amount'] / units['flange']}")
        print(f"Block rewards: {(amounts['farmer_reward_amount'] + amounts['pool_reward_amount']) / units['flange']}")
    ...
    if amounts is None:
        if wallet_not_running:
            print("For details on farmed rewards and fees you should run 'flange start wallet' and 'flange wallet show'")
        elif wallet_not_ready:
            print("For details on farmed rewards and fees you should run 'flange wallet show'")
    else:
        print("Note: log into your key using 'flange wallet show' to see rewards for each key")
