...
from flange.ssl.create_ssl import generate_ca_signed_cert, get_flange_ca_crt_key, make_ca_cert
from flange.util.bech32m import encode_puzzle_hash
from flange.util.config import (
    create_default_flange_config,
    initial_config_file,
    load_config,
    save_config,
...
from flange.wallet.derive_keys import master_sk_to_pool_sk, master_sk_to_wallet_sk
...
def check_keys(new_root: Path) -> None:
    ...
    if len(all_sks) == 0:
        print("No keys are present in the keychain. Generate them with 'flange keys generate'")
        return None
...
def create_all_ssl(root: Path):
    ...
    flange_ca_crt, flange_ca_key = get_flange_ca_crt_key()
    flange_ca_crt_path = ca_dir / "flange_ca.crt"
    flange_ca_key_path = ca_dir / "flange_ca.key"
    flange_ca_crt_path.write_bytes(flange_ca_crt)
    flange_ca_key_path.write_bytes(flange_ca_key)
    ...
    flange_ca_crt, flange_ca_key = get_flange_ca_crt_key()
    generate_ssl_for_nodes(ssl_dir, flange_ca_crt, flange_ca_key, False, overwrite=False)
...
def init(create_certs: Optional[Path], root_path: Path):
    if create_certs is not None:
        if root_path.exists():
            ...
            if flange_init(root_path) == 0 and root_path.exists():
                return init(create_certs, root_path)

            print(f"** {root_path} was not created. Exiting **")
            return -1
    else:
        return flange_init(root_path)


def flange_version_number() -> Tuple[str, str, str, str]:
...
def flange_minor_release_number():
    res = int(flange_version_number()[2])
    print(f"Install release number: {res}")
    return res


def flange_full_version_str() -> str:
    major, minor, patch, dev = flange_version_number()
    return f"{major}.{minor}.{patch}{dev}"


def flange_init(root_path: Path):
    if os.environ.get("FLANGE_ROOT", None) is not None:
        print(
            f"warning, your FLANGE_ROOT is set to {os.environ['FLANGE_ROOT']}. "
            f"Please unset the environment variable and run flange init again\n"
            f"or manually migrate config.yaml"
        )

    print(f"Flange directory {root_path}")
    if root_path.is_dir() and Path(root_path / "config" / "config.yaml").exists():
        # This is reached if CHIA_ROOT is set, or if user has run flange init twice
        # before a new update.
    ...
    check_keys(root_path)
    print("")
    print("To see your keys, run 'flange keys show --show-mnemonic-seed'")
