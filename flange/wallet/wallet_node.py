...
from flange.pools.pool_puzzles import SINGLETON_LAUNCHER_HASH
from flange.protocols import wallet_protocol
from flange.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
from flange.protocols.protocol_message_types import ProtocolMessageTypes
from flange.protocols.wallet_protocol import (
...
from flange.server.node_discovery import WalletPeers
...
from flange.util.errors import Err, ValidationError
from flange.util.ints import uint32, uint128
from flange.util.keychain import Keychain
from flange.util.lru_cache import LRUCache
from flange.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
...
from flange.wallet.settings.settings_objects import BackupInitialized
from flange.wallet.transaction_record import TransactionRecord
from flange.wallet.util.backup_utils import open_backup_file
from flange.wallet.util.wallet_types import WalletType
from flange.wallet.wallet_action import WalletAction
from flange.wallet.wallet_blockchain import ReceiveBlockResult
from flange.wallet.wallet_state_manager import WalletStateManager
...
class WalletNode:
    ...
    def get_key_for_fingerprint(self, fingerprint: Optional[int]) -> Optional[PrivateKey]:
        ...
        if len(private_keys) == 0:
            self.log.warning("No keys present. Create keys with the UI, or with the 'flange keys' program.")
            return None
    ...
    async def on_connect(self, peer: WSFlangeConnection):
        if self.wallet_state_manager is None or self.backup_initialized is False:
            return None
    ...
    async def complete_blocks(self, header_blocks: List[HeaderBlock], peer: WSFlangeConnection):
    ...
    async def new_peak_wallet(self, peak: wallet_protocol.NewPeakWallet, peer: WSFlangeConnection):
        if self.wallet_state_manager is None:
            return
    ...
    async def _sync(self) -> None:
        ...
        peers: List[WSFlangeConnection] = self.server.get_full_node_connections()
        if len(peers) == 0:
    ...
    async def get_additions(
        self, peer: WSFlangeConnection, block_i, additions: Optional[List[bytes32]], get_all_additions: bool = False
    ) -> Optional[List[Coin]]:
    ...
    async def get_removals(
        self, peer: WSFlangeConnection, block_i, additions, removals, request_all_removals=False
    ) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
