...
from flange.cmds.init_funcs import flange_init
from flange.daemon.windows_signal import kill
from flange.server.server import ssl_context_for_root, ssl_context_for_server
from flange.ssl.create_ssl import get_mozilla_ca_crt
from flange.util.flange_logging import initialize_logging
...
from flange.util.service_groups import validate_service
from flange.util.setproctitle import setproctitle
from flange.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Flange.")
    quit()
...
service_plotter = "flange plots create"
...
if getattr(sys, "frozen", False):
    name_map = {
        "flange": "flange",
        "flange_wallet": "start_wallet",
        "flange_full_node": "start_full_node",
        "flange_harvester": "start_harvester",
        "flange_farmer": "start_farmer",
        "flange_introducer": "start_introducer",
        "flange_timelord": "start_timelord",
        "flange_timelord_launcher": "timelord_launcher",
        "flange_full_node_simulator": "start_simulator",
    }
...
class WebSocketServer:
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("flange daemon exiting in 5 seconds")
...
async def async_run_daemon(root_path: Path) -> int:
    flange_init(root_path)
    config = load_config(root_path, "config.yaml")
    setproctitle("flange_daemon")
...
def main() -> int:
    from flange.util.default_root import DEFAULT_ROOT_PATH
