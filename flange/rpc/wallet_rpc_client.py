...
from flange.pools.pool_wallet_info import PoolWalletInfo
from flange.rpc.rpc_client import RpcClient
from flange.types.blockchain_format.coin import Coin
from flange.types.blockchain_format.sized_bytes import bytes32
from flange.util.bech32m import decode_puzzle_hash
...
class WalletRpcClient(RpcClient):
    """
    Client to Flange RPC, connects to a local wallet. Uses HTTP/JSON, and converts back from
    JSON into native python objects before returning. All api calls use POST requests.
    Note that this is not the same as the peer protocol, or wallet protocol (which run Flange's
    ...
    async def log_in(self, fingerprint: int) -> Dict:
        try:
            return await self.fetch(
                "log_in",
                {"host": "https://backup.flange.net", "fingerprint": fingerprint, "type": "start"},
            )
    ...
    async def log_in_and_skip(self, fingerprint: int) -> Dict:
        try:
            return await self.fetch(
                "log_in",
                {"host": "https://backup.flange.net", "fingerprint": fingerprint, "type": "skip"},
            )
        except ValueError as e:
            return e.args[0]
