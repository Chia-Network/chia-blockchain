...
from shitcoin.cmds.init_funcs import check_keys, shitcoin_init
from shitcoin.cmds.passphrase_funcs import default_passphrase, using_default_passphrase
from shitcoin.daemon.keychain_server import KeychainServer, keychain_commands
from shitcoin.daemon.windows_signal import kill
from shitcoin.plotters.plotters import get_available_plotters
from shitcoin.plotting.util import add_plot_directory
from shitcoin.server.server import ssl_context_for_root, ssl_context_for_server
...
from shitcoin.util.keychain import (
    Keychain,
...
from shitcoin.util.service_groups import validate_service
from shitcoin.util.setproctitle import setproctitle
from shitcoin.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting shitcoin.")
    quit()
...
service_plotter = "shitcoin_plotter"
...
if getattr(sys, "frozen", False):
    name_map = {
        "shitcoin": "shitcoin",
        "shitcoin_wallet": "start_wallet",
        "shitcoin_full_node": "start_full_node",
        "shitcoin_harvester": "start_harvester",
        "shitcoin_farmer": "start_farmer",
        "shitcoin_introducer": "start_introducer",
        "shitcoin_timelord": "start_timelord",
        "shitcoin_timelord_launcher": "timelord_launcher",
        "shitcoin_full_node_simulator": "start_simulator",
    }
...
class WebSocketServer:
    ...
    def _shitcoinpos_plotting_command_args(self, request: Any, ignoreCount: bool) -> List[str]:
    ...
    def _build_plotting_command_args(self, request: Any, ignoreCount: bool, index: int) -> List[str]:
        plotter: str = request.get("plotter", "shitcoinpos")
        command_args: List[str] = ["shitcoin", "plotters", plotter]

        command_args.extend(self._common_plotting_command_args(request, ignoreCount))

        if plotter == "shitcoinpos":
            command_args.extend(self._shitcoinpos_plotting_command_args(request, ignoreCount))
        elif plotter == "madmax":
    ...
    async def start_plotting(self, request: Dict[str, Any]):
        ...
        plotter = request.get("plotter", "shitcoinpos")
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("shitcoin daemon exiting in 5 seconds")
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered shitcoin_ROOT
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with shitcoin_ROOT: {os.environ['shitcoin_ROOT']}")
    ...
    if service_command == "shitcoin_full_node_simulator":
        # Set the -D/--connect_to_daemon flag to signify that the child should connect
        # to the daemon to access the keychain
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    # When wait_for_unlock is true, we want to skip the check_keys() call in shitcoin_init
    # since it might be necessary to wait for the GUI to unlock the keyring first.
    shitcoin_init(root_path, should_check_keys=(not wait_for_unlock))
    config = load_config(root_path, "config.yaml")
    setproctitle("shitcoin_daemon")
