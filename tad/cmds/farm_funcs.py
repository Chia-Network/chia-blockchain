...
from tad.util.misc import format_bytes
from tad.util.misc import format_minutes
from tad.util.network import is_localhost
...
async def summary(rpc_port: int, wallet_rpc_port: int, harvester_rpc_port: int, farmer_rpc_port: int) -> None:
    ...
    if amounts is not None:
        print(f"Total tad farmed: {amounts['farmed_amount'] / units['tad']}")
        print(f"User transaction fees: {amounts['fee_amount'] / units['tad']}")
        print(f"Block rewards: {(amounts['farmer_reward_amount'] + amounts['pool_reward_amount']) / units['tad']}")
    ...
    if amounts is None:
        if wallet_not_running:
            print("For details on farmed rewards and fees you should run 'tad start wallet' and 'tad wallet show'")
        elif wallet_not_ready:
            print("For details on farmed rewards and fees you should run 'tad wallet show'")
    else:
        print("Note: log into your key using 'tad wallet show' to see rewards for each key")
