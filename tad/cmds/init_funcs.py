...
from tad.ssl.create_ssl import generate_ca_signed_cert, get_tad_ca_crt_key, make_ca_cert
...
def check_keys(new_root: Path) -> None:
    ...
    if "tad_target_address" not in config["farmer"]:
        print(f"Setting the tad destination address for coinbase fees reward to {all_targets[0]}")
    ...
    if "tad_target_address" not in config["pool"]:
        print(f"Setting the tad destination address for coinbase reward to {all_targets[0]}")
...
def create_all_ssl(root: Path):
    ...
    tad_ca_crt_path.write_bytes(tad_ca_crt)
    tad_ca_key_path.write_bytes(tad_ca_key)
