...
from tad.protocols import full_node_protocol, introducer_protocol
from tad.protocols.protocol_message_types import ProtocolMessageTypes
from tad.server.address_manager import AddressManager, ExtendedPeerInfo
from tad.server.address_manager_store import AddressManagerStore
from tad.server.outbound_message import NodeType, make_msg
from tad.server.server import TadServer
from tad.types.peer_info import PeerInfo, TimestampedPeerInfo
...
class FullNodeDiscovery:
    ...
    async def on_connect(self, peer: ws.WSTadConnection):
        if (
    ...
    async def update_peer_timestamp_on_message(self, peer: ws.WSTadConnection):
        if (
            peer.is_outbound
            and peer.peer_server_port is not None
            and peer.connection_type is NodeType.FULL_NODE
    ...
    async def _introducer_client(self):
        ...
        async def on_connect(peer: ws.WSTadConnection):
