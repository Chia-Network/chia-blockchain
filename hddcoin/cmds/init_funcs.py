...
def check_keys(new_root: Path) -> None:
    ...
    if "hdd_target_address" not in config["farmer"]:
        print(f"Setting the hdd destination address for coinbase fees reward to {all_targets[0]}")
    ...
    if "hdd_target_address" not in config["pool"]:
        print(f"Setting the hdd destination address for coinbase reward to {all_targets[0]}")
