...
from chives.wallet.sign_coin_solutions import sign_coin_solutions
...
class Wallet:
    ...
    async def create_spend_bundle_relative_chives(self, chives_amount: int, exclude: List[Coin]) -> SpendBundle:
        list_of_solutions = []
        utxos = None
        ...
        if chives_amount < 0:
            utxos = await self.select_coins(abs(chives_amount), exclude)
        else:
        ...
        chives_amount = spend_value + chives_amount
        ...
        for coin in utxos:
            ...
            if output_created is None:
                ...
                primaries = [{"puzzlehash": newpuzhash, "amount": chives_amount}]
